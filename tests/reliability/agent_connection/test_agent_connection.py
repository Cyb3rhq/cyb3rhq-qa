...
brief: Agents register their state in the file /var/ossec/var/run/cyb3rhq-agentd.state. This state can vary between
       `disconnected`, `connected` and `pending` states. Similarly, the manager records the number of `tcp_sessions`
        in the file /var/ossec/var/run/cyb3rhq-remoted.state. Under normal conditions, the agents should never disconnect
...
def test_agent_connection(get_report):
    ...
    expected_output:
        - None
    '''
    cyb3rhq_target_report_agentd = get_report['agents']['cyb3rhq-agentd']
    cyb3rhq_target_report_remoted = get_report['managers']['cyb3rhq-remoted']

    error_messages = []
    # Ensure TCP sessions is equal to the number of agent
    if not cyb3rhq_target_report_remoted['min_tcp_sessions'] == get_report['metadata']['n_agents']:
    ...
    if not cyb3rhq_target_report_agentd['ever_disconnected'] == 0:
        error_messages += ['Some agents have disconnected']

    if not cyb3rhq_target_report_agentd['ever_pending'] == 0:
        error_messages += ['Some agents have change to pending status']

    if not cyb3rhq_target_report_agentd['begin_status']['connected'] == \
       cyb3rhq_target_report_agentd['end_status']['connected'] == \
