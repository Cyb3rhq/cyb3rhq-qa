...
import pytest
import yaml
from cyb3rhq_testing.tools import CYB3RHQ_SECURITY_CONF
...
def restore_default_security_settings():
    ...
    response = host_manager.make_api_call('cyb3rhq-master', method='DELETE', endpoint='/security/config', token=token)
...
@pytest.mark.parametrize('login_endpoint', [
    ...
    {},
    # Auth context login
    {'user': 'cyb3rhq-wui', 'password': 'cyb3rhq-wui', 'auth_context': {"username": "elastic"}}
])
...
def test_change_rbac_mode_manually(login_endpoint, set_default_api_conf, restore_default_security_settings):
    ...
    host_manager.modify_file_content(test_hosts[0], path=CYB3RHQ_SECURITY_CONF,
                                     content=yaml.safe_dump({'rbac_mode': new_rbac_mode}))

    # Restart the cyb3rhq-manager service
    host_manager.get_host(test_hosts[0]).ansible('command', f'service cyb3rhq-manager restart', check=False)
