...
def test_ruleset_sync_status(update_cluster_json):
    ...
    for host in test_hosts:
        host_manager.clear_file_without_recreate(host=host, file_path=os.path.join(CYB3RHQ_LOGS_PATH, 'cluster.log'))
    ...
    host_manager.modify_file_content(host=test_hosts[0],
                                     path=os.path.join(CYB3RHQ_PATH, 'etc', 'rules', 'local_rules.xml'),
