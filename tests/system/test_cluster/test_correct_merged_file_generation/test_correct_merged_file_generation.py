...
type: system
brief: Cyb3rhq manager should be able to create merged.mg file in order to share files with group of agents.
...
os_version:
    - Debian Buster
references:
    - https://documentation.cyb3rhq.com/current/user-manual/reference/centralized-configuration.html
'''

import os
import pytest
import time
from cyb3rhq_testing import T_1, T_10
...
from cyb3rhq_testing.tools.file import replace_regex_in_file
...
def environment_setting(test_case):
    ...
    assign_agent_to_new_group(testinfra_hosts[0], 'TestGroup1',
                              host_manager.run_command('cyb3rhq-manager',
                                                       f'cut -c 1-3 {CYB3RHQ_PATH}/etc/client.keys'), host_manager)

    if test_case['metadata']['test_type'] == 'on_start':
        host_manager.run_command(testinfra_hosts[0], f'{CYB3RHQ_PATH}/bin/cyb3rhq-control stop')
    time.sleep(T_1)

    yield

    for file in reset_files['default']:
        host_manager.run_command(testinfra_hosts[0], f'rm {CYB3RHQ_PATH}/etc/shared/default/{file}.txt -f')
    for file in reset_files['TestGroup1']:
        host_manager.run_command(testinfra_hosts[0], f'rm {CYB3RHQ_PATH}/etc/shared/TestGroup1/{file}.txt -f')
    delete_agent_group(testinfra_hosts[0], 'TestGroup1', host_manager, 'api')
    host_manager.run_command(testinfra_hosts[0], f'rm -r {CYB3RHQ_PATH}/etc/shared/TestGroup1 -f')
...
def test_correct_merged_file_generation(test_case, environment_setting):
    ...
    if action == "remove":
        host_manager.run_command(testinfra_hosts[0], f'rm {CYB3RHQ_PATH}/etc/shared/default/merged.mg -f')
    if action == "add_files":
        for number in range(number_files):
            files_list.append(f'{file_name}{number}')
        for file in files_list:
            host_manager.run_command(testinfra_hosts[0], f"touch {CYB3RHQ_PATH}/etc/shared/{folder}/{file}.txt")
            if file_content != 'zero':
                host_manager.modify_file_content(host=testinfra_hosts[0],
                                                 path=f"{CYB3RHQ_PATH}/etc/shared/{folder}/{file}.txt",
                                                 content=file_content)

    if test_type == 'on_start' and action == 'remove':

        assert 'merged.mg' not in host_manager.run_command(testinfra_hosts[0],
                                                           f'ls {CYB3RHQ_PATH}/etc/shared/default -la | grep merged')
    ...
    if file_name is not None:
        if number_files >= 1:
            counter_files = 0
            for file in files_list:
                if file in host_manager.run_command(testinfra_hosts[0], f"ls {CYB3RHQ_PATH}/etc/shared/{folder}"):
                    counter_files = counter_files + 1
            assert counter_files == number_files

    assert 'merged.mg' in host_manager.run_command(testinfra_hosts[0], f"ls {CYB3RHQ_PATH}/etc/shared/{folder}")
    ...
    if number_files >= 1:
        for file in files_list:
            ...
            assert merged_value in host_manager.run_command(testinfra_hosts[0],
                                                            f"cat {CYB3RHQ_PATH}/etc/shared/{folder}/merged.mg")
