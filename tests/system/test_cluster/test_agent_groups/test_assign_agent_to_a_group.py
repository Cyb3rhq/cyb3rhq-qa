...
os_version:
    ...
    - Red Hat 6
references:
    - https://github.com/cyb3rhq/cyb3rhq-qa/issues/2507
tags:
    - cluster
"""
import os
...
from cyb3rhq_testing.modules import CYB3RHQ_SERVICE_PREFIX, CYB3RHQ_SERVICES_STOPPED
...
@pytest.mark.parametrize("agent_target", ["cyb3rhq-master", "cyb3rhq-worker1"])
def test_assign_agent_to_a_group(agent_target, initial_status, clean_environment):
    ...
    if (initial_status == AGENT_STATUS_DISCONNECTED):
        host_manager.control_service(host='cyb3rhq-agent1', service=CYB3RHQ_SERVICE_PREFIX, state=CYB3RHQ_SERVICES_STOPPED)
    ...
    try:
        ...
        assign_agent_to_new_group('cyb3rhq-master', test_group, agent_id, host_manager)
        ...
        delete_agent_group('cyb3rhq-master', test_group, host_manager)
