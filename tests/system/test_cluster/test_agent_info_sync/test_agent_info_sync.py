...
# Hosts
testinfra_hosts = ['cyb3rhq-master', 'cyb3rhq-worker1', 'cyb3rhq-worker2']
test_infra_agents = ['cyb3rhq-agent2', 'cyb3rhq-agent3']
...
label = 'test_label'
modified_agent = 'cyb3rhq-agent2'
deleted_agent = 'cyb3rhq-agent3'
while_time = 5
time_to_sync = 21
...
def clean_cluster_logs():
    ...
    for host in testinfra_hosts:
        ...
        host_manager.clear_file(host=host, file_path=os.path.join(CYB3RHQ_LOGS_PATH, 'ossec.log'))

        # Its required to restart each node after clearing the log files
        host_manager.get_host(host).ansible('command', 'service cyb3rhq-manager restart', check=False)

    for agent in test_infra_agents:
        host_manager.clear_file(host=agent, file_path=os.path.join(CYB3RHQ_LOGS_PATH, 'ossec.log'))
        host_manager.get_host(agent).ansible('command', 'service cyb3rhq-agent restart', check=False)


@pytest.fixture(scope='function')
def remove_labels():
    """Remove any label set to the modified cyb3rhq-agent and restart it to apply the new config."""
    yield
    host_manager.add_block_to_file(host=modified_agent, path=f"{CYB3RHQ_PATH}/etc/ossec.conf",
                                   after='</client>', before='<client_buffer>', replace=os.linesep)
    host_manager.get_host(modified_agent).ansible('command', 'service cyb3rhq-agent restart', check=False)
...
def test_agent_info_sync_remove_agent(clean_cluster_logs):
    ...
    agent_list = host_manager.run_command('cyb3rhq-worker2',
                                          f"{CYB3RHQ_PATH}/framework/python/bin/python3 "
                                          f"{script_path} \"{queries[2]}\"")

    assert deleted_agent in agent_list, f"{deleted_agent} was not found in cyb3rhq-worker2\'s global.db"
    ...
    host_manager.get_host(deleted_agent).ansible('command', 'service cyb3rhq-agent stop', check=False)
    ...
    host_manager.get_host(deleted_agent).ansible('command', 'service cyb3rhq-agent restart', check=False)
    check_agent_status('active', master_token, deleted_agent)
