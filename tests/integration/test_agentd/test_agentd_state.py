...
brief: The 'cyb3rhq-agentd' program is the client-side daemon that communicates with the server.
       These tests will check if the content of the 'cyb3rhq-agentd' daemon statistics file is valid.
       The statistics files are documents that show real-time information about the Cyb3rhq environment.
...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/reference/statistics-files/cyb3rhq-agentd-state.html
...
from cyb3rhq_testing.fim import change_internal_options
...
from cyb3rhq_testing.agent import (CLIENT_KEYS_PATH, set_state_interval, callback_ack, callback_keepalive,
...
test_data_file = os.path.join(test_data_path, 'cyb3rhq_state_tests.yaml')
...
if sys.platform == 'win32':
    state_file_path = os.path.join(CYB3RHQ_PATH, 'cyb3rhq-agent.state')
    internal_options = os.path.join(CYB3RHQ_PATH, 'internal_options.conf')
else:
    state_file_path = os.path.join(CYB3RHQ_PATH, 'var', 'run', 'cyb3rhq-agentd.state')
    internal_options = os.path.join(CYB3RHQ_PATH, 'etc', 'internal_options.conf')
...
def test_agentd_state(configure_environment, test_case):
    '''
    description: Check that the statistics file 'cyb3rhq-agentd.state' is created automatically
    ...
    assertions:
        - Verify that the 'cyb3rhq-agentd.state' statistics file has been created.
        - Verify that the information stored in the 'cyb3rhq-agentd.state' statistics file
          is consistent with the connection status to the 'cyb3rhq-remoted' daemon.

    input_description: An external YAML file (cyb3rhq_conf.yaml) includes configuration settings for the agent.
                       Different test cases that are contained in an external YAML file (cyb3rhq_state_tests.yaml)
...
def test_state_delete(restart_cyb3rhq_daemon_after_finishing):
    '''
    description: Check that the statistics file 'cyb3rhq-agentd.state' is deleted automatically
    ...
    assertions:
        - Verify that the 'cyb3rhq-agentd.state' statistics file has been deleted.
    parameters:
        - restart_cyb3rhq_daemon_after_finishing:
            type: fixture
            brief: Restart cyb3rhq modules after finishing the test module.
    '''
...
def wait_connect(update_position=False):
    ...
    Args:
        update_position (bool, optional): update position after reading.
                                          Defaults to False.
    """
    cyb3rhq_log_monitor.start(timeout=240,
