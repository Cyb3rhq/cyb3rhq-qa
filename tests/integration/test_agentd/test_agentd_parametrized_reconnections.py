...
brief: The 'cyb3rhq-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'cyb3rhq-agentd' daemon behaves when there are delays
       between connection attempts to the 'cyb3rhq-remoted' daemon using TCP and UDP protocols.
...
def start_agent(request):
    """Start Cyb3rhq's agent."""
    control_service('start')


@pytest.fixture(scope="function")
def stop_agent(request):
    """Stop Cyb3rhq's agent."""
...
def test_agentd_parametrized_reconnections(configure_authd_server, start_authd, stop_agent, set_keys,
    ...
    parameters:
        ...
        - start_authd:
            type: fixture
            brief: Enable the 'cyb3rhq-authd' daemon to accept connections and perform enrollments.
        - stop_agent:
            type: fixture
            brief: Stop Cyb3rhq's agent.
        - set_keys:
            type: fixture
    ...
    assertions:
        - Verify that when the 'cyb3rhq-agentd' daemon initializes, it connects to
          the 'cyb3rhq-remoted' daemon of the manager before reaching the maximum number of attempts.
