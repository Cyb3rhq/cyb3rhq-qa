...
brief: A Cyb3rhq cluster is a group of Cyb3rhq managers that work together to enhance the availability
...
How does this test work:
    ...
    - SIMULATOR_NUMBERS: Number of simulator to be instantiated, this should match cyb3rhq_conf.yaml
...
def restart_agentd():
    ...
    control_service('stop', daemon="cyb3rhq-agentd")
    truncate_file(LOG_FILE_PATH)
    control_service('start', daemon="cyb3rhq-agentd", debug_mode=True)
...
@pytest.mark.skip(reason='https://github.com/cyb3rhq/cyb3rhq-qa/issues/3536')
def test_agentd_multi_server(add_hostnames, configure_authd_server, set_authd_id, clean_keys, configure_environment,
    ...
    parameters:
        ...
        - set_authd_id:
            type: fixture
            brief: Sets the agent id to '101' in the 'cyb3rhq-authd' simulated connection.
        - clean_keys:
            type: fixture
    ...
    assertions:
        - Agent without keys. Verify that all servers will refuse the connection to the 'cyb3rhq-remoted' daemon
        ...
        - Agent without keys. Verify that the first server only has enrollment available, and the third server
          only has the 'cyb3rhq-remoted' daemon available. The agent should enroll in the first server and
