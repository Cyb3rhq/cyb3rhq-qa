...
brief: The 'cyb3rhq-agentd' program is the client-side daemon that communicates with the server.
       ...
       the 'cyb3rhq-agentd' daemon are working properly. The statistics files are documents that
       show real-time information about the Cyb3rhq environment.
...
from cyb3rhq_testing.agent import (set_state_interval,
...
from cyb3rhq_testing.tools.services import control_service, check_if_process_is_running
...
test_data_file = os.path.join(test_data_path, 'cyb3rhq_state_config_tests.yaml')
...
@pytest.mark.skipif(sys.platform == 'win32', reason="It will be blocked by #1593 and cyb3rhq/cyb3rhq#8746.")
def test_agentd_state_config(test_case, set_local_internal_options):
    '''
    description: Check that the 'cyb3rhq-agentd.state' statistics file is created
    ...
    assertions:
        ...
        - Verify that the 'cyb3rhq-agentd.state' statistics file is updated at the specified intervals.

    input_description: An external YAML file (cyb3rhq_conf.yaml) includes configuration settings for the agent.
                       Different test cases that are contained in an external YAML file (cyb3rhq_state_config_tests.yaml)
    ...
    expected_output:
        ...
        - r'file_not_enabled'
    '''
    control_service('stop', 'cyb3rhq-agentd')
    ...
    if sys.platform == 'win32':
        if test_case['agentd_ends']:
            ...
            assert (test_case['agentd_ends']
                    is not check_if_process_is_running('cyb3rhq-agentd'))
        else:
            control_service('start')
    else:
        control_service('start', 'cyb3rhq-agentd')
        # Sleep enough time to Cyb3rhq load agent.state_interval configuration and
        # boot cyb3rhq-agentd
    ...
    assert cyb3rhq_log_monitor.result()
