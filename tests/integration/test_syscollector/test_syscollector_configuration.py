...
brief: Cyb3rhq gathers information about the agent system (OS, hardware, packages, etc.) periodically in a DB and sends
...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/capabilities/syscollector.html
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/wodle-syscollector.html
'''
import os
import sys

import pytest
from cyb3rhq_testing import ANALYSISD_DAEMON, DB_DAEMON, MODULES_DAEMON, LOG_FILE_PATH, T_10, T_60
from cyb3rhq_testing.modules import TIER0, SERVER, AGENT, LINUX, MACOS, WINDOWS
from cyb3rhq_testing.tools import get_service
...
from cyb3rhq_testing.modules.syscollector import event_monitor as evm
...
# Variables
local_internal_options = {'cyb3rhq_modules.debug': 2}
...
def test_syscollector_deactivation(configuration, metadata, set_cyb3rhq_configuration,
    ...
    parameters:
        ...
        - set_cyb3rhq_configuration:
            type: fixture
            brief: Set cyb3rhq configuration using the configuration template.
        ...
        - daemons_handler_function:
            type: fixture
            brief: Handler of Cyb3rhq daemons for each test case.
...
def test_syscollector_all_scans_disabled(configuration, metadata, set_cyb3rhq_configuration,
                                         configure_local_internal_options_module, truncate_monitored_files,
                                         daemons_handler_function):
    '''
...
def test_syscollector_invalid_configurations(configuration, metadata, set_cyb3rhq_configuration,
                                             configure_local_internal_options_module, truncate_monitored_files,
                                             daemons_handler_function):
    '''
...
@pytest.mark.xfail(reason='Reported in cyb3rhq/cyb3rhq#15413 - To be removed in 4.5: cyb3rhq/cyb3rhq-qa#3798')
def test_syscollector_default_values(configuration, metadata, set_cyb3rhq_configuration,
                                     configure_local_internal_options_module, truncate_monitored_files,
                                     daemons_handler_function):
    '''
...
def test_syscollector_scanning(configuration, metadata, set_cyb3rhq_configuration,
                               configure_local_internal_options_module, truncate_monitored_files,
                               daemons_handler_function):
    '''
