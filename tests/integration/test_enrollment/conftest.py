...
from cyb3rhq_testing.tools import CYB3RHQ_PATH, CLIENT_KEYS_PATH, get_version
...
from cyb3rhq_testing.tools.utils import get_host_name
from cyb3rhq_testing.tools.configuration import load_cyb3rhq_configurations, set_section_cyb3rhq_conf, write_cyb3rhq_conf

test_data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data')
configurations_path = os.path.join(test_data_path, 'cyb3rhq_enrollment_conf.yaml')
...
AUTHDPASS_PATH = os.path.join(CYB3RHQ_PATH, folder, 'authd.pass')
...
SERVER_PEM_PATH = os.path.join(CYB3RHQ_PATH, folder, 'manager.pem')
AGENT_KEY_PATH = os.path.join(CYB3RHQ_PATH, folder, 'agent.key')
AGENT_CERT_PATH = os.path.join(CYB3RHQ_PATH, folder, 'agent.cert')
AGENT_PEM_PATH = os.path.join(CYB3RHQ_PATH, folder, 'agent.pem')
...
@pytest.fixture(scope='function')
def override_cyb3rhq_conf(get_current_test_case, request):
    """
    Re-writes Cyb3rhq configuration file with new configurations from the test case.
    """
    ...
    conf = load_cyb3rhq_configurations(temp, test_name, )
    os.remove(temp)

    test_config = set_section_cyb3rhq_conf(conf[0]['sections'])
