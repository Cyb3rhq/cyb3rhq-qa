...
brief: This module verifies the correct behavior of Cyb3rhq Agentd during the enrollment under different configurations.
components:
    - agentd
...
from cyb3rhq_testing.tools import get_version
from cyb3rhq_testing.tools.services import control_service
from cyb3rhq_testing.tools.file import read_yaml
from cyb3rhq_testing.tools.monitoring import QueueMonitor, make_callback
...
tests = read_yaml(os.path.join(test_data_path, 'cyb3rhq_enrollment_tests.yaml'))
...
def restart_agentd(get_current_test_case):
    ...
    """
    if 'cyb3rhq-agentd' in get_current_test_case.get('skips', []):
...
def test_agentd_enrollment(configure_environment, override_cyb3rhq_conf, get_current_test_case, create_certificates,
    ...
    """
        ...
        parameters:
            ...
            - override_cyb3rhq_conf:
                type: fixture
                brief: Write a particular Cyb3rhq configuration for the test case.
        ...
        input_description:
            Different test cases are contained in an external YAML file (cyb3rhq_enrollment_tests.yaml) which includes the
