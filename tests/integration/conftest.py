...
from cyb3rhq_testing import ALERTS_JSON_PATH, ARCHIVES_JSON_PATH, ARCHIVES_LOG_PATH, global_parameters, logger, mocking
from cyb3rhq_testing.db_interface.agent_db import update_os_info
from cyb3rhq_testing.db_interface.global_db import get_system, modify_system
from cyb3rhq_testing.logcollector import create_file_structure, delete_file_structure
from cyb3rhq_testing.tools import (PREFIX, LOG_FILE_PATH, CYB3RHQ_CONF, get_service, ALERT_FILE_PATH,
                                 CYB3RHQ_LOCAL_INTERNAL_OPTIONS, AGENT_CONF, AGENT_INFO_SOCKET_PATH)
from cyb3rhq_testing.tools import ALERT_FILE_PATH, LOG_FILE_PATH, CYB3RHQ_CONF, CYB3RHQ_LOCAL_INTERNAL_OPTIONS, get_service
from cyb3rhq_testing.tools import (PREFIX, LOG_FILE_PATH, CYB3RHQ_CONF, get_service, ALERT_FILE_PATH,
                                 CYB3RHQ_LOCAL_INTERNAL_OPTIONS)
from cyb3rhq_testing.tools.configuration import get_minimal_configuration, get_cyb3rhq_conf, write_cyb3rhq_conf
from cyb3rhq_testing.tools.file import (truncate_file, recursive_directory_creation, remove_file, copy, write_file,
                                      delete_path_recursively)
from cyb3rhq_testing.tools.monitoring import FileMonitor, QueueMonitor, SocketController, close_sockets
from cyb3rhq_testing.tools.services import check_daemon_status, control_service, delete_dbs
from cyb3rhq_testing.tools.time import TimeMachine
import cyb3rhq_testing.tools.configuration as conf


if sys.platform == 'win32':
    from cyb3rhq_testing.fim import (KEY_WOW64_32KEY, KEY_WOW64_64KEY,
...
###############################
report_files = [LOG_FILE_PATH, CYB3RHQ_CONF, CYB3RHQ_LOCAL_INTERNAL_OPTIONS]
...
def restart_cyb3rhq(get_configuration, request):
    # Stop Cyb3rhq
...
@pytest.fixture(scope='module')
def restart_cyb3rhq_daemon(daemon=None):
    """
    Restart a Cyb3rhq daemon
    """
    truncate_file(LOG_FILE_PATH)
...
def restart_cyb3rhq_daemon_function(daemon=None):
    """
    Restart a Cyb3rhq daemon
    """
    truncate_file(LOG_FILE_PATH)
...
@pytest.fixture(scope='module')
def restart_cyb3rhq_module(daemon=None):
...
@pytest.fixture(scope='module')
def restart_cyb3rhq_daemon_after_finishing(daemon=None):
    """
    Restart a Cyb3rhq daemon
    """
    yield
...
@pytest.fixture(scope='function')
def restart_cyb3rhq_daemon_after_finishing_function(daemon=None):
    """
    Restart a Cyb3rhq daemon
    """
    yield
...
def restart_analysisd_function():
    """Restart cyb3rhq-analysisd daemon before starting a test, and stop it after finishing"""
    control_service('restart', daemon='cyb3rhq-analysisd')
    yield
    control_service('stop', daemon='cyb3rhq-analysisd')
...
@pytest.fixture(scope='module')
def restart_cyb3rhq_alerts(get_configuration, request):
    # Stop Cyb3rhq
    control_service('stop')
    ...
    setattr(request.module, 'cyb3rhq_alert_monitor', file_monitor)
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        help="number of seconds that any timer will wait until an event is generated. This apply to all tests except"
             ...
             "variable from cyb3rhq_testing package"
    )
    parser.addoption(
        "--fim-database-memory",
        action="store_true",
...
def configure_local_internal_options(get_local_internal_options):
    ...
    if backup_options_dict != get_local_internal_options:
        conf.add_cyb3rhq_local_internal_options(get_local_internal_options)

        control_service('restart')

        yield

        conf.set_cyb3rhq_local_internal_options(backup_options_lines)
...
def configure_environment(get_configuration, request):
    """Configure a custom environment for testing. Restart Cyb3rhq is needed for applying the configuration."""

    # Save current configuration
    backup_config = conf.get_cyb3rhq_conf()

    # Configuration for testing
    test_config = conf.set_section_cyb3rhq_conf(get_configuration.get('sections'))
    ...
    conf.write_cyb3rhq_conf(test_config)
    ...
    conf.write_cyb3rhq_conf(backup_config)
...
def set_agent_conf(get_configuration):
    ...
    new_config = conf.set_section_cyb3rhq_conf(sections, backup_config)
...
def daemons_handler_impl(request):
    ...
    try:
        ...
        if 'all_daemons' in daemons_handler_configuration:
            logger.debug(f"Cyb3rhq control set to {daemons_handler_configuration['all_daemons']}")
...
@pytest.fixture()
def set_cyb3rhq_configuration(configuration):
    ...
    # Configuration for testing
    test_config = conf.set_section_cyb3rhq_conf(configuration.get('sections'))
...
def truncate_monitored_files():
    ...
    if get_service() == 'cyb3rhq-manager':
...
def stop_modules_function_after_execution():
    """Stop cyb3rhq modules daemon after finishing a test"""
    yield
    control_service('stop')
...
def load_cyb3rhq_basic_configuration():
    ...
    backup_ossec_configuration = conf.get_cyb3rhq_conf()

    # Write new configuration
    conf.write_cyb3rhq_conf(minimal_configuration)

    yield

    # Restore the ossec.conf backup
    conf.write_cyb3rhq_conf(backup_ossec_configuration)
