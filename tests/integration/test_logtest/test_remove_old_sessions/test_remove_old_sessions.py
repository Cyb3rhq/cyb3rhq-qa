...
from cyb3rhq_testing.logtest import callback_remove_session, callback_session_initialized
...
from cyb3rhq_testing.tools import LOGTEST_SOCKET_PATH
...
# Functions to manage the comunication with Cyb3rhq-logtest
...
def test_remove_old_session(configure_local_internal_options_module,
    ...
    '''
    description: Check if 'cyb3rhq-logtest' correctly detects and handles the situation where trying to use more
                 sessions than allowed. To do this, it creates more sessions than allowed and wait for the message which
                 informs that 'cyb3rhq-logtest' has removed the oldest session.
    ...
    parameters:
        ...
        - restart_required_logtest_daemons:
            type: fixture
            brief: Cyb3rhq logtests daemons handler.
        - wait_for_logtest_startup:
            type: fixture
    ...
    input_description: Some test cases are defined in the module. These include some input configurations stored in
                       the 'cyb3rhq_conf.yaml' and the session creation data from the module.
    ...
    # This session should do Cyb3rhq-logtest to remove the oldest session
    receiver_socket = create_connection()
    receiver_socket.send(msg_create_session, True)
    msg_recived = receiver_socket.receive()[4:]
    msg_recived = msg_recived.decode()
