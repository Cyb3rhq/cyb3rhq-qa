...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/internal-options.html#analysisd
...
def test_remove_old_session_for_inactivity(configure_local_internal_options_module,
    ...
    '''
    description: Check if 'cyb3rhq-logtest' correctly detects and handles the situation where trying to remove old
                 sessions due to inactivity. To do this, it creates more sessions than allowed and waits session_timeout
                 seconds, then checks that 'cyb3rhq-logtest' has removed the session due to inactivity.
