...
def configure_local_decoders(get_configuration):
    ...
    target_file_test = os.path.join(CYB3RHQ_PATH, 'etc', 'decoders', get_configuration['decoder'])
...
def test_invalid_decoder_syntax(get_configuration, configure_local_decoders,
    ...
    '''
    description: Check if `cyb3rhq-logtest` correctly detects and handles errors when processing a decoders file.
    ...
    assertions:
        - Verify that `cyb3rhq-logtest` retrieves errors when the loaded decoders are invalid.
