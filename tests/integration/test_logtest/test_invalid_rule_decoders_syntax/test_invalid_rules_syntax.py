...
def configure_local_rules(get_configuration, request):
    ...
    target_file_test = os.path.join(CYB3RHQ_PATH, 'etc', 'rules', get_configuration['rules'])
...
def test_invalid_rule_syntax(get_configuration, configure_local_rules,
    ...
    '''
    description: Check if `cyb3rhq-logtest` correctly detects and handles errors when processing a rules file.
    ...
    parameters:
        ...
        - configure_local_rules:
            type: fixture
            brief: Configure a custom rule in local_rules.xml for testing. Restart Cyb3rhq is needed for applying the
                   configuration.
        - restart_required_logtest_daemons:
            type: fixture
    ...
    assertions:
        - Verify that `cyb3rhq-logtest` retrieves errors when the loaded rules are invalid.
