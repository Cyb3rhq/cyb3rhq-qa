...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/gcp-pubsub.html#day
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/gcp-pubsub.html#wday
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/gcp-pubsub.html#time
...
from cyb3rhq_testing.gcloud import callback_detect_start_gcp_sleep
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_schedule_conf.yaml')
...
daemons_handler_configuration = {'daemons': ['cyb3rhq-modulesd']}
...
def test_day_wday(tags_to_apply, get_configuration, configure_environment, reset_ossec_log, daemons_handler_module,
    ...
    input_description: Tree test cases are contained in an external YAML file (cyb3rhq_schedule_conf.yaml)
    ...
    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_start_gcp_sleep,
                            error_message='Did not receive expected "Sleeping until ..." event').result()


@pytest.mark.parametrize('tags_to_apply', [
    ({'ossec_day_multiple_conf'}),
    pytest.param({'ossec_wday_multiple_conf'}, marks=pytest.mark.xfail(reason="Unstable because of cyb3rhq/cyb3rhq#15255")),
...
def test_day_wday_multiple(tags_to_apply, get_configuration, configure_environment, reset_ossec_log,
    ...
    set_datetime_info()
    # Get the expected date before the test run to avoid a day difference with Cyb3rhq's scheduled scan
    expected_next_scan_date = today + datetime.timedelta(**kwargs)

    next_scan_time_log = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout + 60,
