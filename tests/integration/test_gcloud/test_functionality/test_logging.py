...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/gcp-pubsub.html#logging
...
from cyb3rhq_testing.gcloud import callback_detect_all_gcp
...
configurations = conf.load_cyb3rhq_configurations(configurations_path, __name__,
                                                params=p, metadata=m)


# fixtures
@pytest.fixture(scope='module', params=[
    {'cyb3rhq_modules.debug': 0,
     'monitord.rotate_log': 0, 'monitord.day_wait': 0,
     'monitord.keep_log_days': 0, 'monitord.size_rotate': 0},
    {'cyb3rhq_modules.debug': 1,
     'monitord.rotate_log': 0, 'monitord.day_wait': 0,
     'monitord.keep_log_days': 0, 'monitord.size_rotate': 0},
...
def configure_local_internal_options_module(get_local_internal_options):
    ...
    import cyb3rhq_testing.tools.services as services
    services.restart_cyb3rhq_daemon('cyb3rhq-modulesd')
...
def test_logging(get_configuration, configure_environment, reset_ossec_log,
    ...
    '''
    description: Check if the 'gcp-pubsub' module generates logs according to the debug level set for cyb3rhq_modules.
    ...
    logging_opt = int([x[-2] for x in conf.get_cyb3rhq_local_internal_options()
                      if x.startswith('cyb3rhq_modules.debug')][0])
