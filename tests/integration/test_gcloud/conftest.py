...
from cyb3rhq_testing.tools.file import write_file, remove_file
from cyb3rhq_testing.gcloud import detect_gcp_start, publish_sync
...
def handle_credentials_file():
    ...
    write_file(os.path.join(CYB3RHQ_PATH, global_parameters.gcp_credentials_file), global_parameters.gcp_credentials)
    yield
    remove_file(os.path.join(CYB3RHQ_PATH, global_parameters.gcp_credentials_file))
...
def configure_internal_options():
    local_internal_options = {'cyb3rhq_modules.debug': 2, 'analysisd.debug': 2, 'monitord.rotate_log': 0, 'monitord.day_wait': 0, 'monitord.keep_log_days': 0,'monitord.size_rotate': 0}
    conf.set_local_internal_options_dict(local_internal_options)
    yield
    local_internal_options = {'cyb3rhq_modules.debug': 0, 'analysisd.debug': 0, "monitord.rotate_log": 1, "monitord.day_wait": 10, "monitord.keep_log_days": 31,'monitord.size_rotate': 512}
    conf.set_local_internal_options_dict(local_internal_options)
