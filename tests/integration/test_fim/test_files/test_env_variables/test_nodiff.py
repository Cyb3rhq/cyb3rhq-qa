...
from cyb3rhq_testing.tools.configuration import load_cyb3rhq_configurations, PREFIX
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_nodiff.yaml')
...
def test_tag_nodiff(directory, filename, hidden_content, get_configuration, put_env_variables, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon truncates the content in the 'diff' files when testing files
    ...
    input_description: A test case (ossec_conf) is contained in external YAML file (cyb3rhq_conf_nodiff.yaml) which
    ...
    regular_file_cud(directory, cyb3rhq_log_monitor, file_list=files,
