...
from cyb3rhq_testing.fim import (LOG_FILE_PATH, REGULAR, callback_detect_event, create_file)
...
def check_event(dirsrc, dirdst, filename, mod_del_event, mod_add_event):
    ...
    mod_add_event : str
        Mode of added event.
    """
    event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event).result()
...
def test_moving_file_to_whodata(dirsrc, dirdst, filename, mod_del_event, mod_add_event, get_configuration,
    ...
    input_description: A test case (monitoring_realtime) is contained in external YAML file (cyb3rhq_conf.yaml)
