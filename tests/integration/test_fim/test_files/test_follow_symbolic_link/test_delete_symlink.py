...
def test_symbolic_delete_symlink(tags_to_apply, main_folder, aux_folder, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' stops detecting events when deleting the monitored symlink.
    ...
    if tags_to_apply == {'monitored_dir'}:
        ...
        cyb3rhq_log_monitor.start(timeout=10, callback=fim.callback_detect_event,
    ...
    fim.wait_for_audit(get_configuration['metadata']['fim_mode'] == "whodata", cyb3rhq_log_monitor)

    fim.modify_file_content(main_folder, file1, new_content='Sample modification 2')
    fim.check_time_travel(scheduled, monitor=cyb3rhq_log_monitor)
    modify = cyb3rhq_log_monitor.start(timeout=10, callback=fim.callback_detect_event).result()
