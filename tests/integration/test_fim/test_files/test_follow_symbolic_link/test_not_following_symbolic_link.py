...
def test_symbolic_monitor_directory_with_symlink(monitored_dir, non_monitored_dir1, non_monitored_dir2,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events when monitoring a directory with a symlink and
    ...
    input_description: A test case (non_monitored_dir) is contained in external YAML file (cyb3rhq_conf.yaml)
    ...
    with pytest.raises(TimeoutError):
        event = cyb3rhq_log_monitor.start(timeout=5, callback=fim.callback_detect_event)
    ...
    fim.check_time_travel(scheduled, monitor=cyb3rhq_log_monitor)
    result = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=fim.callback_detect_event,
