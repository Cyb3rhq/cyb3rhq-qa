...
def test_symbolic_monitor_symlink(tags_to_apply, main_folder, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events when monitoring a symlink that points
    ...
    if tags_to_apply == {'monitored_dir'}:
        ...
        add = cyb3rhq_log_monitor.start(timeout=10, callback=fim.callback_detect_event).result()
    ...
    fim.check_time_travel(scheduled, monitor=cyb3rhq_log_monitor)
    modify = cyb3rhq_log_monitor.start(timeout=10, callback=fim.callback_detect_event,
    ...
    fim.check_time_travel(scheduled, monitor=cyb3rhq_log_monitor)
    delete = cyb3rhq_log_monitor.start(timeout=10, callback=fim.callback_detect_event,
                                     error_message='Did not receive expected '
                                                   '"Sending FIM event: ..." event').result()
