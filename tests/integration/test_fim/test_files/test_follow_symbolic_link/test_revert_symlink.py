...
def test_symbolic_revert_symlink(tags_to_apply, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects new targets when monitoring a directory with
    ...
    input_description: A test case (monitored_file) is contained in external YAML file (cyb3rhq_conf.yaml) which
                       includes configuration settings for the 'cyb3rhq-syscheckd' daemon and, these  are combined
    ...
    def modify_and_assert(file):
        ...
        ev = cyb3rhq_log_monitor.start(timeout=3, callback=fim.callback_detect_event).result()
