...
import pytest
from cyb3rhq_testing.tools import PREFIX, configuration
...
from cyb3rhq_testing import T_5, T_20, T_30, LOG_FILE_PATH
from cyb3rhq_testing.modules import fim
...
from cyb3rhq_testing.modules.fim.utils import regular_file_cud
...
def test_whodata_policy_change(configuration, metadata, set_cyb3rhq_configuration, create_monitored_folders_module,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' is monitoring a in whodata mode in Windows, and the Audit Policies are
    ...
    evm.detect_windows_sacl_configured(cyb3rhq_log_monitor)
    # Check Whodata engine has started
    evm.detect_whodata_start(cyb3rhq_log_monitor)
    ...
    evm.detect_windows_whodata_mode_change(cyb3rhq_log_monitor)

    # Create/Update/Delete file and check events
    cyb3rhq_log_monitor = FileMonitor(LOG_FILE_PATH)
    regular_file_cud(folder, cyb3rhq_log_monitor, file_list=file_list, event_mode=fim.REALTIME_MODE,
