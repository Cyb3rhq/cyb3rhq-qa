...
import pytest
from cyb3rhq_testing import T_60
from cyb3rhq_testing.fim import LOG_FILE_PATH, callback_num_inotify_watches, generate_params, detect_initial_scan
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_num_watches.yaml')
...
configurations1 = load_cyb3rhq_configurations(configurations_path, __name__, params=p, metadata=m)

p, m = generate_params(extra_params={"TEST_DIRECTORIES": testdir1}, modes=['scheduled'])

configurations2 = load_cyb3rhq_configurations(configurations_path, __name__, params=p, metadata=m)
...
def test_num_watches(realtime_enabled, decreases_num_watches, rename_folder, get_configuration, configure_environment,
    ...
    input_description: A test case (num_watches_conf) is contained in external YAML file (cyb3rhq_conf_num_watches.yaml)
    ...
    try:
        num_watches = cyb3rhq_log_monitor.start(timeout=T_60, callback=callback_num_inotify_watches,
    ...
    # If directories have been removed or renamed, create directories again and check Cyb3rhq add watches
