...
from cyb3rhq_testing.fim import detect_initial_scan, callback_ignore_realtime_flag
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_check_realtime.yaml')
...
daemons_handler_configuration = {'daemons': ['cyb3rhq-syscheckd']}
...
def test_realtime_unsupported(get_configuration, configure_environment, file_monitoring,
    ...
    parameters:
        ...
        - daemons_handler_module:
            type: fixture
            brief: Handle the Cyb3rhq daemons.

    assertions:
        - Verify that FIM changes the monitoring mode from 'realtime' to 'scheduled' when it is not supported.

    input_description: A test case (ossec_conf) is contained in external YAML file (cyb3rhq_conf_check_realtime.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon and, it is combined
