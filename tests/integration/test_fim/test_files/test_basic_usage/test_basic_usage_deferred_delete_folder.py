...
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params, create_file, REGULAR, callback_detect_event, \
...
def test_deferred_delete_file(folder, file_list, filetype, tags_to_apply,
    ...
    # Start monitoring
    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_delete_event,
                            accum_results=len(file_list), error_message='Did not receive expected '
                            '"Sending FIM event: ..." event')
