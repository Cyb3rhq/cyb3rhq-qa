...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these
       ...
       'deleted' are generated when files that exist before starting the Cyb3rhq agent are modified.
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, REGULAR, callback_detect_event, \
...
def test_events_from_existing_files(filename, tags_to_apply, get_configuration,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects 'modified' and 'deleted' events when modifying
                 files that exist before the Cyb3rhq agent is started. For this purpose, the test will modify
    ...
    assertions:
        - Verify that FIM events of type 'modified' and 'deleted' are generated
          when files that exist before starting the Cyb3rhq agent are modified.
    ...
    modified_event = cyb3rhq_log_monitor.start(timeout=timeout, callback=callback_detect_event,
    ...
    check_time_travel(scheduled, monitor=cyb3rhq_log_monitor)
    deleted_event = cyb3rhq_log_monitor.start(timeout=timeout, callback=callback_detect_event,
                                            error_message='Did not receive expected '
                                                          '"Sending FIM event: ..." event').result()
