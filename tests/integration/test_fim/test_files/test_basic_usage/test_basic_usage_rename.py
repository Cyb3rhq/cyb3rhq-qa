...
def test_rename(folder, tags_to_apply,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events when renaming directories or files.
    ...
    event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event,
    ...
    if folder == testdir1:
        ...
        deleted = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
                                          callback=callback_detect_event,
                                          error_message='Did not receive expected '
        ...
        added = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
                                        callback=callback_detect_event,
                                        error_message='Did not receive expected '
                                                      '"Sending FIM event: ..." event').result()
        try:
