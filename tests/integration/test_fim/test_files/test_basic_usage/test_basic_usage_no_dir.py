...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these
       files are modified. Specifically, these tests will check if the 'cyb3rhq-syscheckd' daemon generates
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params, callback_empty_directories
...
configuration1 = load_cyb3rhq_configurations(configurations_path, __name__, params=p, metadata=m)

p, m = generate_params(extra_params={'TEST_DIRECTORIES': testdir, 'MODULE_NAME': __name__})
configuration2 = load_cyb3rhq_configurations(configurations_path, __name__, params=p, metadata=m)
...
def test_new_directory(tags_to_apply, get_configuration, configure_environment, restart_syscheckd):
    ...
    assertions:
        ...
        - Verify that the 'cyb3rhq-syscheckd' daemon does not generate a debug log when
