...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#prefilter-cmd
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params, check_fim_start, callback_configuration_error
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_prefilter_cmd_conf.yaml')
...
def test_prefilter_cmd_conf(get_configuration, configure_environment, install_prelink, restart_syscheckd):
    '''
    description: Check if the 'cyb3rhq-syscheckd' detects the 'prelink' program installed on the system and runs it
    ...
    input_description: A test case (prefilter_cmd) is contained in external YAML file (cyb3rhq_prefilter_cmd_conf.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon and, these are combined
    ...
    if os.path.exists(prefilter.split()[0]):
        check_fim_start(cyb3rhq_log_monitor)
    else:
        cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_configuration_error,
