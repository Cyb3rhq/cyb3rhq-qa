...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these files
       are modified. Specifically, these tests will check if the 'cyb3rhq-syscheckd' daemon runs the scans
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#scan-day
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#scan-time
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, callback_detect_end_scan, generate_params, check_time_travel
...
from cyb3rhq_testing.tools.time import reformat_time
...
def test_scan_day_and_time(tags_to_apply,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' performs a scan on the day of the week and the time specified in
    ...
    input_description: A test case (scan_both) is contained in external YAML file (cyb3rhq_conf.yaml) which
                       includes configuration settings for the 'cyb3rhq-syscheckd' daemon. These are combined
    ...
    if scan_today:
        if (scan_time - current_day).days == 0:
            ...
            cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_end_scan,
    ...
    with pytest.raises(TimeoutError):
        event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_end_scan)
        raise AttributeError(f'Unexpected event {event}')
