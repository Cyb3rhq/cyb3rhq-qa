...
import pytest
from cyb3rhq_testing import global_parameters, LOG_FILE_PATH
from cyb3rhq_testing.fim import callback_audit_event_too_long
...
from cyb3rhq_testing.modules.fim.utils import regular_file_cud, generate_params
...
conf_name = "cyb3rhq_recursion_windows.yaml" if sys.platform == "win32" else "cyb3rhq_recursion.yaml"
    ...
    try:
        ...
        for n in range(recursion_level):
            ...
            if ((recursion_level < edge_limit * 2) or
                ...
                regular_file_cud(path, cyb3rhq_log_monitor, time_travel=is_scheduled, min_timeout=timeout)
...
@pytest.mark.skip(reason="It will be blocked by cyb3rhq/cyb3rhq#9298, when it was solve we can enable again this test")
def test_recursion_level(dirname, subdirname, recursion_level, get_configuration, configure_environment,
                         restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events in a monitored directories hierarchy with
    ...
    input_description: A test case (test_recursion_level) is contained in external YAML files
                       (cyb3rhq_recursion.yaml or cyb3rhq_recursion_windows.yaml) which includes
                       configuration settings for the 'cyb3rhq-syscheckd' daemon and the directories
