...
from cyb3rhq_testing.modules import DATA, TIER1, AGENT, WINDOWS, LINUX
from cyb3rhq_testing.modules.fim import TEST_DIR_1, TEST_DIRECTORIES, YAML_CONF_MAX_EPS_SYNC, SCHEDULED_MODE
from cyb3rhq_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS as local_internal_options
from cyb3rhq_testing.modules.fim.event_monitor import (callback_detect_integrity_check_global,
                                                     callback_integrity_sync_message, ERR_MSG_INTEGRITY_CONTROL_MSG,
                                                     ERR_MSG_MULTIPLE_FILES_CREATION)
from cyb3rhq_testing.tools.file import delete_path_recursively, write_file
...
def test_max_eps_sync_valid_within_range(configure_local_internal_options_module, get_configuration,
                                         create_multiple_files, configure_environment, restart_cyb3rhq):
    '''
    ...
    input_description: A test case (max_eps_synchronization) is contained in external YAML file
                       (cyb3rhq_sync_conf_max_eps.yaml) which includes configuration settings for
                       the 'cyb3rhq-syscheckd' daemon and, these are combined with the
    ...
    try:
        ...
        cyb3rhq_log_monitor.start(timeout=TIMEOUT_CHECK_INTEGRATY_START,
        ...
        result = cyb3rhq_log_monitor.start(timeout=TIMEOUT_CHECK_EACH_INTEGRITY_MSG, accum_results=total_file_created,
