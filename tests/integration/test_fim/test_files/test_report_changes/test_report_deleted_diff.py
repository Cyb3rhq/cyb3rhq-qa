...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#directories
...
from cyb3rhq_testing.tools.configuration import get_cyb3rhq_conf, set_section_cyb3rhq_conf, load_cyb3rhq_configurations
...
from cyb3rhq_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS
...
def change_conf(report_value):
    ...
    return load_cyb3rhq_configurations(configurations_path, __name__, params=conf_params, metadata=conf_metadata)
...
def detect_fim_scan(file_monitor, fim_mode):
    """
    Detect initial scan when restarting Cyb3rhq.
...
def wait_for_event(fim_mode):
    ...
    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event,
...
def disable_report_changes(fim_mode):
    ...
    new_ossec_conf = set_section_cyb3rhq_conf(new_conf[0].get('sections'))

    restart_cyb3rhq_with_new_conf(new_ossec_conf)
    # Wait for FIM scan to finish
    detect_fim_scan(cyb3rhq_log_monitor, fim_mode)
...
def test_report_changes_after_restart(get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon deletes the 'diff' folder created in the 'queue/diff/file'
                 ...
                 directory will be deleted after the 'cyb3rhq-syscheckd' daemon restart but will be created again if
                 the 'report_changes' option is still active. To avoid this, the test will disable the 'report_changes'
                 option (backing the main configuration) before restarting the 'cyb3rhq-syscheckd' daemon to ensure that
    ...
    try:
        ...
        restart_cyb3rhq_with_new_conf(backup_conf)
        detect_fim_scan(cyb3rhq_log_monitor, fim_mode)
