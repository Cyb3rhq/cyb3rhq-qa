...
# Cyb3rhq rules
CYB3RHQ_RULES = {'DELETE': 0x00010000,  # DE
...
FILE_GENERIC_WRITE = (CYB3RHQ_RULES['FILE_WRITE_DATA'] |
                      FILE_APPEND_DATA |
                      FILE_WRITE_EA |
                      CYB3RHQ_RULES['FILE_WRITE_ATTRIBUTES'] |
                      READ_CONTROL |
                      SYNCHRONIZE)
...
FILE_MODIIFY_ACCESS = FILE_ALL_ACCESS & ~(FILE_DELETE_CHILD |
                                          CYB3RHQ_RULES['WRITE_DAC'] |
                                          WRITE_OWNER)

FILE_READ_EXEC_ACCESS = FILE_GENERIC_READ | FILE_GENERIC_EXECUTE

FILE_DELETE_ACCESS = CYB3RHQ_RULES['DELETE'] | SYNCHRONIZE
...
def modify_sacl(lfss, mode, mask='DELETE'):
    ...
    if sd.ControlFlags & SE_SACL_PRESENT:
        for entry in sd.SACL:
            ...
            if mode == 'delete' and CYB3RHQ_RULES[mask] & entry.AccessMask == CYB3RHQ_RULES[mask]:
                entry.AccessMask &= ~CYB3RHQ_RULES[mask]
            # Add rule if it does not exist
            elif mode == 'add' and CYB3RHQ_RULES[mask] & entry.AccessMask == 0:
                entry.AccessMask &= CYB3RHQ_RULES[mask]
            else:
                raise ValueError
...
def get_sacl(lfss) -> set:
    ...
    if sd.ControlFlags & SE_SACL_PRESENT:
        if sd.SACL:
            for entry in sd.SACL:
                for name, mask in CYB3RHQ_RULES.items():
