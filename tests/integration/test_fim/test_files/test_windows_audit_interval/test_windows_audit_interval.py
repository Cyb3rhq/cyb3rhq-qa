...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#whodata
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#windows-audit-interval
...
import pytest
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params
...
CYB3RHQ_RULES = {'DELETE', 'WRITE_DAC', 'FILE_WRITE_DATA', 'FILE_WRITE_ATTRIBUTES'}
previous_rules = set()
...
configurations = load_cyb3rhq_configurations(configurations_path, __name__, params=conf_params, metadata=conf_metadata)
...
def extra_configuration_before_yield():
    """Get list of SACL before Cyb3rhq applies its own rules based on whodata monitoring."""
...
def test_windows_audit_modify_sacl(tags_to_apply, get_configuration, configure_environment, restart_syscheckd,
                                   wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects a SACL change every 'windows_audit_interval'
    ...
    input_description: A test case (audit_interval) is contained in external YAML file (cyb3rhq_conf.yaml)
    ...
    with Privilege('SeSecurityPrivilege'):
        # Assert that Cyb3rhq rules are added
        ...
        for rule in CYB3RHQ_RULES:
            assert rule in dir_rules, f'{rule} not found in {dir_rules}'

        # Delete one of them and assert that after the 'windows_audit_interval' thread, Cyb3rhq is set to real-time
        # monitoring
        modify_sacl(lfss, 'delete', mask=next(iter(CYB3RHQ_RULES)))
        dir_rules = get_sacl(lfss)
        assert next(iter(CYB3RHQ_RULES)) not in dir_rules, f'{next(iter(CYB3RHQ_RULES))} found in {dir_rules}'

    event = cyb3rhq_log_monitor.start(timeout=windows_audit_interval, callback=callback_sacl_changed,
...
def test_windows_audit_restore_sacl(tags_to_apply, get_configuration, configure_environment, restart_syscheckd,
                                    wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon restores previous SACL rules when the Cyb3rhq service is stopped.
                 For this purpose, the test will monitor a folder and verify that the Cyb3rhq SACL rules are applied
    ...
    assertions:
        - Verify that an FIM event is generated indicating that previous SACL rules are restored
          when the Cyb3rhq agent is stopped.
    ...
    with Privilege('SeSecurityPrivilege'):
        ...
        assert dir_rules - previous_rules == CYB3RHQ_RULES

        # Stop Cyb3rhq service to force SACL rules to be restored
        control_service('stop')
        event = cyb3rhq_log_monitor.start(timeout=5, callback=callback_sacl_restored,
                                        error_message='Did not receive expected "The SACL '
                                                      'of \'...\' has been restored correctly" event').result()
    ...
    # Start Cyb3rhq service again so the fixture does not crash
