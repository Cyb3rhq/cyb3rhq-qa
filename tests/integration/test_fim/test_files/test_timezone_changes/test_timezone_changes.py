...
import pytest
from cyb3rhq_testing.fim import (LOG_FILE_PATH, REGULAR, callback_detect_event, callback_detect_end_scan, create_file,
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_timezone_conf.yaml')
...
def test_timezone_changes(get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon' detects events when they appear after the 'baseline' scan.
    ...
    input_description: A test case (timezone_conf) is contained in external YAML file (cyb3rhq_timezone_conf.yaml)
    ...
    check_time_travel(True, monitor=cyb3rhq_log_monitor)

    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event_before_end_scan,
