...
from cyb3rhq_testing import LOG_FILE_PATH, REGULAR, global_parameters
...
from cyb3rhq_testing.modules.fim import SCHEDULED_MODE, FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS
from cyb3rhq_testing.modules.fim.event_monitor import (callback_entries_path_count, CB_FILE_LIMIT_CAPACITY,
...
from cyb3rhq_testing.modules.fim.utils import generate_params, wait_for_scheduled_scan
...
def test_file_limit_capacity_alert(percentage, configure_local_internal_options_module, get_configuration,
    ...
    wait_for_scheduled_scan(True, interval=scan_delay, monitor=cyb3rhq_log_monitor)

    # Look for file_limit percentage alert configure value and check it matches with the expected percentage
    if percentage >= 80:
        file_limit_capacity = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    entries, path_count = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
