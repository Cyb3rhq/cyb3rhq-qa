...
from cyb3rhq_testing.fim import LOG_FILE_PATH, delete_file, generate_params, create_file, REGULAR
...
from cyb3rhq_testing.modules.fim.event_monitor import (callback_detect_event, ERR_MSG_DATABASE_FULL_ALERT_EVENT,
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_delete_full.yaml')
...
def test_file_limit_delete_full(folder, file_name, configure_local_internal_options_module, get_configuration,
    ...
    input_description: A test case (tags_delete_full) is contained in external YAML file (cyb3rhq_conf_delete_full.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon and, it is combined with
    ...
    database_state = cyb3rhq_log_monitor.start(timeout=T_20,
    ...
    with pytest.raises(TimeoutError):
        event = cyb3rhq_log_monitor.start(timeout=T_10,
