...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#ignore
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, callback_detect_event, callback_ignore, create_file, REGULAR, \
...
configurations_path = os.path.join(test_data_path,
                                   'cyb3rhq_conf_win32.yaml' if sys.platform == 'win32' else 'cyb3rhq_conf.yaml')
...
def test_ignore_subdirectory(folder, filename, content, triggers_event,
    ...
    input_description: Different test cases are contained in external YAML files
                       (cyb3rhq_conf.yaml or cyb3rhq_conf_win32.yaml) which includes configuration settings for
                       the 'cyb3rhq-syscheckd' daemon and, these are combined with the testing directories
    ...
    if triggers_event:
        event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 2,
        ...
        while True:
            ignored_file = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 2,
