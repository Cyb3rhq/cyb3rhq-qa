...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these
       files are modified. Specifically, these tests will check if the 'cyb3rhq-syscheckd' daemon skips
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#skip-dev
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#skip-nfs
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#skip-proc
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#skip-sys
...
import distro
import pytest
from cyb3rhq_testing import LOG_FILE_PATH, T_20
from cyb3rhq_testing.tools import PREFIX
from cyb3rhq_testing.tools.configuration import set_section_cyb3rhq_conf, load_cyb3rhq_configurations, check_apply_test
from cyb3rhq_testing.tools.file import truncate_file
from cyb3rhq_testing.tools.monitoring import FileMonitor
from cyb3rhq_testing.tools.services import restart_cyb3rhq_with_new_conf
from cyb3rhq_testing.modules.fim.utils import regular_file_cud, generate_params, check_time_travel
from cyb3rhq_testing.modules.fim.event_monitor import (detect_initial_scan, callback_detect_event,
...
def change_conf(dir_value):
    ...
    return load_cyb3rhq_configurations(configurations_path, __name__,
                                     params=p,
                                     metadata=m
                                     )
...
def test_skip_proc(get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon skips the Linux '/proc' directory at scanning when
    ...
    input_description: A test case (skip_proc) is contained in external YAML file (cyb3rhq_conf.yaml) which
    ...
    if trigger:
        ...
        for conf in new_conf:
            if conf['metadata']['skip'] == 'no' and conf['tags'] == ['skip_proc']:
                new_ossec_conf = set_section_cyb3rhq_conf(conf.get('sections'))
        restart_cyb3rhq_with_new_conf(new_ossec_conf)
        ...
        check_time_travel(time_travel=True, monitor=cyb3rhq_log_monitor)
        ...
        with pytest.raises(TimeoutError):
            event = cyb3rhq_log_monitor.start(timeout=3, callback=callback_detect_registry_integrity_state_event)
            raise AttributeError(f'Unexpected event {event}')


@pytest.mark.parametrize('directory, tags_to_apply', [(os.path.join('/', 'dev'), {'skip_dev'})])
@patch('cyb3rhq_testing.fim.modify_file_inode')
def test_skip_dev(modify_inode_mock, directory, tags_to_apply, get_configuration, configure_environment,
                  restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon skips the Linux '/dev' directory at scanning when the
    ...
    input_description: A test case (skip_dev) is contained in external YAML file (cyb3rhq_conf.yaml) which
                       includes configuration settings for the 'cyb3rhq-syscheckd' daemon and the testing
    ...
    regular_file_cud(directory, cyb3rhq_log_monitor, min_timeout=T_20, triggers_event=trigger)
