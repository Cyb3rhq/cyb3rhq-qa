...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_integrity_scan.yaml')
...
def extra_configuration_before_yield():
    # Create 3000 files before restarting Cyb3rhq to make sure the integrity scan will not finish before testing
...
def test_events_while_integrity_scan(tags_to_apply, get_configuration, configure_environment, install_audit,
    ...
    input_description: A test case (synchronize_events_conf) is contained in external YAML file
                       (cyb3rhq_conf_integrity_scan.yaml) which includes configuration settings
    ...
    if get_configuration['metadata']['fim_mode'] == 'whodata':
        value_1 = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 2,
        ...
        value_2 = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 2,
                                          callback=callback_integrity_or_whodata,
                                          error_message='Did not receive expected "File integrity monitoring '
                                                        'real-time Whodata engine started" or '
                                                        '"Executing FIM sync"').result()
    ...
    sending_event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event,
                                            error_message='Did not receive expected '
