...
brief: Check when the 'cyb3rhq-syscheckd' daemon is performing a synchronization, a normal synchronization will end
...
from cyb3rhq_testing.modules.fim import MONITORED_DIR_1, FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS
from cyb3rhq_testing.modules.fim import event_monitor as evm
...
def test_sync_time(configuration, metadata, set_cyb3rhq_configuration, configure_local_internal_options_function,
                   create_files_in_folder, restart_syscheck_function, wait_fim_start):
    '''
    description: Check when the 'cyb3rhq-syscheckd' daemon is performing a synchronization, a normal synchronization
    ...
    input_description: A test case is contained in external YAML file (cases_sync_interval.yaml) which includes
                       configuration settings for the 'cyb3rhq-syscheckd' daemon. That is combined with the interval
    ...
    sync_time = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    results = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
