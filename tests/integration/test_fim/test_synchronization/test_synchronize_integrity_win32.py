...
from cyb3rhq_testing.modules import TIER2, WINDOWS
from cyb3rhq_testing.modules.fim import (WINDOWS_HKEY_LOCAL_MACHINE, KEY_WOW64_64KEY, registry_parser,
                                       REG_SZ, MONITORED_KEY)
from cyb3rhq_testing.modules.fim.event_monitor import (callback_detect_event, callback_detect_file_added_event,
...
from cyb3rhq_testing.modules.fim.utils import create_registry, generate_params, modify_registry_value
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_integrity_scan_win32.yaml')
...
def extra_configuration_before_yield():
    # Create 1000 files before restarting Cyb3rhq to make sure the integrity scan will not finish before testing
...
def test_events_while_integrity_scan(get_configuration, configure_environment, restart_syscheckd,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events while the synchronization is performed
    ...
    input_description: A test case (synchronize_events_conf) is contained in external YAML file
                       (cyb3rhq_conf_integrity_scan_win32.yaml) which includes configuration settings
                       for the 'cyb3rhq-syscheckd' daemon. That is combined with the testing
    ...
    if get_configuration['metadata']['fim_mode'] == 'whodata':
        value_1 = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 5,
                                          callback=callback_integrity_or_whodata,
                                          error_message=ERR_MSG_INTEGRITY_OR_WHODATA_NOT_STARTED).result()

        value_2 = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 5,
                                          callback=callback_integrity_or_whodata,
                                          error_message=ERR_MSG_INTEGRITY_OR_WHODATA_NOT_STARTED).result()
        ...
        cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout * 3,
    ...
    sending_event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout*3,
