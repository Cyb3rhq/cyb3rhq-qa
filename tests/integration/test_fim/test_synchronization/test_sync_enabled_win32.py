...
import pytest
from cyb3rhq_testing import global_parameters, DATA, LOG_FILE_PATH, REGULAR
...
from cyb3rhq_testing.modules.fim import (TEST_DIR_1, WINDOWS_HKEY_LOCAL_MACHINE, MONITORED_KEY,
                                       YAML_CONF_SYNC_WIN32, TEST_DIRECTORIES, TEST_REGISTRIES,
                                       SYNCHRONIZATION_ENABLED, SYNCHRONIZATION_REGISTRY_ENABLED)
from cyb3rhq_testing.modules.fim.event_monitor import (callback_detect_registry_integrity_event,
...
def test_sync_enabled(get_configuration, configure_environment, create_a_file, restart_syscheckd,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon uses the value of the 'enabled' tag to start/stop
    ...
    input_description: Different test cases are contained in external YAML file (cyb3rhq_sync_conf_win32.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon. That is combined with
    ...
    event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, update_position=True,
