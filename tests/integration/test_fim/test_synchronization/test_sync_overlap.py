...
brief: Check if the 'cyb3rhq-syscheckd' daemon is performing a synchronization at the intervals specified in the
...
from cyb3rhq_testing.tools import LOG_FILE_PATH, configuration
from cyb3rhq_testing.tools.monitoring import FileMonitor, generate_monitoring_callback
from cyb3rhq_testing.modules import TIER1, AGENT, SERVER
from cyb3rhq_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS, MONITORED_DIR_1
from cyb3rhq_testing.modules.fim.event_monitor import (callback_detect_synchronization, CB_INVALID_CONFIG_VALUE,
...
def test_sync_overlap(configuration, metadata, set_cyb3rhq_configuration, configure_local_internal_options_function,
                      create_files_in_folder, restart_syscheck_function, wait_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon is performing a synchronization at the interval specified in the
    ...
    input_description: A test case (sync_interval) is contained in external YAML file (cases_sync_overlap.yaml) which
                       includes configuration settings for the 'cyb3rhq-syscheckd' daemon. That is combined with
    ...
    if sys.platform == 'win32' and metadata['lower']:
        pytest.xfail("It will be blocked by cyb3rhq/cyb3rhq-qa#4071")
    ...
    # Wait for new sync
    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_synchronization,
    ...
    interval = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout*5,
    ...
    if not metadata['lower']:
        result = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout*10,
