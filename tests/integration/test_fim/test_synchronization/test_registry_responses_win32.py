...
import sys
import pytest
from cyb3rhq_testing import LOG_FILE_PATH, DATA, CYB3RHQ_SERVICES_START
...
from cyb3rhq_testing.modules.fim.utils import (find_value_in_event_list, get_sync_msgs, generate_params, create_registry,
                                             modify_registry_value)
from cyb3rhq_testing.modules.fim import (FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS, SCHEDULED_MODE, WINDOWS_REGISTRY,
...
from cyb3rhq_testing.modules.fim.event_monitor import detect_initial_scan
...
def test_registry_sync_after_restart(key_name, value_name, configure_local_internal_options_module,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon synchronizes the registry DB when a modification
    ...
    input_description: A test case (registry_sync_responses) is contained in external YAML file
                       (cyb3rhq_conf_registry_responses_win32.yaml) which includes configuration
                       settings for the 'cyb3rhq-syscheckd' daemon. That is combined with the
    ...
    detect_initial_scan(cyb3rhq_log_monitor)
