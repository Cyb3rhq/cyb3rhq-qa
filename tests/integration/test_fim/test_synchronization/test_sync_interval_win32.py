...
from cyb3rhq_testing.fim import LOG_FILE_PATH, callback_detect_synchronization, generate_params, check_time_travel
...
from cyb3rhq_testing.tools.time import time_to_timedelta
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_win32.yaml')
...
def test_sync_interval(get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon performs the file/registry synchronization at the intervals
    ...
    input_description: A test case (sync_interval) is contained in external YAML file (cyb3rhq_conf_win32.yaml)
    ...
    try:
        ...
        pytest.skip("Expected fail due to issue: https://github.com/cyb3rhq/cyb3rhq-qa/issues/947 ")

    check_time_travel(True, interval=interval / 2)
    try:
        result = cyb3rhq_log_monitor.start(timeout=1 if interval.total_seconds() == 10.0 else 3,
