...
from cyb3rhq_testing.fim import LOG_FILE_PATH, create_registry, modify_registry_value, registry_parser, KEY_WOW64_32KEY, \
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_restrict_conf.yaml')
...
def test_restrict_value(key, subkey, arch, value_name, triggers_event, tags_to_apply,
    ...
    input_description: A test case (value_restrict) is contained in external YAML file
                       (cyb3rhq_restrict_conf.yaml) which includes configuration settings
    ...
    check_time_travel(get_configuration['metadata']['fim_mode'] == 'scheduled', monitor=cyb3rhq_log_monitor)
    ...
    if triggers_event:
        ...
        while True:
            ignored_value = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
...
def test_restrict_key(key, subkey, test_subkey, arch, triggers_event, tags_to_apply,
    ...
    if triggers_event:
        ...
        while True:
            ignored_key = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
                                                  callback=callback_restricted,
                                                  error_message='Did not receive expected '
                                                                '"Sending FIM event: ..." event').result()
