...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these files
       ...
       monitored to generate 'diff' information or the 'queue/diff/local' folder where Cyb3rhq stores the
...
import pytest
from cyb3rhq_testing import LOG_FILE_PATH, global_parameters
...
from cyb3rhq_testing.modules.fim import (WINDOWS_HKEY_LOCAL_MACHINE, MONITORED_KEY, MONITORED_KEY_2, KEY_WOW64_32KEY,
                                       KEY_WOW64_64KEY)
from cyb3rhq_testing.modules.fim.event_monitor import ERR_MSG_CONTENT_CHANGES_EMPTY
from cyb3rhq_testing.modules.fim.utils import (generate_params, calculate_registry_diff_paths, create_values_content,
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_registry_report_changes_limits_quota.yaml')
...
def test_all_limits_disabled(key, subkey, arch, value_name, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon generates all FIM events when the 'file_size' and
    ...
    input_description: A test case (test_limits) is contained in external YAML file
                       (cyb3rhq_registry_report_changes_limits_quota.yaml) which includes
                       configuration settings for the 'cyb3rhq-syscheckd' daemon. That is
    ...
    registry_value_create(key, subkey, cyb3rhq_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
    ...
    registry_value_update(key, subkey, cyb3rhq_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
                          scan_delay=scan_delay, min_timeout=global_parameters.default_timeout, triggers_event=True,
                          validators_after_update=[report_changes_validator_diff])
    # Delete the vaue created to clean up enviroment
    registry_value_delete(key, subkey, cyb3rhq_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
                          scan_delay=scan_delay, min_timeout=global_parameters.default_timeout, triggers_event=True)
