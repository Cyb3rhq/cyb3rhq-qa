...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#registry-ignore
...
import pytest
from cyb3rhq_testing import T_20, fim
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_registry_ignore_conf.yaml')
...
def test_ignore_registry_key(root_key, registry, arch, subkey, triggers_event, tags_to_apply,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon ignores the events from keys that are in a monitored subkey
    ...
    input_description: A test case (ignore_registry_key) is contained in an external YAML file
                       (cyb3rhq_registry_ignore_conf.yaml) which includes configuration settings
    ...
    if triggers_event:
        event = cyb3rhq_log_monitor.start(timeout=T_20,
...
def test_ignore_registry_value(root_key, registry, arch, value, triggers_event, tags_to_apply,
                               get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon ignores the events from values that are in a monitored subkey
    ...
    # Let syscheck perform a new scan
    if triggers_event:
        event = cyb3rhq_log_monitor.start(timeout=T_20, callback=fim.callback_value_event,
