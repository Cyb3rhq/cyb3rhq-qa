...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/syscheck.html#registry-nodiff
...
from cyb3rhq_testing.fim import LOG_FILE_PATH, registry_value_cud, KEY_WOW64_32KEY, KEY_WOW64_64KEY, generate_params
...
def test_no_diff_str(key, subkey, arch, value_name, truncated, tags_to_apply,
                     ...
                     wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon truncates the value changes in the generated events using
    ...
    input_description: A test case (no_diff_str) is contained in an external YAML file (cyb3rhq_conf.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon. That is
...
def test_no_diff_regex(key, subkey, arch, value_name, truncated, tags_to_apply,
    ...
    input_description: A test case (no_diff_regex) is contained in an external YAML file (cyb3rhq_conf.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon. That is
                       combined with the testing registry keys to be monitored defined in the module.
