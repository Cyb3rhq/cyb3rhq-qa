...
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params, create_registry, modify_registry_value, delete_registry, \
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_registry_both.yaml')
...
def test_delete_registry(key, subkey, arch, value_list,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects 'deleted' events from the values contained
    ...
    input_description: A test case (ossec_conf_2) is contained in an external YAML file
                       (cyb3rhq_conf_registry_both.yaml) which includes configuration settings for
    ...
    events = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_value_event,
    ...
    event_list = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_value_event,
