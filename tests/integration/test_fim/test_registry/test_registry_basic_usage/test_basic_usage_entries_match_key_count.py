...
import pytest
from cyb3rhq_testing import T_20, LOG_FILE_PATH
...
from cyb3rhq_testing.modules.fim import registry_parser, KEY_WOW64_64KEY, REG_SZ, REG_MULTI_SZ, REG_DWORD
from cyb3rhq_testing.modules.fim.utils import generate_params, create_registry, modify_registry_value
from cyb3rhq_testing.modules.fim.event_monitor import CB_FIM_REGISTRY_ENTRIES_COUNT, CB_FIM_REGISTRY_VALUES_ENTRIES_COUNT
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_reg_attr.yaml')
...
def test_entries_match_key_count(get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects the correct number of events when adding
    ...
    input_description: A test case (ossec_conf_2) is contained in an external YAML file
                       (cyb3rhq_conf_reg_attr.yaml) which includes configuration settings for
    ...
    tags:
        ...
        - time_travel
    '''
    registry_entries = cyb3rhq_log_monitor.start(timeout=T_20, update_position=False,
    ...
    value_entries = cyb3rhq_log_monitor.start(timeout=T_20, callback=callback,
                                            error_message=f'Did not receive expected \
