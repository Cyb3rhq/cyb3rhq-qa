...
from cyb3rhq_testing.fim import LOG_FILE_PATH, registry_value_cud, registry_key_cud, CHECK_GROUP, \
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_ambiguous_simple.yaml')
...
def test_ambiguous_tags(key, sub_keys, arch,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon adds, in the generated events, the tags specified in
    ...
    input_description: A test case (ambiguous_tag) is contained in an external YAML file
                       (cyb3rhq_ambiguous_simple.yaml) which includes configuration settings for
                       the 'cyb3rhq-syscheckd' daemon. That is combined with the testing registry
    ...
    registry_key_cud(key, sub_keys[0], cyb3rhq_log_monitor, arch=arch, time_travel=True,
                     min_timeout=global_parameters.default_timeout, validators_after_cud=[tag_validator])

    registry_key_cud(key, sub_keys[1], cyb3rhq_log_monitor, arch=arch, time_travel=True,
...
def test_ambiguous_recursion(key, subkey, arch,
                             get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects events for a registry key level defined
    ...
    registry_key_cud(key, subkey, cyb3rhq_log_monitor, arch=arch,
                     time_travel=True, triggers_event=False, min_timeout=global_parameters.default_timeout)

    registry_key_cud(key, expected_recursion_key, cyb3rhq_log_monitor, arch=arch,
                     time_travel=True, triggers_event=True, min_timeout=global_parameters.default_timeout)

    registry_value_cud(key, expected_recursion_key, cyb3rhq_log_monitor, arch=arch,
                       time_travel=True, triggers_event=True, min_timeout=global_parameters.default_timeout)
...
def test_ambiguous_checks(key, subkey, key_checkers, subkey_checkers,
                          get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon adds, in the generated events, the 'check_' fields
    ...
    # Test registry keys.
    registry_key_cud(key, subkey[0], cyb3rhq_log_monitor, min_timeout=global_parameters.default_timeout,
                     options=key_checkers, time_travel=get_configuration['metadata']['fim_mode'] == 'scheduled')

    # Test registry values.
    registry_value_cud(key, subkey[0], cyb3rhq_log_monitor, min_timeout=global_parameters.default_timeout,
                       options=key_checkers, time_travel=get_configuration['metadata']['fim_mode'] == 'scheduled')

    # Test registry keys.
    registry_key_cud(key, subkey[1], cyb3rhq_log_monitor, min_timeout=global_parameters.default_timeout,
                     options=subkey_checkers, time_travel=get_configuration['metadata']['fim_mode'] == 'scheduled')

    # Test registry values.
    registry_value_cud(key, subkey[1], cyb3rhq_log_monitor, min_timeout=global_parameters.default_timeout,
                       options=subkey_checkers, time_travel=get_configuration['metadata']['fim_mode'] == 'scheduled')
