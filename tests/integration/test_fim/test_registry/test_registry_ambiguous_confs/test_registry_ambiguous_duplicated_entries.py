...
from cyb3rhq_testing.fim import LOG_FILE_PATH, registry_value_cud, registry_key_cud, \
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_duplicated_entries.yaml')
...
def test_duplicate_entries(key, subkey, arch, key_list, value_list, checkers, tags_to_apply,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon uses the last entry when setting multiple ones with
    ...
    input_description: Diferent test cases are contained in an external YAML file (cyb3rhq_duplicated_entries.yaml)
                       which includes configuration settings for the 'cyb3rhq-syscheckd' daemon. These are combined
    ...
    if key_list is not None:
        registry_key_cud(key, subkey, cyb3rhq_log_monitor, arch=arch, key_list=key_list, options=checkers,
                         min_timeout=global_parameters.default_timeout, time_travel=True, triggers_event=True)

    if value_list is not None:
        registry_value_cud(key, subkey, cyb3rhq_log_monitor, arch=arch, value_list=value_list, options=checkers,
                           min_timeout=global_parameters.default_timeout, time_travel=True, triggers_event=True)
...
def test_duplicate_entries_rc(key, subkey, arch, value_list, tags_to_apply, report_changes,
                              get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon uses the last entry when setting multiple ones
    ...
    input_description: A test case (duplicate_report_entries) is contained in an external YAML file
                       (cyb3rhq_duplicated_entries.yaml) which includes configuration settings for
                       the 'cyb3rhq-syscheckd' daemon. That is combined with the testing registry keys
    ...
    def report_changes_validator(event):
        ...
        for value in value_list:
            ...
            diff_file = os.path.join(CYB3RHQ_PATH, 'queue', 'diff', 'registry', folder_str,
    ...
    registry_value_cud(key, subkey, cyb3rhq_log_monitor, arch=arch, value_list=value_list,
