...
from cyb3rhq_testing.fim import CHECK_OWNER, LOG_FILE_PATH, registry_value_cud, registry_key_cud, \
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_complex_entries.yaml')
...
def test_ambiguous_complex_checks(key, subkey, key_checkers,
    ...
    input_description: A test case (complex_checks) is contained in an external YAML file
                       (cyb3rhq_complex_entries.yaml) which includes configuration settings
                       for the 'cyb3rhq-syscheckd' daemon. That is combined with the
    ...
    # Test registry keys.
    registry_key_cud(key, subkey, cyb3rhq_log_monitor, min_timeout=global_parameters.default_timeout,
...
def test_ambiguous_report_changes(key, subkey, value_list, report,
                                  get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon generates or not the 'content_changes' field for each event
...
def test_ambiguous_report_tags(key, subkey, tag,
                               get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon generates or not the 'tags' field for each event
