...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_multiple_entries.yaml')
...
def test_multiple_entries(tags_to_apply, get_configuration, configure_environment, restart_syscheckd,
    ...
    input_description: A test case (multiple_reg_entries) is contained in external YAML file
                       (cyb3rhq_conf_multiple_entries.yaml) which includes configuration settings
    ...
    multiple_keys_and_entries_keys(n_regs, subkeys, cyb3rhq_log_monitor, KEY, timeout=global_parameters.default_timeout)
    time.sleep(2)  # These 2 seconds are needed to avoid overlapping between keys and values
    multiple_keys_and_entries_values(n_regs, subkeys, cyb3rhq_log_monitor, KEY, timeout=global_parameters.default_timeout)
