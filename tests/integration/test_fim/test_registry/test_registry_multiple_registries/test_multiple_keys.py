...
from cyb3rhq_testing.fim import LOG_FILE_PATH, generate_params, callback_max_registry_monitored, detect_initial_scan
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_conf_multiple_keys.yaml')
...
def test_multiple_keys(tags_to_apply, get_configuration, configure_environment, restart_syscheckd):
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon detects every event when adding, modifying, and deleting
    ...
    input_description: A test case (multiple_keys) is contained in external YAML file (cyb3rhq_conf_multiple_keys.yaml)
    ...
    discarded = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    detect_initial_scan(cyb3rhq_log_monitor)  # Registry scan only works in scheduled mode

    multiple_keys_and_entries_keys(MAX_MONITORED_ONE_TAG, subkeys, cyb3rhq_log_monitor, KEY,
                                   timeout=global_parameters.default_timeout)
    time.sleep(2)  # These 2 seconds are needed to avoid overlapping between keys and values
    multiple_keys_and_entries_values(MAX_MONITORED_ONE_TAG, subkeys, cyb3rhq_log_monitor, KEY,
                                     timeout=global_parameters.default_timeout)
