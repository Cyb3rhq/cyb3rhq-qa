...
from cyb3rhq_testing.fim import LOG_FILE_PATH, registry_value_cud, generate_params, KEY_WOW64_64KEY, KEY_WOW64_32KEY
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_registry_tag_conf.yaml')
...
def test_tags(key, subkey, arch,
    ...
    '''
    description: Check if the 'cyb3rhq-syscheckd' daemon generates the tags required for each event depending
    ...
    input_description: A test case (ossec_conf) is contained in external YAML file (cyb3rhq_registry_tag_conf.yaml)
    ...
    registry_value_cud(key, subkey, cyb3rhq_log_monitor, arch=arch,
