...
from cyb3rhq_testing.cyb3rhq_db import query_wdb
...
messages_file = os.path.join(os.path.join(test_data_path, 'global'), 'cyb3rhq_db_backup_command.yaml')
...
def test_wdb_backup_command(configure_sockets_environment, connect_to_sockets_module, remove_backups,
    ...
    '''
    description: Check that every input message using the 'backup' command in cyb3rhq-db socket generates
                 the proper output to cyb3rhq-db socket. To do this, it performs a series of queries to the socket with
    ...
    input_description:
        - Test cases are defined in the cyb3rhq_db_backup_command.yaml file. This file contains the amount of backups to
