...
brief: Cyb3rhq-db is the daemon in charge of the databases with all the Cyb3rhq persistent information, exposing a socket
       to receive requests and provide information. Cyb3rhq-db has the capability to do automatic database backups, based
...
from cyb3rhq_testing.tools.services import restart_cyb3rhq_function
...
from cyb3rhq_testing.tools.utils import validate_interval_format
from cyb3rhq_testing.modules import TIER0, LINUX, SERVER
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_db_backups_conf.yaml')
backups_path = os.path.join(CYB3RHQ_PATH, 'backup', 'db')
interval = 5
...
def test_wdb_backup_configs(get_configuration, configure_environment, clear_logs, remove_backups, backups_path):
    ...
    description: Check that given different wdb backup configuration parameters, the expected behavior is achieved.
                 For this, the test gets a series of parameters for the cyb3rhq_db_backups_conf.yaml file and applies
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the 
          cyb3rhq_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    try:
        ...
        if not validate_interval_format(test_interval):
            cyb3rhq_log_monitor.start(callback=generate_monitoring_callback(WRONG_INTERVAL_CALLBACK), timeout=timeout,
                                           error_message='Did not receive expected '
                                                         '"Invalid value element for interval..." event')
            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            cyb3rhq_log_monitor.start(callback=generate_monitoring_callback(WRONG_MAX_FILES_CALLBACK), timeout=timeout,
                                           error_message='Did not receive expected '
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        result= cyb3rhq_log_monitor.start(timeout=timeout, accum_results=test_max_files+1,
