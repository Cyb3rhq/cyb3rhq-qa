...
brief: Cyb3rhq-db is the daemon in charge of the databases with all the Cyb3rhq persistent information, exposing a socket
       ...
       This test checks the usage of the cyb3rhqdb getconfig command used to get the current configuration
tier: 0
modules:
    - cyb3rhq_db
components:
...
messages_file = os.path.join(os.path.join(test_data_path, 'global'), 'cyb3rhqdb_getconfig.yaml')
...
def test_sync_agent_groups(configure_sockets_environment, connect_to_sockets_module, test_case):
    '''
    description: Check that commands about cyb3rhqdb getconfig works properly.
    ...
    input_description:
        - Test cases are defined in the cyb3rhqdb_getconfig.yaml file.
    expected_output:
