...
brief: Cyb3rhq-db is the daemon in charge of the databases with all the Cyb3rhq persistent information, exposing a socket
       to receive requests and provide information. The Cyb3rhq core uses list-based databases to store information
       related to agent keys, and FIM/Rootcheck event data.
       Cyb3rhq-db confirms that is able to save, update and erase the necessary information into the corresponding
...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/reference/daemons/cyb3rhq-db.html
...
from cyb3rhq_testing.tools.monitoring import make_callback, CYB3RHQ_DB_PREFIX
from cyb3rhq_testing.tools.services import control_service, delete_dbs
from cyb3rhq_testing.tools.cyb3rhq_manager import remove_all_agents
from cyb3rhq_testing.tools.file import get_list_of_content_yml
...
CYB3RHQ_DB_CHECKSUM_CALCULUS_TIMEOUT = 20
...
monitored_sockets_params = [('cyb3rhq-db', None, True)]
...
def prepare_range_checksum_data():
    ...
    execute_cyb3rhq_db_query(command+json.dumps(payload))
...
def execute_cyb3rhq_db_query(command, single_response=True):
    """Function to send a command to the cyb3rhq-db socket.
...
def validate_cyb3rhq_db_response(expected_output, response):
    """ Method to validate the Cyb3rhq-DB response.
...
def insert_agent(agent_id, agent_name='TestName'):
    ...
    data = execute_cyb3rhq_db_query(command).split()
...
def remove_agent(agent_id):
    ...
    Args:
        agent_id(int): Unique identifier of an agent
    """
    data = execute_cyb3rhq_db_query(f"global delete-agent {agent_id}").split()
...
def pre_insert_packages():
    ...
    for pkg_n in range(PACKAGES_NUMBER):
        ...
        'Cyb3rhq cyb3rhq@cyb3rhq.com',NULL,'{random.randint(1,10)}.0.0','all',NULL,NULL,'Test package {pkg_n}',\
...
def test_cyb3rhq_db_messages_agent(restart_cyb3rhq, clean_registered_agents, configure_sockets_environment,
                                 connect_to_sockets_module, insert_agents_test, test_case):
    '''
    description: Check that every input agent message in cyb3rhq-db socket generates the proper output to cyb3rhq-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of cyb3rhqdb.
    ...
    for index, stage in enumerate(test_case):
        ...
        response = execute_cyb3rhq_db_query(command, False)

        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            match = True if regex_match(expected_output, response) else False
        else:
            match = validate_cyb3rhq_db_response(expected_output, response)
...
def test_cyb3rhq_db_messages_global(connect_to_sockets_module, restart_cyb3rhq, test_case):
    '''
    description: Check that every global input message in cyb3rhq-db socket generates the proper output to cyb3rhq-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_cyb3rhq_db_chunks(restart_cyb3rhq, configure_sockets_environment, clean_registered_agents,
    ...
    description: Check that commands by chunks work properly when the agents' amount exceeds the response maximum size.
                 To do this, it sends a command to the cyb3rhq-db socket and checks the response from the socket.
    ...
    def send_chunk_command(command):
        response = execute_cyb3rhq_db_query(command)
        status = response.split()[0]
...
def test_cyb3rhq_db_range_checksum(restart_cyb3rhq, configure_sockets_environment, connect_to_sockets_module,
    ...
    execute_cyb3rhq_db_query(command)
    log_monitor.start(timeout=CYB3RHQ_DB_CHECKSUM_CALCULUS_TIMEOUT,
                      callback=make_callback('range checksum: Time: ', prefix=CYB3RHQ_DB_PREFIX,
                                             escape=True),
    ...
    execute_cyb3rhq_db_query(command)
    log_monitor.start(timeout=CYB3RHQ_DB_CHECKSUM_CALCULUS_TIMEOUT,
                      callback=make_callback('range checksum avoided', prefix=CYB3RHQ_DB_PREFIX,
                                             escape=True),
                      error_message='Checksum Range wasn´t avoided the second time')


def test_cyb3rhq_db_timeout(configure_sockets_environment,
                          connect_to_sockets_module,
                          pre_insert_packages,
                          pre_set_sync_info):
    """Check that effectively the socket is closed after timeout is reached"""
    cyb3rhq_db_send_sleep = 2
    command = 'agent 000 package get'
    receiver_sockets[0].send(command, size=True)

    # Waiting Cyb3rhq-DB to process command
    time.sleep(cyb3rhq_db_send_sleep)
