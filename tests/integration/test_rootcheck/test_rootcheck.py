...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/capabilities/policy-monitoring/rootcheck
...
from cyb3rhq_testing.tools.agent_simulator import Sender, Injector, create_agents
...
from cyb3rhq_testing.tools.monitoring import SocketController
from cyb3rhq_testing.tools.services import control_service

pytestmark = [pytest.mark.linux, pytest.mark.tier(level=0), pytest.mark.server]

CYB3RHQ_DB_DIR = os.path.join(CYB3RHQ_PATH, 'queue', 'db')
...
def retrieve_rootcheck_rows(agent_id):
    agent_db_path = os.path.join(CYB3RHQ_DB_DIR, f'{agent_id}.db')
...
def test_rootcheck(get_configuration, configure_environment, restart_service,
    ...
    input_description: Different test cases are contained in an external YAML file (cyb3rhq_manager_conf.yaml)
    ...
    expected_output:
        ...
        - Cyb3rhq DB returned an error trying to delete the agent
    ...
    # Service needs to be stopped otherwise db lock will be held by Cyb3rhq db
    ...
    if check_delete:
        ...
        for agent in agents:
            ...
            assert response.startswith(b'ok'), "Cyb3rhq DB returned an error " \
