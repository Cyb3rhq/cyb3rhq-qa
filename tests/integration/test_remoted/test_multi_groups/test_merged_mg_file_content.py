...
brief: Whenever a file is created or deleted from the group directory the merged.mg file must be updated. This test
       checks the content of the merged.mg file that cyb3rhq-remoted compiles for multi-groups.
...
import pytest
import requests
from cyb3rhq_testing.api import get_api_details_dict
from cyb3rhq_testing.remote import DEFAULT_TESTING_GROUP_NAME, new_agent_group, \
                                  remove_agent_group
from cyb3rhq_testing.tools import REMOTE_DAEMON, CYB3RHQ_PATH, configuration
from cyb3rhq_testing.tools.file import delete_file
import cyb3rhq_testing.tools as tools
from cyb3rhq_testing.tools.services import check_daemon_status, control_service
from cyb3rhq_testing.tools.cyb3rhq_manager import remove_agents
...
agent_ip = 'any'
groups_folder = os.path.join(CYB3RHQ_PATH, 'queue', 'agent-groups')
default_group_name = 'default'
groups_list = [default_group_name, DEFAULT_TESTING_GROUP_NAME]
mg_name = hashlib.sha256(','.join(groups_list).encode()).hexdigest()[:8]
mg_folder_path = os.path.join(CYB3RHQ_PATH, 'var', 'multigroups', mg_name)
merged_mg_file = os.path.join(mg_folder_path, 'merged.mg')
shared_folder_path = os.path.join(CYB3RHQ_PATH, 'etc', 'shared')
...
def restart_remoted():
    """Restart the cyb3rhq-remoted daemon."""
...
def prepare_environment(request, register_agent):
    ...
    sb.run([f"{tools.CYB3RHQ_PATH}/bin/agent_groups", "-q", "-a", "-i", agent_id, "-g", 'default'])
    sb.run([f"{tools.CYB3RHQ_PATH}/bin/agent_groups", "-q", "-a", "-i", agent_id, "-g", DEFAULT_TESTING_GROUP_NAME])
...
def test_merged_mg_file_content(metadata, configure_local_internal_options_module, restart_remoted,
                                prepare_environment):
    '''
    description: Check the content of the merged.mg file that cyb3rhq-remoted compiles for multi-groups.

    cyb3rhq_min_version: 4.2.2
    ...
    parameters:
        ...
        - restart_remoted:
            type: fixture
            brief: Restart the cyb3rhq-remoted daemon.
        - prepare_environment:
            type: fixture
