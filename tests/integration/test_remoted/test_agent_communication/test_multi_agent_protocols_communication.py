...
import cyb3rhq_testing.remote as rd
import cyb3rhq_testing.tools.agent_simulator as ag
import cyb3rhq_testing.tools.monitoring as mo

from time import sleep
from cyb3rhq_testing import TCP, UDP, TCP_UDP
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_multi_agent_protocols_communication.yaml')
...
def test_multi_agents_protocols_communication(get_configuration, configure_environment, restart_cyb3rhq):
    '''
    ...
    parameters:
        ...
        - restart_cyb3rhq:
            type: fixture
            brief: Stop Cyb3rhq, reset ossec.log and start a new monitor. Then start Cyb3rhq.
    ...
    input_description: A configuration template (test_multi_agent_protocols_communication) is contained in an external
                       YAML file, (cyb3rhq_multi_agent_protocols_communication.yaml). That template is combined with
                       different test cases defined in the module. Those include configuration settings for the
                       'cyb3rhq-remoted' daemon and agents info.
