...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_multi_agent_status.yaml')
...
def check_active_agents(num_agents=1, manager_address='127.0.0.1', agent_version='4.2.0', agent_os='debian7',
    ...
    Args:
        ...
        agent_version (str): Agent cyb3rhq version.
...
def test_protocols_communication(get_configuration, configure_environment, restart_remoted):
    ...
    input_description: A configuration template (test_multi_agent_status) is contained in an external YAML file,
                       (cyb3rhq_multi_agent_status.yaml). That template is combined with different test cases defined in
                       the module. Those include configuration settings for the 'cyb3rhq-remoted' daemon and agents info.
