...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_protocols_communication.yaml')
...
def validate_agent_manager_protocol_communication(protocol, manager_port):
    ...
    send_message_thread = ThreadExecutor(rd.send_agent_event, {'cyb3rhq_log_monitor': cyb3rhq_log_monitor,
...
def test_protocols_communication(get_configuration, configure_environment, restart_remoted):
    ...
    input_description: A configuration template (test_protocols_communication) is contained in an external YAML file,
                       (cyb3rhq_protocols_communication.yaml). That template is combined with different test cases defined
