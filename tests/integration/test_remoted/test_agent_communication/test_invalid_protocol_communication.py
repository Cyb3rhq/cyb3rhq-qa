...
from cyb3rhq_testing import TCP, UDP
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_invalid_protocol_communication.yaml')
...
def test_invalid_protocol_communication(get_configuration, configure_environment, restart_remoted):
    ...
    input_description: A configuration template (test_invalid_protocol_communication) is contained in an external YAML
                       file, (cyb3rhq_invalid_protocol_communication.yaml). That template is combined with different test
                       cases defined in the module. Those include configuration settings for the 'cyb3rhq-remoted' daemon
                       and agents info.
