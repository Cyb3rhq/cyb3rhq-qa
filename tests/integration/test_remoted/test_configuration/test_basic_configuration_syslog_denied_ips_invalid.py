...
brief: The 'cyb3rhq-remoted' program is the server side daemon that communicates with the agents.
       Specifically, this test will check that 'cyb3rhq-remoted' doesn't start and produces an error
...
import cyb3rhq_testing.generic_callbacks as gc
from cyb3rhq_testing.tools import CYB3RHQ_CONF_RELATIVE
...
def test_denied_ips_syslog_invalid(get_configuration, configure_environment, restart_remoted):
    '''
    description: Check that 'cyb3rhq-remoted' fails when 'denied-ips' has invalid values.
    ...
    log_callback = gc.callback_error_in_configuration('ERROR', prefix=REMOTED_DETECTOR_PREFIX,
                                                      conf_path=CYB3RHQ_CONF_RELATIVE)
    cyb3rhq_log_monitor.start(timeout=5, callback=log_callback,
                            error_message="The expected error output has not been produced")
