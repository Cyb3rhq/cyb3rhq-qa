...
configurations_path = os.path.join(test_data_path, 'data', 'cyb3rhq_basic_configuration.yaml')
...
configurations = load_cyb3rhq_configurations(configurations_path, 'test_basic_configuration_allowed_denied_ips',
...
def test_denied_ips_syslog(get_configuration, configure_environment, restart_remoted, wait_for_remoted_start_log):
    '''
    description: Check that 'cyb3rhq-remoted' denied connection to the specified 'denied-ips'.
    ...
    input_description: A configuration template (test_basic_configuration_allowed_denied_ips) is contained in an
                       external YAML file, (cyb3rhq_basic_configuration.yaml). That template is combined with different
                       test cases defined in the module. Those include configuration settings for the 'cyb3rhq-remoted'
                       daemon and agents info.

    expected_output:
        - r'Started <pid>: .* Listening on port .*'
        - Cyb3rhq remoted did not start as expected.
    ...
    cyb3rhq_log_monitor.start(timeout=remote.REMOTED_GLOBAL_TIMEOUT, callback=log_callback,
