...
import pytest
import sys
import cyb3rhq_testing.tools.file as file
...
def check_log_format_valid(cfg):
    """Checks if Cyb3rhq runs correctly with the specified log formats.
...
def check_log_format_value_valid(conf):
    """Checks if Cyb3rhq runs correctly with the correct log format and specific content.
...
def check_log_format_value_invalid(conf):
    """Check if Cyb3rhq fails because of content invalid log.
   ...
   """
    ...
    if conf['log_format'] not in log_format_windows_print_analyzing_info:
        ...
        cyb3rhq_log_monitor.start(timeout=5, callback=log_callback, error_message=logcollector.GENERIC_CALLBACK_ERROR)
...
def test_log_format(configure_local_internal_options_module, get_configuration, configure_environment):
    '''
    description: Check if the 'cyb3rhq-logcollector' accepts only allowed values for the 'log_format' tag, and the content
    ...
    input_description: A configuration template (test_log_format_values) is contained in an external YAML
                       file (cyb3rhq_conf.yaml). That template is combined with different test cases defined
