...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#command
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#alias
...
from cyb3rhq_testing.tools.monitoring import LOG_COLLECTOR_DETECTOR_PREFIX
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_command_conf.yaml')
...
def test_command_execution(configure_local_internal_options_module, get_configuration,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon runs commands with special characteristics.
    ...
    input_description: A configuration template (test_command_execution) is contained in an external
                       YAML file (cyb3rhq_command_conf.yaml), which includes configuration settings for
                       the 'cyb3rhq-logcollector' daemon and, it is combined with the test cases
                       (log formats and commands to run) defined in the module.

    expected_output:
        - r'.*cyb3rhq-logcollector.*' (this log must indicate the command executed)
