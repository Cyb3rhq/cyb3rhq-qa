...
from cyb3rhq_testing.tools import monitoring, file
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_keep_running_conf.yaml')
temp_dir = tempfile.gettempdir()
log_test_path = os.path.join(temp_dir, 'cyb3rhq-testing', 'test_log.log')
...
def test_keep_running(configure_local_internal_options_module, get_configuration, file_monitoring,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon keeps running once a log is rotated or truncated.
    ...
    input_description: A configuration template (test_keep_running) is contained in an external YAML file
                       (cyb3rhq_keep_running_conf.yaml). That template is combined with two test cases defined
