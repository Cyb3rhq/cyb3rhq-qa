...
configurations_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data', 'invalid_cyb3rhq_conf.yaml')
...
def test_invalid_cyb3rhq_conf(get_files_list, create_file_structure_module, get_configuration, remove_empty_options,
    ...
    description: Check if the expected message is present in the ossec.log when an invalid <localfile> configuration is
                 set and if Cyb3rhq refuses to restart.
