...
from cyb3rhq_testing.tools.services import get_service
...
def test_restrict_ignore_regex_values(configuration, metadata, new_file_path, create_file, truncate_monitored_files,
    ...
    expected_output:
        - r".*cyb3rhq-logcollector.*Analizing file: '{file}'.*"
        - r".*cyb3rhq-logcollector.*DEBUG: Reading syslog '{message}'.*"
        - r".*cyb3rhq-logcollector.*DEBUG: Ignoring the log line '{message}' due to {tag} config: '{regex}'"
    '''
