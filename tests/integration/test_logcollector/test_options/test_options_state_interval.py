...
def get_local_internal_options(request):
    ...
    if sys.platform == 'win32':
        conf.add_cyb3rhq_local_internal_options({'\n windows.debug': '2'})
    else:
        conf.add_cyb3rhq_local_internal_options({'\n logcollector.debug': '2'})
    conf.add_cyb3rhq_local_internal_options({'logcollector.state_interval': request.param})
...
def test_options_state_interval(get_local_internal_options, file_monitoring):
    ...
    description: Check if the 'cyb3rhq-logcollector' daemon updates the statistic file 'cyb3rhq-logcollector.state'
                 ...
                 beetwen the allowed limits, if not, it will verify that the Cyb3rhq is stopped (on Windows systems)
                 or an error event is generated (on Linux systems). Finally, if the interval value is valid, it
                 will verify that the 'cyb3rhq-logcollector.state' file is updated at the specified intervals by
