...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_configuration.yaml')
...
metadata = [
    {'location': os.path.join(temp_dir, 'cyb3rhq-testing', 'test.txt'), 'log_format': 'syslog', 'regex': False},
    {'location': os.path.join(temp_dir, 'cyb3rhq-testing', '*'), 'log_format': 'syslog', 'regex': True},
]
...
def check_cyb3rhq_logcollector_status_file(file):
...
def test_options_state_interval_no_file(get_local_internal_options_function, configure_local_internal_options_function,
    ...
    description: Check if the 'cyb3rhq-logcollector' daemon updates the statistic file 'cyb3rhq-logcollector.state'
                 ...
                 'cyb3rhq-logcollector.state' file has been created and contains references to the monitored
                 ...
                 the log file is unavailable is generated. Finally, it will wait until the 'cyb3rhq-logcollector.state'
    ...
    assertions:
        - Verify that the logcollector updates the 'cyb3rhq-logcollector.state' file
    ...
    input_description: A configuration template (test_options) is contained in an external YAML file
                       (cyb3rhq_configuration.yaml). That template is combined with different test cases
    ...
    if not use_regex:
        pytest.xfail('Xfailing due to issue: https://github.com/cyb3rhq/cyb3rhq/issues/10783')
    ...
    for file in get_files_list:
        for name in file['filename']:
            ...
            assert all(check_cyb3rhq_logcollector_status_file(log_path))
            ...
            if use_regex:
                assert not any(check_cyb3rhq_logcollector_status_file(log_path)), f"Using regex as location, file \
                                                                                 {location} has not been deleted from \
                                                                                 {LOGCOLLECTOR_STATISTICS_FILE}"
            else:
                assert all(check_cyb3rhq_logcollector_status_file(log_path)),  f"Using hardcoded location, file \
