...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       tag and the Cyb3rhq API returns the same values for the configured 'localfile' section.
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#age
...
from cyb3rhq_testing.tools.monitoring import LOG_COLLECTOR_DETECTOR_PREFIX, WINDOWS_AGENT_DETECTOR_PREFIX
...
if sys.platform == 'win32':
    ...
    cyb3rhq_configuration = 'ossec.conf'
    ...
    cyb3rhq_configuration = 'etc/ossec.conf'
...
def check_configuration_age_valid(cfg):
    """Check if the Cyb3rhq module runs correctly and analyze the desired file.
...
def check_configuration_age_invalid(cfg):
    """Check if the Cyb3rhq fails because the invalid age configuration value.
    ...
    log_callback = gc.callback_error_in_configuration('ERROR', prefix,
                                                      conf_path=f'{cyb3rhq_configuration}')
    cyb3rhq_log_monitor.start(timeout=5, callback=log_callback,
                            error_message=gc.GENERIC_CALLBACK_ERROR_MESSAGE)
...
def test_configuration_age(get_configuration, configure_environment):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon detects invalid configurations for the 'age' tag.
    ...
    input_description: A configuration template (test_basic_configuration_age) is contained in an external YAML file
                       (cyb3rhq_basic_configuration.yaml). That template is combined with different test cases defined
    ...
    if cfg['valid_value']:
        ...
        if cfg['age'] in problematic_values:
            pytest.xfail("Logcollector accepts invalid values: https://github.com/cyb3rhq/cyb3rhq/issues/8158")
        else:
            if sys.platform == 'win32':
