...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'ignore_binaries' tag and the Cyb3rhq API returns the same values for the configured
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#ignore_binaries
...
from cyb3rhq_testing.tools import get_service, LOG_FILE_PATH
...
from cyb3rhq_testing.modules.logcollector import LOG_COLLECTOR_PREFIX, WINDOWS_AGENT_PREFIX
from cyb3rhq_testing.modules.logcollector.event_monitor import check_win_wildcard_pattern_no_match
...
def check_ignore_binaries_valid(cfg):
    """Check if the Cyb3rhq runs correctly with the specified ignore_binaries field value.
...
def check_ignore_binaries_invalid(cfg):
    """Check if the Cyb3rhq fails using a invalid ignore_binaries configuration value.
...
def test_ignore_binaries(get_configuration, configure_environment):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon detects invalid configurations for the 'ignore_binaries' tag.
    ...
    input_description: A configuration template (test_basic_configuration_ignore_binaries) is contained in
                       an external YAML file (cyb3rhq_basic_configuration.yaml). That template is combined
