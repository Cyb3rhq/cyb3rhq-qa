...
from cyb3rhq_testing.tools.monitoring import WINDOWS_AGENT_DETECTOR_PREFIX, FileMonitor, LOG_COLLECTOR_DETECTOR_PREFIX
...
def check_only_future_events_valid(cfg):
    """Check if Cyb3rhq runs correctly with the specified only future events field.
...
def check_only_future_events_invalid(cfg):
    """Check if Cyb3rhq fails due to a invalid only future events configuration value.
...
def test_only_future_events(get_configuration, configure_environment, generate_macos_logs, restart_logcollector):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon detects invalid settings for the 'only-future-events',
