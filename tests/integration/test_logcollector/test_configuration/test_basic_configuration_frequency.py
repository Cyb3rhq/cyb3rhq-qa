...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       'frequency' tag and the Cyb3rhq API returns the same values for the configured 'localfile' section.
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#frequency
...
from cyb3rhq_testing.tools.monitoring import LOG_COLLECTOR_DETECTOR_PREFIX, WINDOWS_AGENT_DETECTOR_PREFIX, FileMonitor
...
def check_configuration_frequency_valid(cfg):
    """Check if the Cyb3rhq module runs correctly and that analyze the desired file.
...
def check_configuration_frequency_invalid(cfg):
    """Check if the Cyb3rhq fails because an invalid frequency configuration value.
...
def test_configuration_frequency(configure_local_internal_options_module,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon detects invalid configurations for the 'frequency' tag.
