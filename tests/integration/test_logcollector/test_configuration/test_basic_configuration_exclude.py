...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'exclude' tag is set in the configuration, and the Cyb3rhq API returns the same values for
...
from cyb3rhq_testing.tools.services import check_if_process_is_running, get_service
...
def test_configuration_exclude(get_configuration, configure_environment, file_monitoring, restart_logcollector):
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon starts properly when the 'exclude' tag is used.
                 ...
                 will verify that the Cyb3rhq component is started by checking its process, and the Cyb3rhq API
    ...
    assertions:
        - Verify that the Cyb3rhq component (agent or manager) can start when the 'exclude' tag is used.
    ...
    if sys.platform == 'win32':
        assert check_if_process_is_running('cyb3rhq-agent.exe')
    else:
        assert check_if_process_is_running('cyb3rhq-logcollector')
