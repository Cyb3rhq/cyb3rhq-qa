...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#only-future-events
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_macos_format_only_future_events.yaml')
...
def test_macos_format_only_future_events(restart_logcollector_required_daemons_package, get_configuration,
    ...
    """
    description: Check if the 'only-future-events' option is used properly by the 'cyb3rhq-logcollector' when
                 ...
                 will stop the 'cyb3rhq-logcollector' daemon, generate a ULS event, and start it again. The test
    ...
    input_description: A configuration template (test_macos_format_only_future_events) is contained in an external
                       YAML file (cyb3rhq_macos_format_only_future_events.yaml). That template is combined with two
