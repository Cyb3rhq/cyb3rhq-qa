...
from cyb3rhq_testing.tools.monitoring import FileMonitor, wait_file
...
configurations_path = join(test_data_path, 'cyb3rhq_macos_file_status_basic.yaml')
...
def test_macos_file_status_basic(restart_logcollector_required_daemons_package, truncate_log_file,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' builds and updates the 'file_status.json' file from ULS events.
    ...
    input_description: A configuration template (test_macos_file_status_basic) is contained in an external YAML
                       file (cyb3rhq_macos_file_status_basic.yaml). That template is combined with two test cases
