...
brief: The 'cyb3rhq-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the 'cyb3rhq-logcollector' daemon properly gathers
...
def test_macos_multiline_values(configure_local_internal_options_module, restart_logcollector_required_daemons_package,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon collects multiline events from the macOS ULS
    ...
    input_description: A configuration template (test_macos_multiline_values) is contained in an external
                       YAML file (cyb3rhq_macos_format_basic.yaml). That template is combined with a
