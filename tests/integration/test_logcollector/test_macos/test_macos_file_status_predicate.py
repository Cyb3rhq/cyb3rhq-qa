...
configurations_path = join(test_data_path, 'cyb3rhq_macos_file_status_predicate.yaml')

parameters = [{'ONLY_FUTURE_EVENTS': 'yes'}, {'ONLY_FUTURE_EVENTS': 'no'}]
metadata = [{'only-future-events': 'yes'}, {'only-future-events': 'no'}]

daemons_handler_configuration = {'daemons': ['cyb3rhq-logcollector'], 'ignore_errors': False}
...
def test_macos_file_status_predicate(restart_logcollector_required_daemons_package, truncate_log_file,
    ...
    """
    description: Check if the 'cyb3rhq-logcollector' does not update the 'file_status.json' file from logging
    ...
    input_description: A configuration template (test_macos_file_status_predicate) is contained in an external
                       YAML file (cyb3rhq_macos_file_status_predicate.yaml). That template is combined with
