...
from cyb3rhq_testing.logcollector import LOG_COLLECTOR_GLOBAL_TIMEOUT, callback_logcollector_started
from cyb3rhq_testing.tools.configuration import load_cyb3rhq_configurations
from cyb3rhq_testing.tools.file import read_json, write_json_file
from cyb3rhq_testing.tools.monitoring import wait_file
...
configurations_path = join(test_data_path, 'cyb3rhq_macos_file_status_when_no_macos.yaml')

dummy_file = join(gettempdir(), 'dummy_file.log')
parameters = [{'FILE_TO_MONITOR': dummy_file}]

# Configuration data
configurations = load_cyb3rhq_configurations(configurations_path, __name__, params=parameters)

daemons_handler_configuration = {'daemons': ['cyb3rhq-logcollector'],
                                 'ignore_errors': False}
...
# Time to wait for file_status.json to be updated (the +8 is due to a delay added by the cyb3rhq-agentd daemmon)
...
def test_macos_file_status_when_no_macos(restart_logcollector_required_daemons_package, truncate_log_file, handle_files,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' does not store and removes if exists, previous
    ...
    input_description: A configuration template (test_macos_file_status_when_no_macos) is contained in an external
                       YAML file (cyb3rhq_macos_file_status_when_no_macos.yaml). That template is combined with
