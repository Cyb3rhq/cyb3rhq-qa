...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/reference/ossec-conf/localfile.html#reconnect-time
...
configurations_path = os.path.join(test_data_path, 'cyb3rhq_reconnect_time.yaml')
...
def test_reconnect_time(start_eventlog_process, get_local_internal_options, configure_local_internal_options, get_configuration,
    ...
    '''
    description: Check if the 'cyb3rhq-logcollector' daemon uses the interval of reconnection attempts when
    ...
    input_description: A configuration template (test_reconnect_time) is contained in an external YAML file
                       (cyb3rhq_reconnect_time.yaml). That template is combined with different test cases
    ...
    if time_to_seconds(config['reconnect_time']) >= timeout_callback_reconnect_time:
        pytest.xfail("Expected fail: https://github.com/cyb3rhq/cyb3rhq/issues/8580")

    log_callback = logcollector.callback_eventchannel_analyzing(config['location'])
    cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout, callback=log_callback,
