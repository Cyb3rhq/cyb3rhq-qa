...
brief: These tests will check if the 'SSL' (Secure Socket Layer) protocol-related settings of
       the 'cyb3rhq-authd' daemon are working correctly. The 'cyb3rhq-authd' daemon can
       automatically add a Cyb3rhq agent to a Cyb3rhq manager and provide the key
...
from cyb3rhq_testing.tools.file import truncate_file, read_yaml
...
def override_cyb3rhq_conf(configuration):
    """
    Write a particular Cyb3rhq configuration for the test case.
    """
    # Stop Cyb3rhq
...
def test_ossec_auth_configurations(get_configuration, configure_environment, configure_sockets_environment):
    '''
    description:
        Checks if the 'SSL' settings of the 'cyb3rhq-authd' daemon work correctly by enrolling agents
