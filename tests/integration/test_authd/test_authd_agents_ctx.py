...
brief: These tests will check if the 'cyb3rhq-authd' daemon correctly handles the enrollment requests
       from agents with pre-existing IP addresses or names. The 'cyb3rhq-authd' daemon can automatically
       add a Cyb3rhq agent to a Cyb3rhq manager and provide the key to the agent. It is used along with
...
from cyb3rhq_testing.tools.file import truncate_file, remove_file, recursive_directory_creation
from cyb3rhq_testing.tools.monitoring import FileMonitor
from cyb3rhq_testing.tools.services import control_service, check_daemon_status
from cyb3rhq_testing.api import remove_groups, set_up_groups
...
ls_sock_path = os.path.join(os.path.join(CYB3RHQ_PATH, 'queue', 'sockets', 'auth'))
...
def clean_diff():
    diff_folder = os.path.join(CYB3RHQ_PATH, 'queue', 'diff')
    try:
...
def clean_rids():
    rids_folder = os.path.join(CYB3RHQ_PATH, 'queue', 'rids')
...
def clean_agents_timestamp():
    timestamp_path = os.path.join(CYB3RHQ_PATH, 'queue', 'agents-timestamp')
...
def check_diff(name, expected, timeout=30):
    diff_path = os.path.join(CYB3RHQ_PATH, 'queue', 'diff', name)
    wait = time.time() + timeout
    while time.time() < wait:
...
def check_rids(id, expected):
    agent_info_path = os.path.join(CYB3RHQ_PATH, 'queue', 'rids', id)
...
def create_rids(id):
    rids_path = os.path.join(CYB3RHQ_PATH, 'queue', 'rids', id)
    try:
...
def create_diff(name):
    SIGID = '533'
    diff_folder = os.path.join(CYB3RHQ_PATH, 'queue', 'diff', name)
    try:
...
def test_ossec_authd_agents_ctx(get_configuration, configure_environment, configure_sockets_environment,
                                     connect_to_sockets_module, restart_cyb3rhq, server_type):
    '''
    description:
        Check if when the 'cyb3rhq-authd' daemon receives an enrollment request from an agent
    ...
    input_description:
        Different test cases are contained in an external YAML file (cyb3rhq_conf.yaml)
        which includes configuration settings for the 'cyb3rhq-authd' daemon.

    expected_output:
        - r'Accepting connections on port 1515' (When the 'cyb3rhq-authd' daemon is ready to accept enrollments)
    ...
    check_daemon_status(running_condition=False, target_daemon='cyb3rhq-authd')
    time.sleep(1)
    clean_logs()
    clean_agents_ctx()
    control_service('start')
    check_daemon_status(running_condition=True, target_daemon='cyb3rhq-authd')
