...
authd_default_password_path = os.path.join(CYB3RHQ_PATH, 'etc', 'authd.pass')
...
def read_random_pass():
    """
    Search for the random password creation in Cyb3rhq logs
    """
    osseclog_path = os.path.join(CYB3RHQ_PATH, 'logs', 'ossec.log')
    passw = None
    try:
...
def test_authd_force_options(get_configuration, configure_environment, configure_sockets_environment,
                             clean_client_keys_file_function, reset_password, restart_cyb3rhq_daemon_function,
