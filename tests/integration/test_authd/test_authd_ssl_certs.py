...
brief: These tests will check if the 'cyb3rhq-authd' daemon is able to handle secure connections using
       the 'SSL' (Secure Socket Layer) protocol. The 'cyb3rhq-authd' daemon can automatically add
       a Cyb3rhq agent to a Cyb3rhq manager and provide the key to the agent.
...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/registering/host-verification-registration.html
...
from cyb3rhq_testing.tools.configuration import set_section_cyb3rhq_conf, write_cyb3rhq_conf, \
    load_cyb3rhq_configurations
from cyb3rhq_testing.tools.file import truncate_file
from cyb3rhq_testing.tools.monitoring import SocketController, FileMonitor
from cyb3rhq_testing.tools.security import CertificateController
...
def override_cyb3rhq_conf(configuration):
    # Stop Cyb3rhq
    control_service('stop', daemon='cyb3rhq-authd')
    time.sleep(1)
    check_daemon_status(running_condition=False, target_daemon='cyb3rhq-authd')
    truncate_file(LOG_FILE_PATH)

    # Configuration for testing
    test_config = set_section_cyb3rhq_conf(configuration.get('sections'))
    # Set new configuration
    write_cyb3rhq_conf(test_config)
    # Start Cyb3rhq daemons
    time.sleep(1)
    control_service('start', daemon='cyb3rhq-authd')
...
def test_authd_ssl_certs(get_configuration, generate_ca_certificate, tear_down):
    '''
    description:
        Checks if the 'cyb3rhq-authd' daemon can manage 'SSL' connections with agents
    ...
    assertions:
        - Verify that the agent can only connect to the 'cyb3rhq-authd' daemon socket using a valid certificate.
    ...
    option = get_configuration['metadata']['sim_option']
    override_cyb3rhq_conf(get_configuration)
