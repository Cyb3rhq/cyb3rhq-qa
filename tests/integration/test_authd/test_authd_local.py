...
brief: This module verifies the correct behavior of 'cyb3rhq-authd' under different messages
...
from cyb3rhq_testing.tools import CYB3RHQ_PATH, CYB3RHQ_DB_SOCKET_PATH
...
receiver_sockets_params = [(ls_sock_path, 'AF_UNIX', 'TCP'), (CYB3RHQ_DB_SOCKET_PATH, 'AF_UNIX', 'TCP')]
test_case_ids = [f"{test_case['name'].lower().replace(' ', '-')}" for test_case in message_tests]

# TODO Replace or delete
monitored_sockets_params = [('cyb3rhq-db', None, True), ('cyb3rhq-authd', None, True)]
...
def test_authd_local_messages(configure_environment, configure_sockets_environment, connect_to_sockets_function,
                              set_up_groups, insert_pre_existent_agents, restart_cyb3rhq_daemon_function,
