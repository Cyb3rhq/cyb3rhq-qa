...
brief: Active responses perform various countermeasures to address active threats, such as blocking access
       ...
       the 'cyb3rhq-analysisd' daemon processes 'active response' messages correctly.
...
ANALYSISD_SOCKET = os.path.join(CYB3RHQ_PATH, 'queue', 'sockets', 'queue')
...
def restart_service():
    """Restart the Cyb3rhq manager and clean the ossec.log file."""
...
def validate_new_ar_message(agent_id, message, extra_args, timeout, all_agents):
    ...
    assert json_alert['origin']['module'] == 'cyb3rhq-analysisd', 'Invalid module in JSON message'
...
def test_os_exec(set_debug_mode, get_configuration, configure_environment, restart_service, configure_agents):
    ...
    description: Check if 'active response' messages are sent in the correct format
                 ...
                 (string and JSON) are sent to the socket of the 'cyb3rhq-analisysd' daemon,
    ...
    parameters:
        ...
        - restart_service:
            type: fixture
            brief: Restart the Cyb3rhq manager and clean the 'ossec.log' file.
        - configure_agents:
            type: fixture
    ...
    expected_output:
        ...
        - Active response message with the structure defined in the
          validate_new_ar_message function (for Cyb3rhq versions >= 4.2).
        - Active response message with the structure defined in the
          validate_old_ar_message function (for Cyb3rhq versions < 4.2).
