...
def start_agent(request, get_configuration):
    ...
    try:
        ...
        call([f'{CYB3RHQ_PATH}/{agent_auth_pat}/agent-auth', '-m', SERVER_ADDRESS])
...
def remove_ip_from_iptables(request, get_configuration):
    ...
    """
    metadata = get_configuration['metadata']
    param = '{"version":1,"origin":{"name":"","module":"cyb3rhq-execd"},"command":"delete",' \
            '"parameters":{"extra_args":[],"alert":{"data":{"srcip":"' + metadata['ip'] + \
            '","dstuser":"Test"}},"program":"/var/ossec/active-response/bin/firewall-drop"}}'
    firewall_drop_script_path = os.path.join(CYB3RHQ_PATH, 'active-response/bin', 'firewall-drop')
...
def test_execd_firewall_drop(set_debug_mode, get_configuration, test_version, configure_environment,
    ...
    parameters:
        - set_debug_mode:
            type: fixture
            brief: Set the 'cyb3rhq-execd' daemon in debug mode.
        - get_configuration:
            type: fixture
            brief: Get configurations from the module.
        - test_version:
            type: fixture
            brief: Validate the Cyb3rhq version.
        ...
        - start_agent:
            type: fixture
            brief: Create 'cyb3rhq-remoted' and 'cyb3rhq-authd' simulators, register agent and start it.
        - set_ar_conf_mode:
            type: fixture
