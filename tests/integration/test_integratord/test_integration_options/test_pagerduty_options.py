...
brief: Integratord manages cyb3rhq integrations with other applications such as Slack, Pagerduty, Shuffle, Yara or
...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/capabilities/virustotal-scan/integration.html
    - https://documentation.cyb3rhq.com/current/user-manual/reference/daemons/cyb3rhq-integratord.htm
...
def test_pagerduty_no_option_tag(configuration, metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                                 configure_local_internal_options_module, restart_cyb3rhq_daemon_function,
    ...
    parameters:
        ...
        - restart_cyb3rhq_daemon_function:
            type: fixture
            brief: Restart cyb3rhq daemon before starting a test.
        - wait_for_start_module:
            type: fixture
    ...
    # Check integration is enabled
    evm.detect_integration_enabled(integration=metadata['integration'], file_monitor=cyb3rhq_monitor)

    # Check no options JSON file is detected
    evm.detect_options_json_file_does_not_exist(file_monitor=cyb3rhq_monitor)

    # Check the message is sent to the integration's server
    evm.get_message_sent(integration='PagerDuty', file_monitor=cyb3rhq_monitor)
...
def test_pagerduty_options(configuration, metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                           configure_local_internal_options_module, restart_cyb3rhq_daemon_function,
                           wait_for_start_module):
    '''
    ...
    if not metadata['sends_message']:
        ...
        evm.detect_unable_to_run_integration(integration=metadata['integration'], file_monitor=cyb3rhq_monitor)
    else:
        # Verify that the message is sent
        message = evm.get_message_sent(integration='PagerDuty', file_monitor=cyb3rhq_monitor)
