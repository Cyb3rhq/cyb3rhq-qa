...
brief: The cyb3rhq-analysisd daemon uses a series of decoders and rules to analyze and interpret logs and events and
...
references:
    - https://documentation.cyb3rhq.com/current/user-manual/ruleset/ruleset-xml-syntax/rules.html#if-sid
'''
import os
import pytest

from cyb3rhq_testing import LOG_FILE_PATH, T_5, T_10
...
from cyb3rhq_testing.modules.analysisd import event_monitor as ev
...
def test_valid_signature_id(configuration, metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                            prepare_custom_rules_file, restart_cyb3rhq_function):
    '''
    ...
    parameters:
        ...
        - restart_cyb3rhq_function:
            type: fixture
            brief: Restart cyb3rhq at the start of the module to apply configuration.

    assertions:
        - Check that cyb3rhq starts
        - Check ".*Signature ID '(\\d*)' was not found and will be ignored in the 'if_sid'.* of rule '(\\d*)'" event
        - Check ".*cyb3rhq-testrule.*Empty 'if_sid' value. Rule '(\\d*)' will be ignored.*"
    ...
    with pytest.raises(TimeoutError):
        ev.check_if_sid_not_found(cyb3rhq_log_monitor)
    # Check that no log appears for rules if_sid field being empty string
    with pytest.raises(TimeoutError):
        ev.check_empty_if_sid(cyb3rhq_log_monitor)
    # Check that no log appears for rules if_sid field being invalid
    with pytest.raises(TimeoutError):
        ev.check_invalid_if_sid(cyb3rhq_log_monitor, is_empty=False)


@pytest.mark.tier(level=1)
@pytest.mark.parametrize('configuration, metadata', zip(t2_configurations, t2_configuration_metadata), ids=t2_case_ids)
def test_invalid_signature_id(configuration, metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                              prepare_custom_rules_file, restart_cyb3rhq_function):
    '''
    ...
    ev.check_invalid_if_sid(cyb3rhq_log_monitor, metadata['is_empty'])


# Tests
@pytest.mark.tier(level=1)
@pytest.mark.parametrize('configuration, metadata', zip(t3_configurations, t3_configuration_metadata), ids=t3_case_ids)
def test_null_signature_id(configuration, metadata, set_cyb3rhq_configuration, truncate_monitored_files,
                           prepare_custom_rules_file, restart_cyb3rhq_function):
    '''
