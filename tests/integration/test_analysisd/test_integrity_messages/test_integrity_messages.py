...
from cyb3rhq_testing.analysis import callback_analysisd_message, validate_analysis_integrity_state, \
    callback_cyb3rhq_db_integrity
...
def test_integrity_messages(configure_sockets_environment, connect_to_sockets_module, wait_for_analysisd_startup,
    ...
    description: Check if when the 'cyb3rhq-analysisd' daemon socket receives a message with
                 ...
                 sends to the 'cyb3rhq-db' daemon socket.
    ...
    for stage in test_case:
        ...
        response = monitored_sockets[0].start(timeout=3 * global_parameters.default_timeout,
                                              callback=callback_cyb3rhq_db_integrity).result()
