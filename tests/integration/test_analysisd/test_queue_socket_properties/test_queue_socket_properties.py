...
brief: The 'cyb3rhq-analysisd' daemon refreshes the queue socket file every time the configuration test is executed
       Specifically, this test will check if after running the configuration test of 'cyb3rhq-analysisd' the properties
...
from cyb3rhq_testing.tools import ANALYSISD_BINARY_PATH, ANALYSISD_QUEUE_SOCKET_PATH
...
def socket_file_properties():
    """Get the inode and modification time values of the 'queue' socket of 'cyb3rhq-analysisd'"""
...
def run_analysisd_test_config():
    """Run the daemon configuration test mode of 'cyb3rhq-analysisd'"""
...
def test_queue_socket_properties(restart_analysisd, before_socket_properties, run_analysisd_test_config,
    ...
    '''
    description: Check if after running the configuration test of 'cyb3rhq-analysisd' the properties
                 of the queue socket file are changed.
    ...
    parameters:
        ...
        - run_analysisd_test_config:
            type: fixture
            brief: Run the daemon configuration test mode of 'cyb3rhq-analysisd'
        - after_socket_properties:
            type: fixture
    ...
    assertions:
        - Verify that the Inode value of the socket file does not change its value after running the
          configuration test of 'cyb3rhq-analysisd'
    ...
    input_description: The test gets the current properties of the socket file and some parameters
                       to run the daemon configuration test of 'cyb3rhq-analysisd'.
