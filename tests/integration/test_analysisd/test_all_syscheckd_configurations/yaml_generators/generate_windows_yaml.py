...
from cyb3rhq_testing.tools import CYB3RHQ_LOGS_PATH, LOG_FILE_PATH, CYB3RHQ_PATH
from cyb3rhq_testing.tools.file import truncate_file
from cyb3rhq_testing.tools.monitoring import ManInTheMiddle, QueueMonitor
...
def generate_analysisd_yaml(n_events, modify_events):
    ...
    def remove_logs():
        for root, _, files in os.walk(CYB3RHQ_LOGS_PATH):
            for file in files:
    ...
    control_service('start', daemon='cyb3rhq-remoted', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyb3rhq-remoted')
...
def kill_daemons():
    for daemon in ['cyb3rhq-remoted', 'cyb3rhq-analysisd', 'cyb3rhq-db']:
