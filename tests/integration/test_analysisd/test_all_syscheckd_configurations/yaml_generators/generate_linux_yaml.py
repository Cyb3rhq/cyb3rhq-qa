...
from cyb3rhq_testing.analysis import callback_analysisd_agent_id, callback_analysisd_event
from cyb3rhq_testing.fim import REGULAR, create_file, modify_file, delete_file, detect_initial_scan
from cyb3rhq_testing.tools import CYB3RHQ_LOGS_PATH, LOG_FILE_PATH, CYB3RHQ_PATH, CYB3RHQ_CONF, PREFIX
...
from cyb3rhq_testing.tools.monitoring import ManInTheMiddle, QueueMonitor, FileMonitor
from cyb3rhq_testing.tools.services import control_service, check_daemon_status, delete_sockets

alerts_json = os.path.join(CYB3RHQ_LOGS_PATH, 'alerts', 'alerts.json')
...
def generate_analysisd_yaml(n_events, modify_events):
    ...
    def remove_logs():
        for root, dirs, files in os.walk(CYB3RHQ_LOGS_PATH):
            for file in files:
    ...
    control_service('start', daemon='cyb3rhq-db', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyb3rhq-db')

    control_service('start', daemon='cyb3rhq-analysisd', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyb3rhq-analysisd')
    ...
    control_service('start', daemon='cyb3rhq-syscheckd', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyb3rhq-syscheckd')
...
def kill_daemons():
    for daemon in ['cyb3rhq-analysisd', 'cyb3rhq-db', 'cyb3rhq-syscheckd']:
