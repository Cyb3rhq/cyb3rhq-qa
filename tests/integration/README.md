...
Cyb3rhq - Quality assurance automation templates
...
You can run these tests on a manager or an agent. In case you are using an agent, please remember to register it and use the correct version (Cyb3rhq branch).
...
- Install cyb3rhq-testing framework

```shell script
cd deps/cyb3rhq_testing/
...
Our newest integration tests are located in `cyb3rhq-qa/tests/integration/`. They are organized by capabilities:
...
It is necessary to install the cyb3rhq testing framework dependencies:
```shell script
python -m pip install -r requirements.txt
```
...
- **Services**: from controlling Cyb3rhq services, daemons and socket to common processes.
...
- `--wpk_version`: Specify the WPK package version used to upgrade on WPK tests. (ex. --wpk_version=v4.2.0). Note: This
  ...
  the [repository](packages-dev.cyb3rhq.com/trash/wpk/).
- `--wpk_package_path`: Specify the WPK package path used to upgrade on WPK tests. (ex. --wpk_package_path='packages-dev.cyb3rhq.com/trash/wpk/').
...
rootdir: /vagrant/cyb3rhq-qa/test_cyb3rhq, inifile: pytest.ini
collected 12 items
