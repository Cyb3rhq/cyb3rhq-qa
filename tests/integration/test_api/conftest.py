...
from cyb3rhq_testing.tools import configuration as conf
from cyb3rhq_testing.tools import services as svcs
...
def configure_api_environment(get_configuration, request):
    ...
    backup_config = conf.get_api_conf(fw.CYB3RHQ_API_CONF)

    # Save current security config
    backup_security_config = conf.get_api_conf(fw.CYB3RHQ_SECURITY_CONF) if \
        os.path.exists(fw.CYB3RHQ_SECURITY_CONF) else None

    # Set new configuration
    api_config = get_configuration.get('configuration', None)
    if api_config:
        conf.write_api_conf(fw.CYB3RHQ_API_CONF, api_config)

    # Set security configuration
    security_config = get_configuration.get('security_config', None)
    if security_config:
        conf.write_security_conf(fw.CYB3RHQ_SECURITY_CONF, security_config)
    ...
    conf.write_api_conf(fw.CYB3RHQ_API_CONF, backup_config if backup_config else {})

    # Restore previous RBAC configuration
    if backup_security_config:
        conf.write_security_conf(fw.CYB3RHQ_SECURITY_CONF, backup_security_config)
    elif security_config and not backup_security_config:
        os.remove(fw.CYB3RHQ_SECURITY_CONF)
...
def set_api_configuration(configuration):
    ...
    conf.write_api_conf(fw.CYB3RHQ_API_CONF, test_config)
...
def restart_api_function():
    ...
    # Stop daemons in reverse to simulate Cyb3rhq's behavior
