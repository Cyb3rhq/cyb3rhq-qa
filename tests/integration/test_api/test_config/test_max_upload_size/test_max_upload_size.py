...
references:
    ...
    - https://documentation.cyb3rhq.com/current/user-manual/api/configuration.html
...
from cyb3rhq_testing.tools.configuration import check_apply_test, load_cyb3rhq_configurations
...
daemons_handler_configuration = {'daemons': ['cyb3rhq-apid']}
file_to_monitor = API_LOG_FILE_PATH
test_data_path = join(dirname(realpath(__file__)), 'data')
configurations_path = join(test_data_path, 'cyb3rhq_max_upload_size.yaml')
...
def restart_required_api_cyb3rhq():
    """Restart API-required services and stop them all at the end of the test."""
    required_api_daemons = ['cyb3rhq-modulesd', 'cyb3rhq-analysisd', 'cyb3rhq-execd', 'cyb3rhq-db', 'cyb3rhq-remoted']
...
def test_max_upload_size(tags_to_apply, get_configuration, configure_api_environment, restart_required_api_cyb3rhq,
    ...
    input_description: A test case is (test_upload_size) contained in an external YAML file
                       (cyb3rhq_max_upload_size.yaml) which includes API configuration
