...
from cyb3rhq_testing import global_parameters, logger, REGULAR, LOG_FILE_PATH, CYB3RHQ_PATH
from cyb3rhq_testing.tools.file import create_file, modify_file_content, delete_file, generate_string
...
from cyb3rhq_testing.modules.fim import event_monitor as ev
from cyb3rhq_testing.modules.fim.classes import CustomValidator, EventChecker, RegistryEventChecker
...
def get_sync_msgs(timeout, new_data=True):
    ...
    events = []
    if new_data:
        cyb3rhq_log_monitor.start(timeout=timeout,
    ...
    for _ in range(0, fim.MAX_EVENTS_VALUE):
        try:
            sync_event = cyb3rhq_log_monitor.start(timeout=global_parameters.default_timeout,
