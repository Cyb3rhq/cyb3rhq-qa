...
from cyb3rhq_testing import LOG_FILE_PATH, logger, T_60, T_30
...
def check_fim_event(file_monitor=None, callback='', error_message=None, update_position=True,
    ...
    Args:
        ...
        callback (str): log regex to check in Cyb3rhq log
        error_message (str): error message to show in case of expected event does not occur
        update_position (boolean): filter configuration parameter to search in Cyb3rhq log
        timeout (str): timeout to check the event in Cyb3rhq log
...
def get_messages(callback, timeout=T_30):
    ...
    for _ in range(0, MAX_EVENTS_VALUE):
        event = None
        try:
            event = cyb3rhq_log_monitor.start(timeout=timeout, accum_results=1,
                                            callback=callback,
