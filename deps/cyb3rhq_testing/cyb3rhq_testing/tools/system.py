...
from cyb3rhq_testing.tools import (API_LOG_FILE_PATH, CYB3RHQ_API_CONF, CYB3RHQ_CONF,
                                 CYB3RHQ_LOCAL_INTERNAL_OPTIONS)
from cyb3rhq_testing.tools.configuration import set_section_cyb3rhq_conf
...
class HostManager:
    ...
    def move_file(self, host: str, src_path: str, dest_path: str = '/var/ossec/etc/ossec.conf', check: bool = False):
        ...
        if 'os_name' in host_variables and host_variables['os_name'] == 'windows':
            ...
            result = self.get_host(host).ansible('copy', f'src={src_path} dest={dest_path} '
                                                 'owner=cyb3rhq group=cyb3rhq mode=preserve',
                                                 check=check)
    ...
    def control_service(self, host: str, service: str = 'cyb3rhq', state: str = "started", check: bool = False):
        ...
        """
        if service == 'cyb3rhq':
            service = 'cyb3rhq-agent' if 'agent' in host else 'cyb3rhq-manager'
    ...
    def apply_config(self, config_yml_path: str, dest_path: str = CYB3RHQ_CONF, clear_files: list = None,
        ...
        for host, payload in config.items():
            ...
            parse_configurations[host] = set_section_cyb3rhq_conf(sections=payload['sections'],
    ...
    def apply_api_config(self, api_config: str or dict = None, host_list: list = None, dest_path: str = CYB3RHQ_API_CONF,
        ...
        for host in host_list:
            self.control_service(host=host, service='cyb3rhq-manager', state='restarted')
            if clear_log:
                self.clear_file(host=host, file_path=API_LOG_FILE_PATH)

    def get_api_token(self, host, user='cyb3rhq', password='cyb3rhq', auth_context=None, port=55000, check=False):
        """Return an API token for the specified user.

        Args:
            host (str): HostName in inventory.
            user (str, optional): API username. Default `cyb3rhq`
            password (str, optional): API password. Default `cyb3rhq`
    ...
    def configure_local_internal_options(self, local_internal_options: dict):
        ...
        for target_host in local_internal_options:
            ...
            backup_local_internal_options = self.get_file_content(target_host, CYB3RHQ_LOCAL_INTERNAL_OPTIONS)
            ...
            self.modify_file_content(target_host, CYB3RHQ_LOCAL_INTERNAL_OPTIONS, replace)
    ...
    def handle_cyb3rhq_services(self, host, operation):
        """
        Handles Cyb3rhq services on the specified host.

        Args:
            host (str): The target host on which to handle Cyb3rhq services.
            operation (str): The operation to perform ('start', 'stop', 'restart').

        Example:
            host_manager.handle_cyb3rhq_services('my_host', 'restart')
        """
        ...
        if os == 'windows':
            if operation == 'restart':
                self.get_host(host).ansible('ansible.windows.win_shell', f'NET stop Cyb3rhq', check=False)
                self.get_host(host).ansible('ansible.windows.win_shell', f'NET start Cyb3rhq', check=False)
            else:
                result = self.get_host(host).ansible('ansible.windows.win_shell', f'NET {operation} Cyb3rhq', check=False)
        else:
            if os == 'linux':
                result = binary_path = f"/var/ossec/bin/cyb3rhq-control"
            elif os == 'macos':
                result = binary_path = f"/Library/Ossec/bin/cyb3rhq-control"
    ...
    def control_environment(self, operation, group_list, parallel=False):
        """
        Controls the Cyb3rhq services on hosts in the specified groups.

        Args:
            operation (str): The operation to perform on Cyb3rhq services ('start', 'stop', 'restart').
            group_list (list): A list of group names whose hosts' Cyb3rhq services should be controlled.
        ...
        if parallel:
            ...
            for group in group_list:
                for host in self.get_group_hosts(group):
                    thread = Thread(target=self.handle_cyb3rhq_services, args=(host, operation))
            ...
            for group in group_list:
                for host in self.get_group_hosts(group):
                    self.handle_cyb3rhq_services(host, operation)
    ...
    def get_cluster_name(self):
        ...
        cluster_name = first_manager_vars.vars.get('cluster_name', 'cyb3rhq')
