...
REMOTED_DETECTOR_PREFIX = r'.*cyb3rhq-remoted.*'
LOG_COLLECTOR_DETECTOR_PREFIX = r'.*cyb3rhq-logcollector.*'
AGENT_DETECTOR_PREFIX = r'.*cyb3rhq-agent.*' if sys.platform == 'win32' else r'.*cyb3rhq-agentd.*'
WINDOWS_AGENT_DETECTOR_PREFIX = r'.*cyb3rhq-agent.*'
AUTHD_DETECTOR_PREFIX = r'.*cyb3rhq-authd.*'
MODULESD_DETECTOR_PREFIX = r'.*cyb3rhq-modulesd.*'
CYB3RHQ_DB_PREFIX = r'.*cyb3rhq-db.*'

DEFAULT_POLL_FILE_TIME = 1
DEFAULT_WAIT_FILE_TIMEOUT = 30


def cyb3rhq_unpack(data, format_: str = "<I"):
...
def cyb3rhq_pack(data, format_: str = "<I"):
...
class SocketController:
    ...
    def send(self, message, size=False):
        ...
        try:
            msg_bytes = cyb3rhq_pack(len(msg_bytes)) + msg_bytes if size else msg_bytes
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def unix_forward(self, data):
        ...
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as forwarded_sock:
            ...
            forwarded_sock.sendall(cyb3rhq_pack(len(data)) + data)

            # Receive data from the server and shut down
            size = cyb3rhq_unpack(self.recvall_size(forwarded_sock, 4, socket.MSG_WAITALL))
    ...
    def default_cyb3rhq_handler(self):
        ...
        while not self.server.mitm.event.is_set():
            ...
            self.request.sendall(cyb3rhq_pack(len(response)) + response)
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam('cyb3rhq').pw_uid
