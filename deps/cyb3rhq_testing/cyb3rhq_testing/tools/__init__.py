# Copyright (C) 2015-2022, Cyb3rhq Inc.
# Created by Cyb3rhq, Inc. <info@cyb3rhq.com>.
# This program is free software; you can redistribute it and/or modify it under the terms of GPLv2
import os
import platform
import subprocess
import sys


if sys.platform == 'win32':
    CYB3RHQ_PATH = os.path.join("C:", os.sep, "Program Files (x86)", "ossec-agent")
    CYB3RHQ_CONF = os.path.join(CYB3RHQ_PATH, 'ossec.conf')
    CYB3RHQ_LOCAL_INTERNAL_OPTIONS = os.path.join(CYB3RHQ_PATH, 'local_internal_options.conf')
    CYB3RHQ_SOURCES = os.path.join('/', 'cyb3rhq')
    AGENT_CONF = os.path.join(CYB3RHQ_PATH, 'shared', 'agent.conf')
    LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'ossec.log')
    CLIENT_KEYS_PATH = os.path.join(CYB3RHQ_PATH, 'client.keys')
    PREFIX = os.path.join('c:', os.sep)
    GEN_OSSEC = None
    CYB3RHQ_API_CONF = None
    CYB3RHQ_SECURITY_CONF = None
    API_LOG_FILE_PATH = None
    API_JSON_LOG_FILE_PATH = None
    API_LOG_FOLDER = None
    AGENT_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'cyb3rhq-agent.state')
    LOGCOLLECTOR_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'cyb3rhq-logcollector.state')
    REMOTE_STATISTICS_FILE = None
    ANALYSIS_STATISTICS_FILE = None
    UPGRADE_PATH = os.path.join(CYB3RHQ_PATH, 'upgrade')
    AGENT_AUTH_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'agent-auth.exe')
    ANALYSISD_BINARY_PATH = None
    HOSTS_FILE_PATH = os.path.join("C:", os.sep, "Windows", "System32", "drivers", "etc", "hosts")
    GLOBAL_DB_PATH = None
    CYB3RHQ_UNIX_USER = 'cyb3rhq'
    CYB3RHQ_UNIX_GROUP = 'cyb3rhq'
    GLOBAL_DB_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'db', 'global.db')
    ARCHIVES_LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'logs', 'archives', 'archives.log')
    ACTIVE_RESPONSE_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'active-response', 'bin')
else:
    CYB3RHQ_SOURCES = os.path.join('/', 'cyb3rhq')

    CYB3RHQ_UNIX_USER = 'cyb3rhq'
    CYB3RHQ_UNIX_GROUP = 'cyb3rhq'

    if sys.platform == 'darwin':
        CYB3RHQ_PATH = os.path.join("/", "Library", "Ossec")
        PREFIX = os.path.join('/', 'private', 'var', 'root')
        GEN_OSSEC = None
    else:
        CYB3RHQ_PATH = os.path.join("/", "var", "ossec")
        GEN_OSSEC = os.path.join(CYB3RHQ_SOURCES, 'gen_ossec.sh')
        PREFIX = os.sep

    CYB3RHQ_CONF_RELATIVE = os.path.join('etc', 'ossec.conf')
    CYB3RHQ_LOCAL_INTERNAL_OPTIONS = os.path.join(CYB3RHQ_PATH, 'etc', 'local_internal_options.conf')
    CYB3RHQ_CONF = os.path.join(CYB3RHQ_PATH, CYB3RHQ_CONF_RELATIVE)
    AGENT_CONF = os.path.join(CYB3RHQ_PATH, 'etc', 'shared', 'agent.conf')
    CYB3RHQ_API_CONF = os.path.join(CYB3RHQ_PATH, 'api', 'configuration', 'api.yaml')
    CYB3RHQ_SECURITY_CONF = os.path.join(CYB3RHQ_PATH, 'api', 'configuration', 'security', 'security.yaml')
    LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'logs', 'ossec.log')
    CLIENT_KEYS_PATH = os.path.join(CYB3RHQ_PATH, 'etc', 'client.keys')
    API_LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'logs', 'api.log')
    API_JSON_LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'logs', 'api.json')
    API_LOG_FOLDER = os.path.join(CYB3RHQ_PATH, 'logs', 'api')
    ARCHIVES_LOG_FILE_PATH = os.path.join(CYB3RHQ_PATH, 'logs', 'archives', 'archives.log')
    AGENT_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'var', 'run', 'cyb3rhq-agentd.state')
    LOGCOLLECTOR_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'var', 'run', 'cyb3rhq-logcollector.state')
    LOGCOLLECTOR_FILE_STATUS_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'logcollector', 'file_status.json')
    REMOTE_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'var', 'run', 'cyb3rhq-remoted.state')
    ANALYSIS_STATISTICS_FILE = os.path.join(CYB3RHQ_PATH, 'var', 'run', 'cyb3rhq-analysisd.state')
    UPGRADE_PATH = os.path.join(CYB3RHQ_PATH, 'var', 'upgrade')
    PYTHON_PATH = os.path.join(CYB3RHQ_PATH, 'framework', 'python')
    AGENT_AUTH_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'bin', 'agent-auth')
    ANALYSISD_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'bin', 'cyb3rhq-analysisd')
    ACTIVE_RESPONSE_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'active-response', 'bin')
    AGENT_GROUPS_BINARY_PATH = os.path.join(CYB3RHQ_PATH, 'bin', 'agent_groups')

    if sys.platform == 'sunos5':
        HOSTS_FILE_PATH = os.path.join('/', 'etc', 'inet', 'hosts')
    else:
        HOSTS_FILE_PATH = os.path.join('/', 'etc', 'hosts')
    GLOBAL_DB_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'db', 'global.db')

    try:
        import grp
        import pwd

        CYB3RHQ_UID = pwd.getpwnam(CYB3RHQ_UNIX_USER).pw_uid
        CYB3RHQ_GID = grp.getgrnam(CYB3RHQ_UNIX_GROUP).gr_gid
    except (ImportError, KeyError, ModuleNotFoundError):
        pass


def get_version():

    if platform.system() in ['Windows', 'win32']:
        with open(os.path.join(CYB3RHQ_PATH, 'VERSION'), 'r') as f:
            version = f.read()
            return version[:version.rfind('\n')]

    else:  # Linux, sunos5, darwin, aix...
        return subprocess.check_output([
          f"{CYB3RHQ_PATH}/bin/cyb3rhq-control", "info", "-v"
        ], stderr=subprocess.PIPE).decode('utf-8').rstrip()


def get_service():
    if platform.system() in ['Windows', 'win32']:
        return 'cyb3rhq-agent'

    else:  # Linux, sunos5, darwin, aix...
        service = subprocess.check_output([
          f"{CYB3RHQ_PATH}/bin/cyb3rhq-control", "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return 'cyb3rhq-manager' if service == 'server' else 'cyb3rhq-agent'


_data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'data')

CUSTOM_RULES_PATH = os.path.join(CYB3RHQ_PATH, 'etc', 'rules')
LOCAL_RULES_PATH = os.path.join(CUSTOM_RULES_PATH, 'local_rules.xml')
LOCAL_DECODERS_PATH = os.path.join(CYB3RHQ_PATH, 'etc', 'decoders', 'local_decoder.xml')

SERVER_KEY_PATH = os.path.join(CYB3RHQ_PATH, 'etc', 'manager.key')
SERVER_CERT_PATH = os.path.join(CYB3RHQ_PATH, 'etc', 'manager.cert')

CLIENT_CUSTOM_KEYS_PATH = os.path.join(_data_path, 'sslmanager.key')
CLIENT_CUSTOM_CERT_PATH = os.path.join(_data_path, 'sslmanager.cert')

CYB3RHQ_LOGS_PATH = os.path.join(CYB3RHQ_PATH, 'logs')
ALERT_FILE_PATH = os.path.join(CYB3RHQ_LOGS_PATH, 'alerts', 'alerts.json')
ALERT_LOGS_PATH = os.path.join(CYB3RHQ_LOGS_PATH, 'alerts', 'alerts.log')
CLUSTER_LOGS_PATH = os.path.join(CYB3RHQ_LOGS_PATH, 'cluster.log')
QUEUE_SOCKETS_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'sockets')
QUEUE_ALERTS_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'alerts')
QUEUE_DB_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'db')
CLUSTER_SOCKET_PATH = os.path.join(CYB3RHQ_PATH, 'queue', 'cluster')


AGENT_INFO_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, '.agent_info')
ANALYSISD_ANALISIS_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'analysis')
ANALYSISD_QUEUE_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'queue')
AUTHD_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'auth')
EXECD_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'com')
LOGCOLLECTOR_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'logcollector')
LOGTEST_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'logtest')
MONITORD_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'monitor')
REMOTED_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'remote')
SYSCHECKD_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'syscheck')
CYB3RHQ_DB_SOCKET_PATH = os.path.join(QUEUE_DB_PATH, 'wdb')
MODULESD_WMODULES_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'wmodules')
MODULESD_DOWNLOAD_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'download')
MODULESD_CONTROL_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'control')
MODULESD_KREQUEST_SOCKET_PATH = os.path.join(QUEUE_SOCKETS_PATH, 'krequest')
MODULESD_C_INTERNAL_SOCKET_PATH = os.path.join(CLUSTER_SOCKET_PATH, 'c-internal.sock')
ACTIVE_RESPONSE_SOCKET_PATH = os.path.join(QUEUE_ALERTS_PATH, 'ar')

CYB3RHQ_SOCKETS = {
    'cyb3rhq-agentd': [],
    'cyb3rhq-apid': [],
    'cyb3rhq-agentlessd': [],
    'cyb3rhq-csyslogd': [],
    'cyb3rhq-analysisd': [
                        ANALYSISD_ANALISIS_SOCKET_PATH,
                        ANALYSISD_QUEUE_SOCKET_PATH
                       ],
    'cyb3rhq-authd': [AUTHD_SOCKET_PATH],
    'cyb3rhq-execd': [EXECD_SOCKET_PATH],
    'cyb3rhq-logcollector': [LOGCOLLECTOR_SOCKET_PATH],
    'cyb3rhq-monitord': [MONITORD_SOCKET_PATH],
    'cyb3rhq-remoted': [REMOTED_SOCKET_PATH],
    'cyb3rhq-maild': [],
    'cyb3rhq-syscheckd': [SYSCHECKD_SOCKET_PATH],
    'cyb3rhq-db': [CYB3RHQ_DB_SOCKET_PATH],
    'cyb3rhq-modulesd': [
                        MODULESD_WMODULES_SOCKET_PATH,
                        MODULESD_DOWNLOAD_SOCKET_PATH,
                        MODULESD_CONTROL_SOCKET_PATH,
                        MODULESD_KREQUEST_SOCKET_PATH
                      ],
    'cyb3rhq-clusterd': [MODULESD_C_INTERNAL_SOCKET_PATH],
    'cyb3rhq-integratord': []
}

# These sockets do not exist with default Cyb3rhq configuration
CYB3RHQ_OPTIONAL_SOCKETS = [
    MODULESD_KREQUEST_SOCKET_PATH,
    AUTHD_SOCKET_PATH
]

# Cyb3rhq daemons
LOGCOLLECTOR_DAEMON = 'cyb3rhq-logcollector'
AGENTLESS_DAEMON = 'cyb3rhq-agentlessd'
CSYSLOG_DAEMON = 'cyb3rhq-csyslogd'
REMOTE_DAEMON = 'cyb3rhq-remoted'
ANALYSISD_DAEMON = 'cyb3rhq-analysisd'
API_DAEMON = 'cyb3rhq-apid'
MAIL_DAEMON = 'cyb3rhq-maild'
SYSCHECK_DAEMON = 'cyb3rhq-syscheckd'
EXEC_DAEMON = 'cyb3rhq-execd'
MODULES_DAEMON = 'cyb3rhq-modulesd'
CLUSTER_DAEMON = 'cyb3rhq-clusterd'
INTEGRATOR_DAEMON = 'cyb3rhq-integratord'
MONITOR_DAEMON = 'cyb3rhq-monitord'
DB_DAEMON = 'cyb3rhq-db'
AGENT_DAEMON = 'cyb3rhq-agentd'


ALL_MANAGER_DAEMONS = [LOGCOLLECTOR_DAEMON, AGENTLESS_DAEMON, CSYSLOG_DAEMON, REMOTE_DAEMON, ANALYSISD_DAEMON,
                       API_DAEMON, MAIL_DAEMON, SYSCHECK_DAEMON, EXEC_DAEMON, MODULES_DAEMON, CLUSTER_DAEMON,
                       INTEGRATOR_DAEMON, MONITOR_DAEMON, DB_DAEMON]
ALL_AGENT_DAEMONS = [AGENT_DAEMON, EXEC_DAEMON, LOGCOLLECTOR_DAEMON, SYSCHECK_DAEMON, MODULES_DAEMON]
API_DAEMONS_REQUIREMENTS = [API_DAEMON, MODULES_DAEMON, ANALYSISD_DAEMON, EXEC_DAEMON, DB_DAEMON, REMOTE_DAEMON]


DISABLE_MONITORD_ROTATE_LOG_OPTION = {'monitord.rotate_log': '0'}
ANALYSISD_LOCAL_INTERNAL_OPTIONS = {'analysisd.debug': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
AGENTD_LOCAL_INTERNAL_OPTIONS = {'agent.debug': '2', 'execd': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
GCLOUD_LOCAL_INTERNAL_OPTIONS = {'analysisd.debug': '2',
                                 'cyb3rhq_modules.debug': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
LOGTEST_LOCAL_INTERNAL_OPTIONS = {'analysisd.debug': '2'}
REMOTED_LOCAL_INTERNAL_OPTIONS = {'remoted.debug': '2', 'cyb3rhq_database.interval': '2', 'cyb3rhq_db.commit_time': '2',
                                  'cyb3rhq_db.commit_time_max': '3'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
VD_LOCAL_INTERNAL_OPTIONS = {'cyb3rhq_modules.debug': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
WPK_LOCAL_INTERNAL_OPTIONS = {'cyb3rhq_modules.debug': '2'}
