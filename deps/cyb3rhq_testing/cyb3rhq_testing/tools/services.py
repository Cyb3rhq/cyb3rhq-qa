...
from cyb3rhq_testing.tools import CYB3RHQ_PATH, get_service, CYB3RHQ_SOCKETS, QUEUE_DB_PATH, CYB3RHQ_OPTIONAL_SOCKETS
from cyb3rhq_testing.tools.configuration import write_cyb3rhq_conf
from cyb3rhq_testing.modules import CYB3RHQ_SERVICES_START, CYB3RHQ_SERVICES_STOP


def restart_cyb3rhq_daemon(daemon):
    """Restarts a Cyb3rhq daemon.
    ...
    daemon_path = os.path.join(CYB3RHQ_PATH, 'bin')
    subprocess.check_call([f'{daemon_path}/{daemon}'])


def restart_cyb3rhq_with_new_conf(new_conf, daemon='cyb3rhq-syscheckd'):
    """Restart Cyb3rhq service applying a new ossec.conf.

    Args:
        new_conf ( ET.ElementTree) : New config file.
        daemon (str, optional): Daemon to restart when applying the configuration.
    """
    write_cyb3rhq_conf(new_conf)
...
def delete_sockets(path=None):
    ...
    try:
        if path is None:
            path = os.path.join(CYB3RHQ_PATH, 'queue', 'sockets')
            for file in os.listdir(path):
                os.remove(os.path.join(path, file))
            if os.path.exists(os.path.join(CYB3RHQ_PATH, 'queue', 'db', 'wdb')):
                os.remove(os.path.join(CYB3RHQ_PATH, 'queue', 'db', 'wdb'))
            if os.path.exists(os.path.join(CYB3RHQ_PATH, 'queue', 'cluster', 'c-internal.sock')):
                os.remove(os.path.join(CYB3RHQ_PATH, 'queue', 'cluster', 'c-internal.sock'))
        else:
            for item in path:
                os.remove(item)
    ...
    if sys.platform == 'win32':
        ...
        if daemon is None:
            if sys.platform == 'darwin' or sys.platform == 'sunos5':
                result = subprocess.run([f'{CYB3RHQ_PATH}/bin/cyb3rhq-control', action]).returncode
            else:
            ...
            if action == 'restart':
                ...
                for proc in psutil.process_iter():
                    try:
                        if daemon in ['cyb3rhq-clusterd', 'cyb3rhq-apid']:
                            for file in os.listdir(f'{CYB3RHQ_PATH}/var/run'):
                                if daemon in file:
                                    pid = file.split("-")
                                    pid = pid[2][0:-4]
                                    if pid == str(proc.pid):
                ...
                delete_sockets(CYB3RHQ_SOCKETS[daemon])
            else:
                daemon_path = os.path.join(CYB3RHQ_PATH, 'bin')
...
def restart_cyb3rhq_function():
    """Restarts Cyb3rhq."""
    control_service(CYB3RHQ_SERVICES_STOP)
    control_service(CYB3RHQ_SERVICES_START)
...
def check_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == 'win32':
            condition_met = check_if_process_is_running('cyb3rhq-agent.exe') == running_condition
        else:
            control_status_output = subprocess.run([f'{CYB3RHQ_PATH}/bin/cyb3rhq-control', 'status'],
