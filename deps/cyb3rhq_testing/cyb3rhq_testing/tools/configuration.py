...
from cyb3rhq_testing import global_parameters, logger, VALID_FIM_MODES, OS_EXCLUDED_FROM_RT_WD
from cyb3rhq_testing.tools import CYB3RHQ_PATH, GEN_OSSEC, CYB3RHQ_CONF, PREFIX, CYB3RHQ_LOCAL_INTERNAL_OPTIONS, AGENT_CONF, \
                                LOCAL_RULES_PATH
from cyb3rhq_testing import global_parameters, logger
...
def set_cyb3rhq_conf(cyb3rhq_conf: List[str]):
    """
    Set up Cyb3rhq configuration. Cyb3rhq will be restarted to apply it.

    Args:
        cyb3rhq_conf (ET.ElementTree): ElementTree with a custom Cyb3rhq configuration.
    """
    write_cyb3rhq_conf(cyb3rhq_conf)
    print("Restarting Cyb3rhq...")
    command = os.path.join(CYB3RHQ_PATH, 'bin/cyb3rhq-control')
    arguments = ['restart']
    check_call([command] + arguments, stdout=DEVNULL, stderr=DEVNULL)


def generate_cyb3rhq_conf(args: List = None) -> ET.ElementTree:
    """
    Generate a configuration file for Cyb3rhq.

    Args:
        args (list, optional): Arguments to generate ossec.conf (install_type, distribuition, version). Default `None`

    Returns:
        (ET.ElementTree): New Cyb3rhq configuration generated from 'gen_ossec.sh'.
    """
    gen_ossec_args = args if args else ['conf', 'manager', 'rhel', '7']
    cyb3rhq_config = check_output([GEN_OSSEC] + gen_ossec_args).decode(encoding='utf-8', errors='ignore')

    return ET.ElementTree(ET.fromstring(cyb3rhq_config))
...
def write_cyb3rhq_conf(cyb3rhq_conf: List[str]):
    """
    ...
    """
    with open(CYB3RHQ_CONF, 'w') as f:
        f.writelines(cyb3rhq_conf)
...
def write_security_conf(path: str, security_conf: dict):
    ...
    if not os.path.exists(path):
        from cyb3rhq_testing.tools import CYB3RHQ_UID, CYB3RHQ_GID

        open(path, mode='w').close()
        os.chown(uid=CYB3RHQ_UID, gid=CYB3RHQ_GID, path=path)
    write_api_conf(path, security_conf)


def set_section_cyb3rhq_conf(sections, template=None):
    """
    ...
    for section in sections:
        ...
        section_conf = find_module_config(cyb3rhq_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(cyb3rhq_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
...
def load_cyb3rhq_configurations(yaml_file_path: str, test_name: str, params: list = None, metadata: list = None) -> Any:
    r"""
...
def get_cyb3rhq_local_internal_options() -> List[str]:
    """Get current `internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `ossec.conf` file.
    """
    with open(CYB3RHQ_LOCAL_INTERNAL_OPTIONS) as f:
        lines = f.readlines()
    return lines


def set_cyb3rhq_local_internal_options(cyb3rhq_local_internal_options: List[str]):
    """Set up Cyb3rhq `local_internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `local_interal_options.conf` file.
    """
    with open(CYB3RHQ_LOCAL_INTERNAL_OPTIONS, 'w') as f:
        f.writelines(cyb3rhq_local_internal_options)


def add_cyb3rhq_local_internal_options(cyb3rhq_local_internal_options_dict):
    """Add new local internal options to the current configuration.

    Args:
        cyb3rhq_local_internal_options_dict (List of str): A list containing local internal options to add.
    """
    local_internal_options_str = create_local_internal_options(cyb3rhq_local_internal_options_dict)
    with open(CYB3RHQ_LOCAL_INTERNAL_OPTIONS, 'a') as f:
...
def create_local_internal_options(dict_local_internal_options):
    ...
    for key, value in dict_local_internal_options.items():
        cyb3rhq_local_internal_options += f"{str(key)}={str(value)}\n"
    return cyb3rhq_local_internal_options
...
def write_cyb3rhq_local_rules(local_rules: List[str]):
    """
...
def get_minimal_configuration():
    ...
    Returns:
        str: Cyb3rhq minimal configuration data.
    """
