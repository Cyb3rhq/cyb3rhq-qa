...
from cyb3rhq_testing.tools import CYB3RHQ_PATH, ACTIVE_RESPONSE_SOCKET_PATH
from cyb3rhq_testing.tools.utils import retry

request_socket = path.join(CYB3RHQ_PATH, 'queue', 'sockets', 'request')
request_protocol = "tcp"


class Cyb3rhqSocket:
    def __init__(self, socket_file=request_socket):
        """Encapsulate cyb3rhq-socket communication (header with message size)
    ...
    def send(self, msg, response_size=4):
        """Send and receive data to cyb3rhq-socket (header with message size)
        ...
        try:
            cyb3rhq_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            encoded_msg = msg_json.encode('utf-8')
            request_msg = struct.pack("<I", len(encoded_msg)) + encoded_msg

            connection(cyb3rhq_socket, self.file)
            send_msg(cyb3rhq_socket, request_msg)
            response = recv_response(cyb3rhq_socket, response_size)
            cyb3rhq_socket.close()
...
def send_request(msg_request, response_size=100, cyb3rhq_socket=request_socket):
    ...
    connection(sock, cyb3rhq_socket)
