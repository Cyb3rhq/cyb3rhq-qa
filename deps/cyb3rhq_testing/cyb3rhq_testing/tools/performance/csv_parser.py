...
class ClusterCSVParser:
    """Class to load and parse CSVs with data produced by the Cyb3rhq cluster.
    ...
    def _add_child_process_to_parent_process(files):
        ...
        concat_dfs["Daemon"] = concat_dfs["Daemon"].apply(lambda x: "cyb3rhq-clusterd")
        concat_dfs.reset_index(inplace=True)
        return {'cyb3rhq-clusterd': concat_dfs}
...
class ClusterCSVResourcesParser(ClusterCSVParser):
    """Class to load, parse CSVs and obtain stats of resources used by the 'cyb3rhq-clusterd' process.
    ...
    def __init__(self, artifacts_path, columns=None):
        ...
        super().__init__(artifacts_path, files_to_load=['cyb3rhq_clusterd', 'integrity_sync', 'cyb3rhq_clusterd_child_1',
                                                        'cyb3rhq_clusterd_child_2'])
