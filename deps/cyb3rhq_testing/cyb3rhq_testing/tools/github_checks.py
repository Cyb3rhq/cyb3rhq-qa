...
def version_is_released(version, organization='cyb3rhq', repository='cyb3rhq'):
...
def branch_exists(branch_name, organization='cyb3rhq', repository='cyb3rhq'):
...
def get_last_cyb3rhq_version():
    """Get the last Cyb3rhq version that has been tagged, regardless of possible release candidate tags.
    ...
    Returns:
        str: Last Cyb3rhq tag (no rc).
    """
    url = 'https://github.com/cyb3rhq/cyb3rhq/tags'
    req = requests.get(url)
    last_tags = re.findall(r'<a href="/cyb3rhq/cyb3rhq/releases/tag/v\d.\d.\d">', req.text)

    try:
        last_cyb3rhq_version = re.compile(r'<a href="/cyb3rhq/cyb3rhq/releases/tag/v(\d.\d.\d)').search(last_tags[0]).group(1)
    except IndexError:
        raise QAValueError(f"Could not find a valid cyb3rhq tag in {url}")

    return last_cyb3rhq_version
