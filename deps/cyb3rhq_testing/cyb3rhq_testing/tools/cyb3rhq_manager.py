...
def list_agents_ids():
    cyb3rhqdb_result = query_wdb('global get-all-agents last_id -1')
    list_agents = [agent['id'] for agent in cyb3rhqdb_result if not (0 == agent.get('id'))]

    return list_agents


def remove_agents(agents_id, remove_type='cyb3rhqdb'):
    ...
    if agents_id:
        for agent_id in agents_id:
            if remove_type == 'manage_agents':
                subprocess.call([f"{CYB3RHQ_PATH}/bin/manage_agents", "-r", f"{agent_id}"], stdout=open(os.devnull, "w"),
...
def create_group(group):
    ...
    """
    result = subprocess.run([f'{CYB3RHQ_PATH}/bin/agent_groups', '-a', '-q', '-g', f'{group}']).returncode
...
def delete_group(group):
    ...
    Returns:
        result(str): Return code
    """
    result = subprocess.run([f'{CYB3RHQ_PATH}/bin/agent_groups', '-r', '-q', '-g', f'{group}']).returncode
