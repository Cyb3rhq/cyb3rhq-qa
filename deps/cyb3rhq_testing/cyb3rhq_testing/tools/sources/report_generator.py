...
class LogAnalyzer:
    """This class group several statics methods to gather specific information from Cyb3rhq logs."""
    ...
    def keep_alive_log_parser(log_files):
        ...
        """
        keep_alive_regex = '(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}) cyb3rhq\-remoted.* inserting ' + \
                           '\'(.*)\|(.*)\|(.*)\|(.*)\|(.* \[.*\].*)\n(.*)\n.*"_agent_ip":(\S+)'
...
class StatisticsAnalyzer:
    """This class group several statics methods to gather specific information from Cyb3rhq statistics."""
...
class Cyb3rhqStatisticsAnalyzer():
    """This class group several statics methods to gather specific information from Cyb3rhq statistics."""
    @staticmethod
    def analyze_agentd_statistics(agentd_statistics_files):
        """Create a report for cyb3rhq-agentd daemon.
    ...
    def analyze_remoted_statistics(remoted_statistics_files):
        """Create a report for cyb3rhq-remoted daemon.
...
class ReportGenerator:
    ...
    def __init__(self, artifact_path):
        self.daemons_manager = ['cyb3rhq-modulesd', 'cyb3rhq-monitord', 'cyb3rhq-remoted', 'cyb3rhq-authd',
                                'cyb3rhq-db', 'cyb3rhq-syscheckd', 'cyb3rhq-analysisd']

        self.daemons_agent = ['cyb3rhq-modulesd', 'cyb3rhq-logcollector', 'cyb3rhq-syscheckd',
                              'cyb3rhq-agentd', 'cyb3rhq-execd']
    ...
    def get_instances_artifacts(self, component, hosts_regex=".*"):
        ...
        Args:
            component (str): Cyb3rhq installation type (agents/managers/all).
    ...
    def agentd_report(self, component='agents', hosts_regex='.*'):
        """Generate cyb3rhq-agentd daemon report, combining metric and statistics.

        Args:
            component (str): Cyb3rhq installation type (agents/managers/all).
            hosts_regex (str): Regex to filter by hostname.
        """
        agentd_report = Cyb3rhqStatisticsAnalyzer.analyze_agentd_statistics(self.get_instances_statistics('cyb3rhq-agentd',
                                                                          component,
                                                                          hosts_regex))
        return agentd_report

    def remoted_report(self, component='managers', hosts_regex='.*'):
        """Generate cyb3rhq-remoted daemon report, combining metric and statistics.

        Args:
            component (str): Cyb3rhq installation type (agents/managers/all).
            hosts_regex (str): Regex to filter by hostname.
        """
        remoted_report = Cyb3rhqStatisticsAnalyzer.analyze_remoted_statistics(self.get_instances_statistics(
                                                                                                        'cyb3rhq-remoted',
                                                                                                        component,
                                                                                                        hosts_regex))
    ...
    def metric_report(self, component='manager', hosts_regex='.*'):
        """Generate all Cyb3rhq metrics report.
    ...
    def make_report(self):
        ...
        report['managers'] = LogAnalyzer.get_error_logs_hosts(log_dict=self.get_instances_logs(log='all',
                                                                                               component='managers'))
        try:
            report['agents']['cyb3rhq-agentd'] = self.agentd_report()
        except Exception as e:
            unexpected_error = 'Unexpected error calculating agentd statistics'
            logging.error(unexpected_error)
            logging.error(e)
            report['agents']['cyb3rhq-agentd'] = {'ERROR': unexpected_error}

        try:
            report['managers']['cyb3rhq-remoted'] = self.remoted_report()
        except Exception as e:
            unexpected_error = 'Unexpected error calculating remoted statistics'
            logging.error(unexpected_error)
            logging.error(e)
            report['managers']['cyb3rhq-remoted'] = {'ERROR': unexpected_error}
