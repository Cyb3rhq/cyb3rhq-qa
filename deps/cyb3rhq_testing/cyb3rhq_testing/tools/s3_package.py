...
PROD_BUCKET = 'packages.cyb3rhq.com'
DEV_BUCKET = 'packages-dev.cyb3rhq.com'
...
def get_s3_package_url(repository, target, version, revision, system, architecture, install_dir='/var', short_url=True):
    ...
    Args:
        ...
        target (string): Type of the cyb3rhq installation package.
        ...
        install_dir (string): Cyb3rhq installation directory
...
def get_repository_url(target, version, system, revision, repository, architecture, short_url=True):
    ...
    system_path = {
       SYSTEMS['rpm']: '/yum',
       SYSTEMS['deb']: f"/apt/pool/main/w/cyb3rhq-{target}",
...
def get_package_name(target, version, system, revision, repository, architecture):
    ...
    if system == SYSTEMS['rpm']:
        ...
        package_name = f"cyb3rhq_agent_v{version}{revision_section}_linux_x86_64.wpk"
    elif system == SYSTEMS['wpk-windows']:
        revision_section = f"-{revision}" if revision != '1' else ''
        package_name = f"cyb3rhq_agent_v{version}{revision_section}_windows.wpk"
    else:
...
def get_production_package_url(cyb3rhq_target, os_system, cyb3rhq_version):
    ...
    Args:
        ...
        cyb3rhq_version (str): Cyb3rhq version (e.g 4.2.5)
    ...
    return get_s3_package_url('live', cyb3rhq_target, cyb3rhq_version, '1', package_system, architecture)


def get_last_production_package_url(cyb3rhq_target, os_system):
    ...
    """
    last_cyb3rhq_version = github_checks.get_last_cyb3rhq_version()

    return get_production_package_url(cyb3rhq_target, os_system, last_cyb3rhq_version)
