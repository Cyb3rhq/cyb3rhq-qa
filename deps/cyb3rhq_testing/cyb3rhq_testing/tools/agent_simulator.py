...
import cyb3rhq_testing.data.syscollector as syscollector
import cyb3rhq_testing.data.winevt as winevt
import cyb3rhq_testing.cyb3rhq_db as wdb
from cyb3rhq_testing import TCP
from cyb3rhq_testing import is_udp, is_tcp
from cyb3rhq_testing.tools.monitoring import cyb3rhq_unpack, Queue
from cyb3rhq_testing.tools.remoted_sim import Cipher
from cyb3rhq_testing.tools.utils import retry, get_random_ip, get_random_string
...
class Agent:
    ...
    Attributes:
        ...
        labels (dict): Cyb3rhq agent labels. Each dict key will be a new label. Default `None`.
    ...
    @staticmethod
    def cyb3rhq_padding(compressed_event):
        """Add the Cyb3rhq custom padding to each event sent.
        Args:
            compressed_event (bytes): Compressed event with zlib.
        Returns:
            bytes: Padded event.
        Examples:
            >>> cyb3rhq_padding(b'x\\x9c\\x15\\xc7\\xc9\\r\\x00 \\x08\\x04\\xc0\\x96\\\\\\x94\\xcbn0H\\x03\\xda\\x7f
    ...
    def create_event(self, message):
        ...
        # Padding
        padded_event = self.cyb3rhq_padding(compressed_event)
        # Encrypt
