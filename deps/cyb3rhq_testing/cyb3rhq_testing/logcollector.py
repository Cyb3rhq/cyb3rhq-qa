...
from cyb3rhq_testing.tools import LOGCOLLECTOR_STATISTICS_FILE, CYB3RHQ_PATH, monitoring
...
DEFAULT_AUTHD_REMOTED_SIMULATOR_CONFIGURATION = {
    ...
    'server_keys': os.path.join(CYB3RHQ_PATH, 'etc', 'sslmanager.key'),
    'server_cert': os.path.join(CYB3RHQ_PATH, 'etc', 'sslmanager.cert'),
    'authd_port': 1515,
    'remoted_port': 1514,
    'protocol': 'tcp',
...
def callback_unable_to_open(file_path, n_attempt):
    """Create a callback to detect if `cyb3rhq-logcollector` fails to open the specified file.
    Args:
        file_path (str): Path of the file that `cyb3rhq-logcollector` is trying to open.
...
def callback_trying_to_reconnect(location, reconnect_time):
    """Create a callback to detect if `cyb3rhq-agentd` is trying to reconnect to specified channel.
    Args:
...
def callback_invalid_format_value(line, option, location):
    ...
    Args:
        ...
        location (str): Cyb3rhq manager configuration option.
...
def get_data_sending_stats(log_path, socket_name):
    ...
    For this purpose, it parses the "cyb3rhq-logcollector.state" file and retrieves the data.
    See:
    https://documentation-dev.cyb3rhq.com/current/user-manual/reference/statistics-files/cyb3rhq-logcollector-state.html
...
def get_next_stats(current_stats, log_path, socket_name, state_interval):
    """Return the next statistics to be written to the "cyb3rhq-logcollector.state" file and the seconds elapsed.
...
def compose_macos_log_command(type='', level='', predicate='', is_sierra=False):
    """
    This function replicates how the command 'log' will be called from the Cyb3rhq agent given the query parameters
