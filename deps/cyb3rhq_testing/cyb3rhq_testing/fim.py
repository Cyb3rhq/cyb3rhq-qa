...
from cyb3rhq_testing.tools.file import generate_string
...
def change_internal_options(param, value, opt_path=None, value_regex='[0-9]*'):
    ...
    """
    if opt_path is None:
        local_conf_path = os.path.join(CYB3RHQ_PATH, 'local_internal_options.conf') if sys.platform == 'win32' else \
            os.path.join(CYB3RHQ_PATH, 'etc', 'local_internal_options.conf')
    else:
...
def change_conf_param(param, value):
    ...
    Args:
        ...
        value (obj): new value.
    """
    conf_path = os.path.join(CYB3RHQ_PATH, 'ossec.conf') if sys.platform == 'win32' else \
        os.path.join(CYB3RHQ_PATH, 'etc', 'ossec.conf')
...
def callback_audit_key(line):
    if 'Match audit_key' in line and 'key="cyb3rhq_hc"' not in line and 'key="cyb3rhq_fim"' not in line:
        return line
    return None
...
def calculate_registry_diff_paths(reg_key, reg_subkey, arch, value_name):
    ...
    diff_file = os.path.join(CYB3RHQ_PATH, 'queue', 'diff', 'registry', folder_path,
...
def detect_initial_scan_start(file_monitor):
    """Detect initial scan start when restarting Cyb3rhq.
...
def detect_realtime_start(file_monitor):
    """Detect realtime engine start when restarting Cyb3rhq.
...
def detect_whodata_start(file_monitor):
    """Detect whodata engine start when restarting Cyb3rhq.
...
def create_folder_file(host_manager, folder_path):
    # Create folder
    host_manager.run_command('cyb3rhq-agent1', f'mkdir {folder_path}')
...
def query_db(host_manager, script, db_path, query):
    return host_manager.run_command('cyb3rhq-manager', "python {} --db_path {} --query {}".format(script, db_path, query))
