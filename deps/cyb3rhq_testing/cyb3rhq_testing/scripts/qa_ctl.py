...
from cyb3rhq_testing.qa_ctl.deployment.qa_infraestructure import QAInfraestructure
from cyb3rhq_testing.qa_ctl.provisioning.qa_provisioning import QAProvisioning
from cyb3rhq_testing.qa_ctl.run_tests.qa_test_runner import QATestRunner
from cyb3rhq_testing.qa_ctl.run_tasks.qa_tasks_launcher import QATasksLauncher
from cyb3rhq_testing.qa_ctl.configuration.qa_ctl_configuration import QACTLConfiguration
...
from cyb3rhq_testing.tools.file import recursive_directory_creation
...
TASKS_KEY = 'tasks'
TEST_KEY = 'tests'
CYB3RHQ_QA_FILES = os.path.join(gettempdir(), 'cyb3rhq_qa_ctl', 'cyb3rhq-qa')
...
def validate_configuration_data(configuration_data, qa_ctl_mode):
    ...
    if sys.platform == 'win32' and qa_ctl_mode == MANUAL_MODE:
        ...
        if not github_checks.branch_exists(configuration_data['config']['qa_ctl_launcher_branch'],
                                           repository=CYB3RHQ_QA_REPO):
            raise QAValueError(f"{configuration_data['config']['qa_ctl_launcher_branch']} branch specified as "
                               'qa_ctl_launcher_branch  does not exist in Cyb3rhq QA repository.', qactl_logger.error,
                               QACTL_LOGGER)
...
def validate_test_module(type=None, component=None, suite=None, module=None):
    ...
    Args:
        ...
        module (str): Test module.
    """
    tests_path = os.path.join(CYB3RHQ_QA_FILES, 'tests')
...
def set_parameters(parameters):
    ...
    Raises:
        QAValueError: If could not find a valid cyb3rhq tag in the first github tags page (It can happen if on the first
    ...
    try:
        parameters.version = parameters.version if parameters.version else github_checks.get_last_cyb3rhq_version()
    except QAValueError:
        raise QAValueError('The latest version of Cyb3rhq could not be obtained. Maybe there is no valid (non-rc) one at '
                           'https://github.com/cyb3rhq/cyb3rhq/tags. Try specifying the version manually using the '
...
def set_environment(parameters):
    ...
    Args:
        (argparse.Namespace): Object with the user parameters.
    """
    # Create the cyb3rhq_qa_ctl temporary folder
    recursive_directory_creation(os.path.join(gettempdir(), 'cyb3rhq_qa_ctl'))

    if parameters.run:
        # Download cyb3rhq-qa repository locally to run qa-docs tool and get the tests info
        local_actions.download_local_cyb3rhq_qa_repository(branch=parameters.qa_branch,
                                                         path=os.path.join(gettempdir(), 'cyb3rhq_qa_ctl'))
...
def validate_parameters(parameters):
    ...
    Raises:
        QAValueError: If parameters are incompatible, or version has not a valid format, or the specified cyb3rhq version
                      has not been released, or cyb3rhq QA branch does not exist (calculated from cyb3rhq_version).
    """
    ...
    if not github_checks.version_is_released(parameters.version):
        raise QAValueError(f"The cyb3rhq {parameters.version} version has not been released. Enter a right version.",
                           qactl_logger.error, QACTL_LOGGER)
    ...
    # Check if specified tests exist. Cyb3rhq-qa repository needs to be downloaded locally before.
    if parameters.run:
...
def get_script_parameters():
    ...
    description = \
        ...
        Description: qa-ctl is a tool for launching tests locally, automating the deployment, provisioning and testing
                     ...
                     You can find more information in https://github.com/cyb3rhq/cyb3rhq-qa/wiki/QACTL-tool
        '''
    ...
    parser.add_argument('--qa-branch', type=str, action='store', required=False, dest='qa_branch',
                        help='Set a custom cyb3rhq-qa branch to use in the run and provisioning. This '
...
def main():
    ...
    if qa_ctl_mode == AUTOMATIC_MODE:
        ...
        config_generator = QACTLConfigGenerator(modules_data, arguments.version, arguments.qa_branch,
                                                CYB3RHQ_QA_FILES, arguments.operating_systems)
