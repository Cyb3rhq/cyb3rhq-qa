...
from cyb3rhq_testing.tools.performance.statistic import StatisticMonitor, logger

METRICS_FOLDER = join(gettempdir(), 'cyb3rhq_statistics')
...
def get_script_arguments():
    parser = argparse.ArgumentParser(usage="%(prog)s [options]", description="Cyb3rhq statistics collector",
    ...
    parser.add_argument('-t', '--target', dest='target_list', required=True, type=str, nargs='+', action='store',
                        help='Type the statistics target to collect separated by whitespace. '
                             'Targets: agent, logcollector, remote, analysis and cyb3rhqdb')
...
def main():
    ...
    logging.basicConfig(filename=join(METRICS_FOLDER, 'cyb3rhq-statistics.log'), filemode='a',
