...
from cyb3rhq_testing.qa_docs.lib.config import Config
from cyb3rhq_testing.qa_docs.lib.index_data import IndexData
from cyb3rhq_testing.qa_docs.lib.sanity import Sanity
from cyb3rhq_testing.qa_docs.lib import utils
from cyb3rhq_testing.qa_docs.doc_generator import DocGenerator
...
def validate_parameters(parameters, parser):
    ...
    if parameters.run_with_docker:
        ...
        if f"remotes/origin/{parameters.qa_branch}" not in branches:
            raise QAValueError(f"{parameters.qa_branch} not found in Cyb3rhq-QA repo.",
                               qadocs_logger.error)
    else:
