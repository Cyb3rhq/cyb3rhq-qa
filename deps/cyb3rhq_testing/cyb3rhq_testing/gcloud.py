...
def callback_detect_start_gcp(line):
    match = re.match(r'.*cyb3rhq-modulesd:gcp-pubsub.*INFO: Module started.$', line)
    if match:
        return line
    return None
...
def detect_gcp_start(file_monitor):
    """Detect module gcp-pubsub starts after restarting Cyb3rhq.
...
def callback_detect_all_gcp(line):
    match = re.match(r'.*cyb3rhq-modulesd:gcp-pubsub.*', line)
    if match:
        return line
    return None
...
def publish(id_project, name_topic, credentials, messages=None):
    if CYB3RHQ_PATH in credentials:
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "{}".format(credentials)
    else:
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "{}/{}".format(CYB3RHQ_PATH, credentials)
