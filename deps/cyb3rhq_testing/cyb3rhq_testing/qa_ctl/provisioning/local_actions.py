...
from cyb3rhq_testing.tools.github_checks import branch_exists
from cyb3rhq_testing.tools.logging import Logging
from cyb3rhq_testing.tools.exceptions import QAValueError
from cyb3rhq_testing.tools.file import delete_path_recursively, recursive_directory_creation
...
def download_local_cyb3rhq_qa_repository(branch, path):
    """Download cyb3rhq QA repository in local machine.

    Important note: Path must not include the cyb3rhq-qa folder

    Args:
        branch (string): Cyb3rhq QA repository branch.
        path (string): Local path where save the repository files.
    """
    # Create path if it does not exist
    recursive_directory_creation(path)

    cyb3rhq_qa_path = os.path.join(path, 'cyb3rhq-qa')
    mute_output = '&> /dev/null' if sys.platform != 'win32' else '>nul 2>&1'
    command = ''

    if not branch_exists(branch, repository=CYB3RHQ_QA_REPO):
        raise QAValueError(f"{branch} branch does not exist in Cyb3rhq QA repository.", LOGGER.error, QACTL_LOGGER)

    # Delete previous files if exist
    delete_path_recursively(cyb3rhq_qa_path)

    if sys.platform == 'win32':
        command = f"cd {path} && curl -OL https://github.com/cyb3rhq/cyb3rhq-qa/archive/{branch}.tar.gz {mute_output} && " \
                  f"tar -xzf {branch}.tar.gz {mute_output} && move cyb3rhq-qa-{branch} cyb3rhq-qa {mute_output} && " \
                  f"del {branch}.tar.gz {mute_output}"
    else:
        command = f"cd {path} && curl -Ls https://github.com/cyb3rhq/cyb3rhq-qa/archive/{branch}.tar.gz | tar zx " \
                  f"{mute_output} && mv cyb3rhq-* cyb3rhq-qa {mute_output} && rm -rf *tar.gz {mute_output}"

    LOGGER.debug(f"Downloading {branch} files of cyb3rhq-qa repository in {cyb3rhq_qa_path}")
...
def qa_ctl_docker_run(config_file, qa_branch, debug_level, topic):
    ...
    Args:
        ...
        qa_branch (str): Cyb3rhq qa branch with which qa-ctl will be launched.
    ...
    docker_image_name = 'cyb3rhq/qa-ctl'
    ...
    run_local_command_printing_output(f"docker run --rm -v {os.path.join(gettempdir(), 'cyb3rhq_qa_ctl')}:/cyb3rhq_qa_ctl "
