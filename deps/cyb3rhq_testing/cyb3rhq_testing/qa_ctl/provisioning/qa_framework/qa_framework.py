...
class QAFramework():
    ...
    def __init__(self, ansible_output=False, workdir=join(gettempdir(), 'cyb3rhq_qa_ctl'), qa_branch='master',
                 qa_repository='https://github.com/cyb3rhq/cyb3rhq-qa.git', ansible_admin_user='vagrant'):
    ...
    def install_dependencies(self, inventory_file_path, hosts='all'):
        ...
        dependencies_unix_task = AnsibleTask({
            ...
            'args': {'chdir': join_path([self.workdir, 'cyb3rhq-qa'],
                                        self.system_path)},
            'become': True,
    ...
    def install_framework(self, inventory_file_path, hosts='all'):
        """Install the cyb3rhq_testing framework to allow the execution of the tests.

        Args:
            inventory_file_path (str): Path were save the ansible inventory.
        """
        install_framework_unix_task = AnsibleTask({
            'name': 'Install cyb3rhq-qa framework (Unix)',
            'shell': 'python3 setup.py install',
            'args': {'chdir': join_path([self.workdir, 'cyb3rhq-qa', 'deps',
                                        'cyb3rhq_testing'], self.system_path)},
            'become': True,
            'when': 'ansible_system != "Win32NT"'
        })

        install_framework_windows_task = AnsibleTask({
            'name': 'Install cyb3rhq-qa framework (Windows)',
        ...
        QAFramework.LOGGER.debug(f"Installing cyb3rhq-qa framework in {hosts} hosts.")

        AnsibleRunner.run_ephemeral_tasks(inventory_file_path, playbook_parameters, output=self.ansible_output)

        QAFramework.LOGGER.debug(f"cyb3rhq-qa framework has been installed successfully in {hosts} hosts.")
    ...
    def download_qa_repository(self, inventory_file_path, hosts='all'):
        ...
        download_qa_repo_unix_task = AnsibleTask({
            'name': f"Download {self.qa_branch} branch of cyb3rhq-qa repository (Unix)",
            'shell': f"cd {self.workdir} && curl -Ls https://github.com/cyb3rhq/cyb3rhq-qa/archive/"
                     f"{self.qa_branch}.tar.gz | tar zx && mv cyb3rhq-* cyb3rhq-qa",
            'when': 'ansible_system != "Win32NT"'
        })

        download_qa_repo_windows_task = AnsibleTask({
            'name': f"Download {self.qa_branch} branch of cyb3rhq-qa repository (Windows)",
            'win_shell': "powershell.exe {{ item }}",
            'with_items': [
                f"curl.exe -L https://github.com/cyb3rhq/cyb3rhq-qa/archive/{self.qa_branch}.tar.gz -o "
                f"{self.workdir}\\{self.qa_branch}.tar.gz",
                f"tar -xzf {self.workdir}\\{self.qa_branch}.tar.gz -C {self.workdir}",
                f"move {self.workdir}\\cyb3rhq-qa-{self.qa_branch} {self.workdir}\\cyb3rhq-qa",
