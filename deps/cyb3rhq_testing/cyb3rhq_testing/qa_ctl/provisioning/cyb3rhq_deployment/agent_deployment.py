...
class AgentDeployment(Cyb3rhqDeployment):
    """Deploy Cyb3rhq agent with all the elements needed, set from the configuration file
    ...
    def install(self):
        """Child method to install Cyb3rhq in agent
    ...
    def register_agent(self):
        """Set the manager ip in the ossec.conf, and restart Cyb3rhq agent to let autoenrollment register it.
    ...
    def health_check(self):
        ...
        tasks_list.append(AnsibleTask({
            ...
            'register': 'status',
            'become': True,
            'failed_when': ['"cyb3rhq-agentd" not in status.stdout',
                            '"cyb3rhq-execd" not in status.stdout']
        }))
