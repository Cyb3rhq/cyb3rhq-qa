...
class Cyb3rhqInstallation(ABC):
    """Install a Cyb3rhq instance set from the configuration file

    Args:
        cyb3rhq_target (string): Type of the cyb3rhq installation desired (manager or agent).
        installation_files_path (string): Path where the Cyb3rhq instalation files are located.
    ...
    def __init__(self, cyb3rhq_target, installation_files_path, qa_ctl_configuration):
        self.cyb3rhq_target = cyb3rhq_target
    ...
    def download_installation_files(self, inventory_file_path, ansible_tasks, hosts='all'):
        """Download the installation files of Cyb3rhq  by creating an ansible playbook and launching it
