...
from cyb3rhq_testing.qa_ctl.provisioning.ansible import read_ansible_instance, remove_known_host
from cyb3rhq_testing.qa_ctl.provisioning.ansible.ansible_inventory import AnsibleInventory
from cyb3rhq_testing.qa_ctl.provisioning.cyb3rhq_deployment.cyb3rhq_local_package import Cyb3rhqLocalPackage
from cyb3rhq_testing.qa_ctl.provisioning.cyb3rhq_deployment.cyb3rhq_s3_package import Cyb3rhqS3Package
from cyb3rhq_testing.qa_ctl.provisioning.cyb3rhq_deployment.cyb3rhq_sources import Cyb3rhqSources
from cyb3rhq_testing.qa_ctl.provisioning.cyb3rhq_deployment.agent_deployment import AgentDeployment
from cyb3rhq_testing.qa_ctl.provisioning.cyb3rhq_deployment.manager_deployment import ManagerDeployment
...
from cyb3rhq_testing.qa_ctl.provisioning.qa_framework.qa_framework import QAFramework
...
from cyb3rhq_testing.qa_ctl.provisioning.local_actions import qa_ctl_docker_run


class QAProvisioning():
    """Class to control different options and instances to provisioning with Cyb3rhq and QA Framework.
    ...
    Attributes:
        ...
        cyb3rhq_installation_paths (dict): Dict indicating the Cyb3rhq installation paths for every host.
        qa_ctl_configuration (QACTLConfiguration): QACTL configuration.
    """
    ...
    def __init__(self, provision_info, qa_ctl_configuration):
        ...
        self.cyb3rhq_installation_paths = {}
    ...
    def __process_config_data(self, host_provision_info):
        """Process config file info to generate all the tasks needed for deploy Cyb3rhq
        ...
        if 'cyb3rhq_deployment' in host_provision_info:
            deploy_info = host_provision_info['cyb3rhq_deployment']
            health_check = False if 'health_check' not in host_provision_info['cyb3rhq_deployment'] \
                else host_provision_info['cyb3rhq_deployment']['health_check']
            ...
            cyb3rhq_install_path = '/var/ossec' if 'cyb3rhq_install_path' not in deploy_info else \
                deploy_info['cyb3rhq_install_path']
            cyb3rhq_branch = 'master' if 'cyb3rhq_branch' not in deploy_info else deploy_info['cyb3rhq_branch']
            ...
            installation_files_parameters = {'cyb3rhq_target': install_target}
            ...
            if install_type == 'sources':
                installation_files_parameters['cyb3rhq_branch'] = cyb3rhq_branch
                installation_files_parameters['cyb3rhq_install_path'] = cyb3rhq_install_path
                installation_instance = Cyb3rhqSources(**installation_files_parameters)
            ...
            if install_type == 'package':
                if s3_package_url is None and local_package_path is None:
                    ...
                    installation_instance = Cyb3rhqS3Package(**installation_files_parameters)
                    ...
                    installation_instance = Cyb3rhqLocalPackage(**installation_files_parameters)
                    remote_files_path = installation_instance.download_installation_files(self.inventory_file_path,
                                                                                          hosts=current_host)
                else:
            ...
            if install_target == 'agent':
                deployment_instance = AgentDeployment(remote_files_path,
                                                      ...
                                                      install_dir_path=cyb3rhq_install_path,
            ...
            if health_check:
                # Wait for Cyb3rhq initialization before health_check
                health_check_sleep_time = 30
                QAProvisioning.LOGGER.info(f"Performing a Cyb3rhq installation healthcheck in {current_host} host")
                sleep(health_check_sleep_time)
                deployment_instance.health_check()

            self.cyb3rhq_installation_paths[deployment_instance.hosts] = deployment_instance.install_dir_path

        if 'qa_framework' in host_provision_info:
            qa_framework_info = host_provision_info['qa_framework']
            cyb3rhq_qa_branch = 'master' if 'cyb3rhq_qa_branch' not in qa_framework_info \
                else qa_framework_info['cyb3rhq_qa_branch']

            QAProvisioning.LOGGER.info(f"Provisioning the {current_host} host with the Cyb3rhq QA framework using "
                                       f"{cyb3rhq_qa_branch} branch.")
            qa_instance = QAFramework(qa_branch=cyb3rhq_qa_branch, workdir=qa_framework_info['qa_workdir'],
    ...
    def run(self):
        ...
        if sys.platform == 'win32':
            ...
            tmp_config_file = os.path.join(gettempdir(), 'cyb3rhq_qa_ctl', tmp_config_file_name)
