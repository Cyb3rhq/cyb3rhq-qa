...
from cyb3rhq_testing.qa_ctl.provisioning.local_actions import run_local_command_returning_output
...
class QACTLConfigGenerator:
    ...
    be also a cyb3rhq version parameter that indicates which version of cyb3rhq the user wants to install.

    Args:
        tests (list): list with all the test that the user desires to run.
        cyb3rhq_version (string): version of the cyb3rhq packages that the user desires to install.
        ...
        cyb3rhq package version will be taken from the documetation test information
    ...
    WINDOWS_DEFAULT_CYB3RHQ_INSTALL_PATH = 'C:\\Program Files (x86)\\ossec-agent'
    LINUX_DEFAULT_CYB3RHQ_INSTALL_PATH = '/var/ossec'
    ...
    def __init__(self, test_modules_data=None, cyb3rhq_version=None, qa_branch='master',
                 qa_files_path=join(gettempdir(), 'cyb3rhq_qa_ctl', 'cyb3rhq-qa'), systems=None):
        self.test_modules_data = test_modules_data
        self.cyb3rhq_version = cyb3rhq_version
        self.systems = systems
        self.qactl_used_ips_file = join(gettempdir(), 'cyb3rhq_qa_ctl', 'qactl_used_ips.txt')
        self.config_file_path = join(gettempdir(), 'cyb3rhq_qa_ctl', f"config_{get_current_timestamp()}.yaml")
        self.config = {}
        self.hosts = []
        ...
        file.recursive_directory_creation(join(gettempdir(), 'cyb3rhq_qa_ctl'))
    ...
    def __get_module_info(self, type, component, suite, module):
        ...
        qa_docs_command = f"qa-docs -p {join(self.qa_files_path, 'tests')} -o {join(gettempdir(), 'cyb3rhq_qa_ctl')} " \
                          f"-t {type} -c {component} {suite_command} -m {module} --no-logging"
        test_data_file_path = f"{join(gettempdir(), 'cyb3rhq_qa_ctl', 'output', module)}.json"
    ...
    def __validate_test_info(self, test_info):
        ...
        if parse(str(test_info['tests'][0]['cyb3rhq_min_version'])) > parse(str(self.cyb3rhq_version)):
            error_message = f"The minimal version of cyb3rhq to launch the {test_info['test_name']} is " \
                            f"{test_info['tests'][0]['cyb3rhq_min_version']} and you are using {self.cyb3rhq_version}"
            raise QAValueError(error_message, QACTLConfigGenerator.LOGGER.error, QACTL_LOGGER)
    ...
    def __add_instance(self, os_version, test_name, test_target, os_platform):
        ...
        instance = {
            'enabled': True,
            'vagrantfile_path': join(gettempdir(), 'cyb3rhq_qa_ctl'),
    ...
    def __get_package_url(self, instance):
        ...
        architecture = Cyb3rhqS3Package.get_architecture(system)

        package_url = get_s3_package_url('live', target, self.cyb3rhq_version, '1', system, architecture)
    ...
    def __process_provision_data(self):
        ...
        for instance in self.config['deployment'].keys():
            self.config['provision']['hosts'][instance] = {'host_info': {}, 'cyb3rhq_deployment': {}, 'qa_framework': {}}
            ...
            cyb3rhq_install_path = self.WINDOWS_DEFAULT_CYB3RHQ_INSTALL_PATH if system == 'windows' else \
                self.LINUX_DEFAULT_CYB3RHQ_INSTALL_PATH

            self.config['provision']['hosts'][instance]['cyb3rhq_deployment'] = {
                'type': 'package',
                'target': target,
                's3_package_url': s3_package_url,
                'installation_files_path': installation_files_path,
                'health_check': True,
                'cyb3rhq_install_path': cyb3rhq_install_path
            }
            ...
            if target == 'agent':
                ...
                self.config['provision']['hosts'][instance]['cyb3rhq_deployment']['manager_ip'] = \
            ...
            self.config['provision']['hosts'][instance]['qa_framework'] = {
                'cyb3rhq_qa_branch': self.qa_branch,
                'qa_workdir': file.join_path([installation_files_path, 'cyb3rhq_qa_ctl'], system)
            }
    ...
    def __add_testing_config_block(self, instance, installation_files_path, system, test_path, test_name, modules,
        ...
        Args:
            ...
            installation_files_path (str): Path where locate cyb3rhq qa-ctl files.
        ...
        self.config['tests'][instance]['test'] = {
            'type': 'pytest',
            'path': {
                'test_files_path': file.join_path([installation_files_path, 'cyb3rhq_qa_ctl', 'cyb3rhq-qa',
                                                   test_path], system),
                'run_tests_dir_path': file.join_path([installation_files_path, 'cyb3rhq_qa_ctl', 'cyb3rhq-qa',
                                                      'tests', 'integration'], system),
                'test_results_path': join(gettempdir(), 'cyb3rhq_qa_ctl', f"{test_name}_{get_current_timestamp()}")
            },
            'cyb3rhq_install_path': cyb3rhq_install_path,
            'system': system,
            'component': component,
    ...
    def __set_testing_config(self, tests_info):
        ...
        for test in tests_info:
            ...
            component = self.config['provision']['hosts'][instance]['cyb3rhq_deployment']['target']
            # Cut out the full path, and convert it to relative path (tests/integration....)
            test_path = re.sub(r".*cyb3rhq-qa.*(tests.*)", r"\1", test['path'])
