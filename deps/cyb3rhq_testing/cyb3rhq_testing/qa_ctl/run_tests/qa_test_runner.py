...
from cyb3rhq_testing.qa_ctl.run_tests.test_launcher import TestLauncher
from cyb3rhq_testing.qa_ctl.run_tests.pytest import Pytest
...
class QATestRunner():
    ...
    def __build_test(self, test_params, host=['all'], ansible_admin_user=None):
        ...
        if test_params['type'] == 'pytest':
            ...
            test_dict['cyb3rhq_install_path'] = test_params['cyb3rhq_install_path'] if 'cyb3rhq_install_path' in test_params \
                else None
    ...
    def run(self):
        ...
        if sys.platform == 'win32':
            ...
            # the Windows host and the docker container (Windows tmp as /cyb3rhq_qa_ctl).
            test_results_folder = f"test_results_{get_current_timestamp()}"
            temp_test_results_files_path = f"/cyb3rhq_qa_ctl/{test_results_folder}"
            ...
            try:
                ...
                for _, host_data in self.test_parameters.items():
                    source_directory = os.path.join(gettempdir(), 'cyb3rhq_qa_ctl', f"{test_results_folder}_{index}")
