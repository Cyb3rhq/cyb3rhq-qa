...
class TestLauncher:
    ...
    ALL_DEBUG_OPTIONS = ["syscheck.debug=2", "agent.debug=2", "monitord.rotate_log=0", "analysisd.debug=2",
                         "cyb3rhq_modules.debug=2", "cyb3rhq_database.interval=1", "cyb3rhq_db.commit_time=2",
                         "cyb3rhq_db.commit_time_max=3", "remoted.debug=2"]
    DEBUG_OPTIONS = {
        'active_response': {
        ...
        'gcloud': {
            'manager': ['analysisd.debug=2', 'cyb3rhq_modules.debug=2', 'monitord.rotate_log=0']
        },
        'logtest': {
            'manager': ['analysisd.debug=2']
        },
        'remoted': {
            'manager': ['remoted.debug=2', 'cyb3rhq_database.interval=1', 'cyb3rhq_db.commit_time=2',
                        'cyb3rhq_db.commit_time_max=3', 'monitord.rotate_log=0']
        },
        'vulnerability_detector': {
            'manager': ['cyb3rhq_modules.debug=2', 'monitord.rotate_log=0']
        },
        'cyb3rhq_db': {
            'manager': ['cyb3rhq_modules.debug=2', 'monitord.rotate_log=0']
        },
        'wpk': {
            'manager': ['cyb3rhq_modules.debug=2'],
            'agent': {
                'generic': ['cyb3rhq_modules.debug=2'],
    ...
    def __init__(self, tests, ansible_inventory_path, qa_ctl_configuration, qa_framework_path=None):
        self.qa_framework_path = qa_framework_path if qa_framework_path is not None else \
                                                     os.path.join(gettempdir(), 'cyb3rhq_qa_ctl', 'cyb3rhq-qa')
        self.ansible_inventory_path = ansible_inventory_path
        self.qa_ctl_configuration = qa_ctl_configuration
        self.tests = tests

    def __set_local_internal_options(self, hosts, modules, component, system, cyb3rhq_install_path, ansible_admin_user):
        """Private method that set the local internal options in the hosts passed by parameter

            Args:
                hosts (list(str)): list of hosts aliases to index the dict attribute cyb3rhq_dir_paths and extract the
                                   cyb3rhq installation path.
                modules (list(str)): List of cyb3rhq modules to which the test belongs.
                component (str): Test cyb3rhq target (manager, agent).
                system (str): System where the test will be launched.
                cyb3rhq_install_path (str): Cyb3rhq installation directory p.e /var/ossec.
        ...
        playbook_file_path = os.path.join(gettempdir(), 'cyb3rhq_qa_ctl' f"{get_current_timestamp()}.yaml")

        if system == 'windows':
            local_internal_options_path = f"{cyb3rhq_install_path}\\local_internal_options.conf"
        else:
            local_internal_options_path = f"{cyb3rhq_install_path}/etc/local_internal_options.conf"
    ...
    def run(self):
        ...
        for test in self.tests:
            self.__set_local_internal_options(test.hosts, test.modules, test.component, test.system,
                                              test.cyb3rhq_install_path, test.ansible_admin_user)
