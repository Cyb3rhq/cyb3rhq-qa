...
from cyb3rhq_testing.tools import GLOBAL_DB_PATH, CYB3RHQ_DB_SOCKET_PATH
from cyb3rhq_testing.tools.monitoring import cyb3rhq_pack, cyb3rhq_unpack
from cyb3rhq_testing.tools.services import control_service


def callback_cyb3rhqdb_response(item):
...
def mock_db(func):
    """Decorator used in any function that needs to mock a cyb3rhq db

    This function will execute `func` after stopping cyb3rhq-modulesd and cyb3rhq-db. After that,
    ...
    def magic(*args, **kwargs):
        control_service('stop', daemon='cyb3rhq-modulesd')
        func(*args, **kwargs)
        control_service('start', daemon='cyb3rhq-modulesd')
...
def run_query(db_query, db_path=GLOBAL_DB_PATH):
    """Method used to run sqlite queries on cyb3rhq databases
...
def query_wdb(command):
    ...
    try:
        sock.send(cyb3rhq_pack(len(command)) + command.encode())
