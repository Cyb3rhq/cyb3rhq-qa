...
from cyb3rhq_testing import UDP, TCP, ARCHIVES_LOG_PATH, LOG_FILE_PATH, QUEUE_SOCKETS_PATH, CYB3RHQ_PATH
from cyb3rhq_testing.tools.file import bind_unix_socket, truncate_file
from cyb3rhq_testing.tools.monitoring import FileMonitor, make_callback, ManInTheMiddle, QueueMonitor, \
...
EXAMPLE_VALID_USER_LOG_EVENT = '2021-03-04T02:16:16.998693-05:00 centos-8 su - - [timeQuality tzKnown="1" ' \
                               'isSynced="0"] pam_unix(su:session): session opened for user cyb3rhq_qa by (uid=0)'
...
def new_agent_group(group_name=DEFAULT_TESTING_GROUP_NAME, configuration_file='agent.conf'):
    ...
    sb.run([f"{CYB3RHQ_PATH}/bin/agent_groups", "-q", "-a", "-g", group_name])

    agent_conf_path = os.path.join(data_path, configuration_file)

    with open(f"{CYB3RHQ_PATH}/etc/shared/{group_name}/agent.conf", "w") as agent_conf_file:
        with open(agent_conf_path, 'r') as configuration:
            agent_conf_file.write(configuration.read())


def remove_agent_group(group_name):
    sb.run([f"{CYB3RHQ_PATH}/bin/agent_groups", "-q", "-r", "-g", group_name])


def add_agent_to_group(group_name, agent_id):
    sb.run([f"{CYB3RHQ_PATH}/bin/agent_groups", "-q", "-a", "-i", agent_id, "-g", group_name])
...
def callback_invalid_value(option, value):
    ...
    Args:
        option (str): Cyb3rhq manager configuration option.
...
def callback_ignored_invalid_protocol(protocol):
    ...
    Args:
        protocol (str): Cyb3rhq manager protocol.
...
def callback_detect_remoted_started(port, protocol, connection_type="secure"):
    ...
    cyb3rhq-remoted logs if it has correctly started for each connection type, the port and the protocol in the ossec.log

    Args:
        port (int): port configured for cyb3rhq-remoted.
        protocol (str): protocol configured for cyb3rhq-remoted. It can be UDP, TCP or both options at the same time.
...
def create_archives_log_monitor():
    ...
    cyb3rhq_archives_log_monitor = FileMonitor(ARCHIVES_LOG_PATH)

    return cyb3rhq_archives_log_monitor
...
def check_syslog_event(cyb3rhq_archives_log_monitor, message, port, protocol, timeout=10):
    """Check if a syslog event is properly received by the manager.

    Args:
        cyb3rhq_archives_log_monitor (FileMonitor): FileMonitor object to monitor the archives.log.
    ...
    # Syslog events may contain a PRI header at the beginning of the message <1>. If cyb3rhq-remoted receives a message
    ...
    for msg in parsed_msg.split("\n"):
        detect_archives_log_event(archives_monitor=cyb3rhq_archives_log_monitor,
...
def check_remoted_log_event(cyb3rhq_log_monitor, callback_pattern, error_message='', update_position=False,
                            timeout=REMOTED_GLOBAL_TIMEOUT):
    """Allow to monitor the ossec.log file and search for a remoted event.

    Args:
        cyb3rhq_log_monitor (FileMonitor): FileMonitor object to monitor the Cyb3rhq log.
...
def check_tcp_connection_established_log(cyb3rhq_log_monitor, update_position=False, ip_address='127.0.0.1'):
    ...
    check_remoted_log_event(cyb3rhq_log_monitor, callback_pattern, error_message, update_position)


def wait_to_remoted_key_update(cyb3rhq_log_monitor):
    ...
    check_remoted_log_event(cyb3rhq_log_monitor, callback_pattern, error_message, timeout=20)


def wait_to_remoted_update_groups(cyb3rhq_log_monitor):
    ...
    check_remoted_log_event(cyb3rhq_log_monitor, callback_pattern, error_message, timeout=SYNC_FILES_TIMEOUT)


def send_agent_event(cyb3rhq_log_monitor, message=EXAMPLE_MESSAGE_EVENT, protocol=TCP, manager_address='127.0.0.1',
    ...
    wait_to_remoted_key_update(cyb3rhq_log_monitor)
...
def check_queue_socket_event(raw_events=EXAMPLE_MESSAGE_PATTERN, timeout=30, update_position=False):
    ...
    try:
        ...
        mitm.shutdown()
        control_service('start', daemon='cyb3rhq-analysisd')
...
def check_push_shared_config(agent, sender, injector=None):
    ...
    try:
        ...
        cyb3rhq_log_monitor.start(timeout=REMOTED_GLOBAL_TIMEOUT, callback=log_callback,
        ...
        wait_to_remoted_update_groups(cyb3rhq_log_monitor)

        keep_alive_agent = multiprocessing.Process(target=keep_alive_until_group_configuration_sent,
                                                   args=(sender,))
        keep_alive_agent.start()

        log_callback = make_callback(pattern=r".*End sending file '.+' to agent '\d+'\.", prefix=r'.*cyb3rhq-remoted.*')
