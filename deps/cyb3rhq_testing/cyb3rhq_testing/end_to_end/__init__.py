...
def get_alert_indexer_api(query, credentials, ip_address, index='cyb3rhq-alerts-4.x-*'):
    """Get an alert from the cyb3rhq-indexer API

    Make a request to the cyb3rhq-indexer API to get the last indexed alert that matches the values passed in
    query.

    Args:
        ip_address (str): cyb3rhq-indexer IP address.
        index (str): Index in which to search for the alert.
        query (dict): Query to send to the API.
        credentials(dict): cyb3rhq-indexer credentials.
...
def delete_index_api(credentials, ip_address, index='cyb3rhq-alerts-4.x-*'):
    """Delete indices from cyb3rhq-indexer using its API.

    Make a request to the cyb3rhq-indexer API to delete indices that match a given name.
