...
def test_restart(cyb3rhq_params):
    GeneralComponentActions.component_restart(cyb3rhq_params['master'], 'cyb3rhq-manager')

    for indexer_params in cyb3rhq_params['indexers']:
        GeneralComponentActions.component_restart(indexer_params, 'cyb3rhq-indexer')

    GeneralComponentActions.component_restart(cyb3rhq_params['dashboard'], 'cyb3rhq-dashboard')
    GeneralComponentActions.component_restart(cyb3rhq_params['master'], 'filebeat')
...
def test_dashboard_status(cyb3rhq_params):
    assert 'active' in GeneralComponentActions.get_component_status(cyb3rhq_params['dashboard'], 'cyb3rhq-dashboard'), logger.error(f"The Cyb3rhq dashboard in {HostInformation.get_os_name_and_version_from_inventory(cyb3rhq_params['dashboard'])} is not active")
...
def test_manager_api_port(cyb3rhq_params):
    assert Cyb3rhqManager.is_cyb3rhq_api_port_open(cyb3rhq_params['master']),logger.error(f"The Cyb3rhq manager API port in {HostInformation.get_os_name_and_version_from_inventory(cyb3rhq_params['master'])} is closed")


def test_manager_agent_port(cyb3rhq_params):
    assert Cyb3rhqManager.is_cyb3rhq_agent_port_open(cyb3rhq_params['master']),logger.error(f"The Cyb3rhq manager API port in {HostInformation.get_os_name_and_version_from_inventory(cyb3rhq_params['master'])} is closed")


def test_manager_agent_enrollment_port(cyb3rhq_params):
    assert Cyb3rhqManager.is_cyb3rhq_agent_enrollment_port_open(cyb3rhq_params['master']),logger.error(f"The Cyb3rhq manager API port in {HostInformation.get_os_name_and_version_from_inventory(cyb3rhq_params['master'])} is closed")
