...
def test_stop(cyb3rhq_params):
    for workers in cyb3rhq_params['workers']:
        GeneralComponentActions.component_stop(workers, 'cyb3rhq-manager')
    ...
    for worker in cyb3rhq_params['workers']:
        assert 'inactive ' in GeneralComponentActions.get_component_status(workers, 'cyb3rhq-manager'), logger.error(f'The {HostInformation.get_os_name_and_version_from_inventory(worker)} is not active')
